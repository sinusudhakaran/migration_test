...
interface
  ...
  uses Windows, MAPIDefs, MAPIX, MAPITags, SysUtils, Forms,
       edkmdb, edkguid, emsabTag,
       MAPIUtil;

  function HrMailBoxLogon(lplhSession : IMAPISession;
                          lpMDB : IMsgStore;
                          lpszMsgStoreDN : PChar;
                          lpszMailBoxDN : PChar;
                          var lppXManageStore : IExchangeManageStore;
                          var lppMailboxMDB : IMsgStore) : HResult; stdcall;

  function HrGetServerDN(Session:IMAPISession):string;
  //function HrGetServerDN(ProfName,Password:string):string;
  function HrGetMailboxDN(Session:IMAPISession):string;

implementation

function PosFromEnd(SubStr, Str:string):integer;
...
begin
  ...
  if res = S_OK then begin
    res:=ProfAdmin.AdminServices(PChar(ProfName),PChar(Password),Application.Handle,0,ServAdmin);
    if res = S_OK then begin
