...
//------------------------------------------------------------------------------
//  Export for MYOB General Journal format
//
//  Steve Teare 9-May-2004
...
uses
  TransactionUtils, Classes, Traverse, Globals, GenUtils, bkDateUtils,
  TravUtils, LogUtil, BaObj32, dlgSelect, BkConst, MoneyDef, SysUtils,
...
begin
   ...
   if AAmount < 0 then begin
     Write(XFile, TAB, TAB); // Debit entries
     AmountStr := FormatAmount((Abs(AAmount) - Abs(AGST))/100.0);
     Write(XFile, AmountStr, TAB); // Credit exc tax
     AmountStr := FormatAmount(Abs(AAmount)/100.0);
     Write(Xfile, AmountStr, TAB); // Credit inc tax
   end else begin
     AmountStr := FormatAmount((AAmount - AGST)/100.0);
     Write(XFile, AmountStr, TAB); // Debit exc tax
     AmountStr := FormatAmount(AAmount/100.0);
     Write(Xfile, AmountStr, TAB); // Debit inc tax
     Write(XFile, TAB, TAB); // Credit entries
   end;
   Write(XFile, JobCode , TAB); // Job
   Write(XFile, AGSTClass, TAB); // Tax Code
   Write(XFile, '$0.00', TAB); // Non-GST/LCT Amount (blank)
   Write(XFile, '$', Abs(AGST)/100.0:0:2, TAB); // GST amount
   Write(XFile, '$0.00', TAB); // LCT Amount (Blank)
   Write(XFile, '$0.00', TAB); // Import Duty Amount (Blank)
  ...
  with MyClient.clFields, Transaction^, Bank_Account.baFields do begin
    ...
    if (baAccount_Type = btBank) then begin
      ...
      MYOBWrite( JnlID,
                 ...
                 0, 'N-T', Indicate, Transaction^.txJob_Code);
    end;
  end;
