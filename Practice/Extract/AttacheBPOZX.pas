...
{
   Attache Business Partner OZ Version

   Same as NZ. FOR TESTING ONLY.
...
Begin
   ...
   Nar := TrimSpacesS(Copy(ReplaceCommasAndQuotes(ANarration), 1, NarrationLength));
   if Nar = '' then
      Nar := ' ';
   Write( XFile, Nar, ',' );
   ...
   Write( XFile, ReplaceCommasAndQuotes(MakeBPCode(AAccount)), ',');
...
Begin
   ...
   Msg := 'Extract data [Attache Business Partner OZ format] from '+BkDate2Str( FromDate ) + ' to ' + bkDate2Str( ToDate );
