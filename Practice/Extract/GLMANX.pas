...

{
   ...
   This is the extract data program for the latest version of

   MYOB Accountants Office for New Zealand (previously CA-Systems GL)
}
...
Begin
   ...
   Begin
      ...
      Begin
         ...
         { EXT_REF}        S := ReplaceCommasAndQuotes(GetReference(TransAction,Bank_Account.baFields.baAccount_Type));
                           if Trim( S) = '' then
                             Write( XFile, '"0",')
                           else
                             Write( XFile, '"',S,'",' );
         ...
         { EXT_GSTACC }       write( XFile, '"', clGST_Class_Codes[ txGST_Class ], '",' );
                           end;
...
Begin
   ...
   Begin
      ...
      { EXT_REF}        S := ReplaceCommasAndQuotes(getDsctReference(Dissection,Transaction,Traverse.Bank_Account.baFields.baAccount_Type));
                        if Trim( S) = '' then
                          Write( XFile, '"0",')
                        else
                         Write( XFile, '"',S,'",' );
      ...
      { EXT_GSTACC }       write( XFile, '"', clGST_Class_Codes[ dsGST_Class ], '",' );
                        end;
...
Begin
   ...
   Begin
      ...
      Begin
         ...
         { EXT_REF}        S := GetReference(TransAction,Bank_Account.baFields.baAccount_Type);
                           S := ReplaceCommasAndQuotes( S );
                           if Trim( S) = '' then
                             Write( XFile, '"0",')
                           else
                             Write( XFile, '"',S,'",' );
...
Begin
   ...
   Begin
      ...
      Begin
         ...
         Begin
            if not CASystemsGSTOK( dsGST_Class ) then
               HasInvalidGSTRates := True;
            dThis := dsNext;
         end;
      end
      else
         if not CASystemsGSTOK( txGST_Class ) then
               HasInvalidGSTRates := True;
   end;
   if DebugMe then LogUtil.LogMsg(lmDebug, UnitName, ThisMethodName + ' Ends' );
end;
...
Begin
   ...
   Msg := 'Extract data [GLMAN format] from '+BkDate2Str( FromDate ) + ' to ' + bkDate2Str( ToDate );
   ...
   Try
      ...
      begin
         ...
         if HasUncodes then
         Begin
            HelpfulInfoMsg( 'There are uncoded entries.  You must code all the entries before you can extract them.',  0 );
            exit;
            //if AskYesNo( 'Extract Uncoded Entries?',
            //             'Some of the entries are uncoded. Are you sure you want to extract them?',
         ...
         Begin
            ...
            Begin
               ...
               Begin
                  ...
                  Begin
                     HelpfulInfoMsg( 'Account "'+baBank_Account_Number+'" has entries with invalid GST Types. ' +
                     'The GST types must be set to Input Tax/Output Tax/Exempt or Zero Rated before you can extract them.',  0 );
                     Exit;
                  end;
               end;
            end;
