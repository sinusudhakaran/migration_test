...
uses
  ...
  Registry, windows, WinUtils, Globals;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if bmid = '' then
                aMsg := 'The BankLink ID for the selected ledger path is not set.'#13#13 +
                        'Do you still want to refresh the chart?'
              else
              aMsg := 'The BankLink ID for the selected ledger path is set to ' + bmid +'. '#13#13 +
...
begin
  ...
  begin
    ...
    begin
      ...
      if not NoMsgIfOK then
        HelpfulInfoMsg( 'The BankLink ID is set correctly for ledger path ' + LedgerPath, 0);
    end
    else
    begin
      if bmid = '' then
      begin
        ...
        aMsg := 'The BankLink ID in ledger path ' + LedgerPath + ' is not currently set.'#13#13+
      ...
      begin
        ...
        begin
          ...
          aMsg := 'The BankLink ID in ledger path ' + LedgerPath + ' is currently set to "' +
                bmid + '"."'#13#13 +
        ...
        begin
          ...
          aMsg := 'The BankLink ID in ledger path ' + LedgerPath + ' does not match your existing client code.';
          HelpfulErrorMsg( aMSg, 0);
          exit;
        end;
      end;

      if AskYesNo( 'Set BankLink ID', aMsg, dlg_Yes, 0) = dlg_Yes then
      begin
        try
          ...
          On e : exception do
            HelpfulErrorMsg( 'The BankLink ID could not be set due to the following error:'#13#13+
                             E.Message + '.', 0);
        end;
      end;
    end;
  ...
  begin
    if myobao_utils.GetLastMYOBErrorNo = 67 then
      aMsg := 'Unable to check BankLink ID. ' + LedgerPath + ' may not be a valid ledger path.'
    else
      aMsg := 'Unable to check BankLink ID. ' + GetLastMYOBError + '.';
