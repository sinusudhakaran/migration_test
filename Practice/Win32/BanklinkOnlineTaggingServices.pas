...
type
  ...
  private
    ...
    class function IsExportableBankAccount(BankAccount: TBank_Account; CoreAccountId: Integer; ClientBankAccountVendors: TBloArrayOfDataPlatformBankAccount): Boolean; static;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        CoreAccountId := GetCoreAccountId(BankAccount.baFields.baBank_Account_Number);

        if IsExportableBankAccount(BankAccount, CoreAccountId, ClientBankAccountVendors) and HasExportableTransactions(BankAccount, MaxTransactionDate) then
        begin
          BankAccount.baFields.baCore_Account_ID := CoreAccountId;
          
          BankAccountNode := ParentNode.OwnerDocument.CreateElement('BKBankAccount', '');

          BankAccountNode.Attributes['CoreId'] := CoreAccountId;
...
class function TBanklinkOnlineTaggingServices.IsExportableBankAccount(BankAccount: TBank_Account; CoreAccountId: Integer; ClientBankAccountVendors: TBloArrayOfDataPlatformBankAccount): Boolean;
begin
  Result := not (BankAccount.IsManual or BankAccount.IsAJournalAccount) and (CoreAccountId > 0) and IsBankAccountTagged(BankAccount, ClientBankAccountVendors);
end;
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      if IsExportableBankAccount(BankAccount, GetCoreAccountId(BankAccount.baFields.baBank_Account_Number), ClientBankAccountVendors) then
                      begin
                        for TransactionIndex := 0 to BankAccount.baTransaction_List.ItemCount - 1 do
                        begin
                          Transaction := BankAccount.baTransaction_List[TransactionIndex];
