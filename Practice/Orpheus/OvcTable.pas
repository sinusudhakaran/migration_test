...
const
  tbNotSorted = -1; {BankLink modification}
...
type
  ...
  protected {private}
      ...
      FSortedCol      : TColNum;             {BankLink modification}
      ...
      FVSThumbChanged     : TRowNotifyEvent;        {banklink - thumb changing on vertical scroll bar}
    ...
    protected
      ...
      function GetVisibleRows: integer;  {BankLink mjch 12.11.02}
      ...
      procedure SetSortedCol(ColNum : TColNum); {BankLink Modification}
      ...
      function  tbEditCellHasFocus(FocusHandle : HWND) : boolean;
      ...
      procedure DoVSThumbChanged( RowNum : TRowNum ); Virtual;
      ...
      procedure ctimQuerySortedCol(var Msg : TMessage); message ctim_QuerySortedCol;  {BankLink modification}
      ...
      property SortedCol : TColNum            {BankLink modification}
         read FSortedCol write SetSortedCol
         default tbNotSorted;
      ...
      property VisibleRows : integer    {BankLink mjch 12.11.02}
         read GetVisibleRows;
      ...
      {BankLink modification}
      property OnVSThumbChanged : TRowNotifyEvent
         read FVSThumbChanged write FVSThumbChanged;
         
    public
      CommandOnEnter : integer; {BankLink Modification}
  ...
  TOvcTable = class(TOvcCustomTable)
    public
      ...
      property RowOffset;
      property TableState;
      property SortedCol;  {BankLink modification}
      property VisibleRows;{BankLink mjch 12.11.02}
    published
      {Properties}
      property LockedRows;
      property TopRow;
      ...
      property OnVSThumbChanged;     {banklink modification}
    end;
...
constructor TOvcCustomTable.Create(AOwner : TComponent);
  begin
    ...
    tbState := [otsNormal];
    CommandOnEnter := ccDown;   {BankLink modification}
...
{--------}
function TOvcCustomTable.GetVisibleRows: integer;   {Banklink modification}
  begin
    result := tbRowNums^.Count;
  end;
{--------}
...
{--------}
{BankLink modification}
procedure TOvcCustomTable.SetSortedCol(ColNum : TColNum);
  begin
    FSortedCol := ColNum;
  end;
{--------}
...
{--------}
function  TOvcCustomTable.tbEditCellHasFocus(FocusHandle : HWND) : boolean;
  var
    ChildHandle : HWND;
  begin
    Result := false;
...
{--------}
{BankLink modification}
procedure TOvcCustomTable.DoVSThumbChanged( RowNum : TRowNum );
  Begin
    if ((ComponentState * [csLoading, csDestroying]) = []) and
       Assigned(FVSThumbChanged ) then
      FVSThumbChanged(Self, RowNum );
  end;
{====================================================================}
...
{--------}
{BankLink modification}
procedure TOvcCustomTable.ctimQuerySortedCol(Var Msg: TMessage);
  begin
    Msg.Result := longint(SortedCol);
  end;
{--------}
      ...
      if (Msg.Pos <> TopRow) then
        begin
          if RowLimit < (16*1024) then
            ...
            NewTopRow := LockedRows
          else begin
            if (RowLimit > (16*1024)) then                             {!!.05}
              Divisor := Succ(RowLimit div $400)                       {!!.05}
            else                                                       {!!.05}
              Divisor := Succ(RowLimit div $40);                       {!!.05}
            if (Msg.Pos = RowLimit div Divisor) then
              NewTopRow := pred(RowLimit)
            else
              NewTopRow := Msg.Pos * Divisor;
          end;
          DoVSThumbChanged( NewTopRow );
        end;
    end;
  {------}
  ...
  begin
    ...
    if (Msg.ScrollCode = SB_ENDSCROLL) or
       ((Msg.ScrollCode = SB_THUMBTRACK) and
        (not (otoThumbTrack in Options))) then
      begin
        If Msg.ScrollCode = SB_THUMBTRACK then TrackThumb;
