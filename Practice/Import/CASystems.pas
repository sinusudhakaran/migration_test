...
uses
  ...
  BKUtil32, glConst, WinUtils, Software, MYOBAO;
 ...
 try
   with Reader do begin
     ...
     for i := 1 to MAX_GST_CLASS do with MyClient.clFields do
       if clGST_Rates[i,1] <> 0 then GSTInfo := true;

     if not GSTInfo then with MyClient.clFields do begin
   ...
   with reader do for i := 1 to Header.NrOfRecs do begin
     ...
     if (return = 0) and not Deleted then begin
       ...
       if Code<>'' then begin
         ...
         GetField('ACC_GSTIND',CharCType,TaxRate);

         Case TaxRate of
            'I' : GST_Class := 1;    //input  = expenses
            'O' : GST_Class := 2;    //output = sales
            'E' : GST_Class := 3;
            'Z' : GST_Class := 4;
            'N' : GST_Class := 5;
         else
...
procedure RefreshChart;
begin
  if Software.CanUseMYOBAO_DLL_Refresh( MyClient.clFields.clCountry, MyClient.clFields.clAccounting_System_Used ) then
    MYOBAO.RefreshChart
  else
    RefreshChartStd;
end;
