...
(*
   The Australian User who specified this format was:
...
uses
  ...
  ChartUtils, GenUtils, Progress, ErrorMoreFrm,stStrs, LogUtil, WinUtils;
      ...
      try
        ...
        try
          ...
          While not EOF( F ) do Begin
             ...
             S := TrimSpacesAndQuotes( ExtractAsciiS( 4, Line, ',', '"' ) );
             If ( S='0' ) OR ( S='1' ) OR ( S='2' ) OR ( S='3' ) then Begin
               ACode := TrimSpacesAndQuotes( ExtractAsciiS( 1, Line, ',', '"' ) );
               ADesc := TrimSpacesAndQuotes( ExtractAsciiS( 2, Line, ',', '"' ) );
