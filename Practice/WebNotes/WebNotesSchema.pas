...
//------------------------------------------------------------------------------
{
   ...
   Description:
      ...
      Used in 
         CheckWebNotesdata
         WebNotesService   (Upload to webnotes)
...
(* ^^^^^^^^^^^^  DO NOT CHANGE UNLESS THE SCHEMA AT  HE WEBNOTES END DEMANDS IT ^^^^^^^^^ *)
implementation
uses
  OmniXMLUtils,
  ZLibExGZ,
...
procedure SetBoolAttr(var OnNode: IXMLNode; Name: string;
  Value: Boolean);
begin
   if Value then
...
procedure AddBooleanOption(var ToNode: IXMLNode; name: string;
  Value: Boolean);
var lNode : IXMLNode;
begin
...
procedure SetCodeByAttr(var OnNode: IXMLNode; Name: string;
  Value: Byte);
begin
...
function GetCodeByAttr(FromNode: IXMLNode; Name: string): Integer;
begin
   if Assigned(FromNode.AttributeNodes.FindNode(Name)) then begin
...
procedure SetDateAttr(var OnNode: IXMLNode; Name: string;
  Value: TstDate);
begin
   if Value <> 0 then
...
procedure SetQtyAttr(var OnNode: IXMLNode; Name: string;
  Value: Money);
begin
  if Value <> 0 then
...
procedure SetRateAttr(var OnNode: IXMLNode; Name: string;
  Value: Money);
begin
   if Value <> 0 then
      OnNode.Attributes [Name] := (Value / 10000);
...
begin
   ...
   else             OnNode.Attributes [Name] := 'Generated';
...
procedure SetTextAttr(var OnNode: IXMLNode; Name,
  Value: string);
begin
  if Value > '' then
...
procedure SetUPIState(var OnNode: IXMLNode; Name: string;
  Value: Byte);
begin
