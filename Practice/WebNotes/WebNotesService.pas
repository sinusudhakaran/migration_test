unit WebNotesService;

//------------------------------------------------------------------------------
{
   Title: WebNotesService
   ...
   Remarks:
      ...
      This is Upload to Webnotes only, should rename...
...
uses
 CheckWebnotesData,
  ...
  EWebNotesServiceError = class(Exception)
  end;



  TWebNotesService = class(Tobject)
...
begin
   ...
   service := TWebNotesService.Create;
   try  try
      // Setup the basics..
      RefreshAdmin;
      ...
      if WebClient.UpLoad(Batch, Reply) then begin
         // Deal with the reply...
         if Service.TestReply(Reply) then begin
            inc(EntriesCount, Service.EntriesCount);
            NotifyClient := Service.NotifyClient;

            if Service.NotifyClient
            and (Service.ReplyURL > '') then
               EmailMessage := format
      ...
      on E:Exception do begin
         ...
         raise EWebNotesServiceError.Create( E.Message );
      end;
   end;
   finally
      WebClient.Free;
...
function TWebNotesService.AddAccount(Account: TBank_Account; var AccountsNode: IXMLNode): Boolean;
...
var ti : Integer;
    ...
    begin
       ...
       if TransactionsNode = nil then begin
...
begin
   ...
   for ti := Account.baTransaction_List.First to Account.baTransaction_List.Last do begin
      Trans := Account.baTransaction_List.Transaction_At(ti);// Easier Debugging...

      (**********************************************************************)
      if IncludeTrans(Account,Trans) then begin
         //set  ID, this will be used to synchronise the transaction
         if Trans.txECoding_Transaction_UID = 0 then begin
         ...
         if FClient.clFields.clECoding_Send_Superfund
         and Software.CanUseSuperFundFields(FClient.clFields.clCountry,FClient.clFields.clAccounting_System_Used) then begin
   ...
   if Result // Something to add
   and IsScheduledReport
   and (SchdSummaryList <> nil) then begin
...
procedure TWebNotesService.AddChart(ChartNode: IXMLnode);
var I: Integer;
    EntryNode : IXMLnode;
begin
   ...
   for I := FClient.clChart.First to FClient.clChart.Last do
      with FClient.clChart.Account_At(I)^ do begin
         // add The Node...
...
procedure TWebNotesService.AddClient(BatchNode: IXMLNode);
var
   lNode,
   TaxesNode: IXMLNode;
   acNo: Integer;
   ...
   begin
       ...
       for I := low(FClient.clExtra.ceTAX_Rates) to High(FClient.clExtra.ceTAX_Rates) do
          for J := 1 to 5 do
             if FClient.clExtra.ceTAX_Rates[I,J] <> 0 then begin
   ...
   begin
       ...
       for I := low(FClient.clFields.clGST_Class_Codes) to High(FClient.clFields.clGST_Class_Codes) do
          if (FClient.clFields.clGST_Class_Codes[I] > '') then begin
             ...
             for J := 2 to 5 do
                if (FClient.clFields.clGST_Rates [I,J] <> 0)
                or (FClient.clFields.clGST_Applies_From[J] <> 0) then begin
...
begin // AddClient
   ...
   lNode := BatchNode.AddChild(nConfiguration);
    ...
    // Chart...
    if (not FClient.clFields.clECoding_Dont_Send_Chart)
    and (FClient.clChart.ItemCount > 0)  then begin
      AddChart(BatchNode.AddChild(nChart))
    end;

    // Payees
    if (not FClient.clFields.clECoding_Dont_Send_Payees)
    and (FClient.clPayee_List.ItemCount > 0)  then begin
       AddPayees(BatchNode.AddChild(nPayees));
    end;

    // Jobs
    if (not FClient.clExtra.ceECoding_Dont_Send_Jobs)
    and (FClient.clJobs.ItemCount > 0) then begin
       AddJobs(BatchNode.AddChild(nJobs))
    end;

    // add The accounts...
    lnode := BatchNode.AddChild(nAccounts);
    for acNo := 0 to Pred(FClient.clBank_Account_List.ItemCount) do begin
       ...
       if IncludeAccount(BA) then begin
...
procedure TWebNotesService.AddJobs(JobsNode: IXMLNode);
var I: Integer;
    JobNode : IXMLNode;
begin
   ...
   for I := FClient.clJobs.First to FClient.clJobs.last do
      with FClient.clJobs.Job_At(I)^ do begin
...
procedure TWebNotesService.AddPayees(PayeesNode: IXMLNode);
var I, L: Integer;
...
begin
   ...
   for I := FClient.clPayee_List.First to FClient.clPayee_List.Last do
      with FClient.clPayee_List.Payee_At(I) do begin
         ...
         for L := pdLines.First to pdLines.Last do
            with pdLines.PayeeLine_At(L)^ do begin
               ...
               if plLine_Type = BKCONST.pltDollarAmt then begin
...
constructor TWebNotesService.Create;
begin
   inherited Create;
   Reset;
end;

destructor TWebNotesService.Destroy;
begin
   inherited Destroy;
end;


procedure TWebNotesService.FinaliseSheduled(Success: Boolean);
var I: Integer;
begin
   if SchdSummaryList = nil then
      Exit;

    if Assigned(FirstSummaryRec) then begin
    ...
    for I := 0 to SchdSummaryList.Count - 1 do begin
...
function TWebNotesService.GetBatchXML: string;
...
function TWebNotesService.IncludeAccount(var Account: TBank_Account): Boolean;
var
...
function TWebNotesService.IncludeTrans(var Account: TBank_Account; Trans: pTransaction_Rec): boolean;
begin
   Result := false;
   ...
   if (Client.clFields.clECoding_Entry_Selection = esUncodedOnly)
   and (Trans.txAccount > '') then
      Exit;

   //check date of last transaction sent if scheduled reports run

   if IsScheduledReport then begin
      if (Trans.txDate_Effective > Account.baFields.baTemp_Date_Of_Last_Trx_Printed) // In the Range
      and (not IsUPCFromPreviousMonth(Trans.txDate_Effective, Trans.txUPI_State, ReportStartDate))
...
procedure TWebNotesService.Reset;
begin
   AccountsFound := 0;
   AccountsExported := 0;
   FirstSummaryRec := nil;
...
procedure TWebNotesService.SetClient(const Value: TClientObj);
begin
  FClient := Value;
end;


procedure TWebNotesService.SetDateFrom(const Value: Integer);
begin
  FDateFrom := Value;
end;

procedure TWebNotesService.SetDateTo(const Value: Integer);
begin
  FDateTo := Value;
end;

procedure TWebNotesService.SetIsScheduledReport(const Value: boolean);
begin
  FIsScheduledReport := Value;
end;

procedure TWebNotesService.SetNotifyClient(const Value: Boolean);
begin
  FNotifyClient := Value;
end;

procedure TWebNotesService.SetReplyURL(const Value: string);
begin
  FReplyURL := Value;
end;

procedure TWebNotesService.SetScheduledReportPrintAll(const Value: boolean);
begin
...
function TWebNotesService.TestReply(Reply: string): Boolean;
...
begin
   ...
   try
      ...
      if TestResponse(ReplyNode,'ResponseType') then begin
