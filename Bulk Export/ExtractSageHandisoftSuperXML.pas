unit ExtractSageHandisoftSuperXML;
...
begin
   Result := False;
   case Index of
   SAGE_HANDISOFT_SUPER_XML : begin
          EType.Index := SAGE_HANDISOFT_SUPER_XML;
          EType.Code  := SageHandisoftSuperXml;
          EType.Description := 'Sage HandiSoft SuperFund XML';
...
begin
   ...
   if Session.IniFile <> nil then
     case Session.Index of
       SAGE_HANDISOFT_SUPER_XML : Result :=  GetPrivateProfileText(SageHandisoftSuperXml,keyExtractPath, '', string(Session.IniFile));
     end;
end;
...
begin
   ...
   try
      ...
      ldlg.Caption := 'Sage Handisoft Super XML Setup';
      ldlg.eFilename.Text := GetExtractPath(Session);
      ldlg.rbSplit.Checked := Bool(GetPrivateProfileInt(SageHandisoftSuperXml,keySeparateClientFiles,0,Session.IniFile));
      if ldlg.ShowModal = mrOK then begin
         WritePrivateProfileString(SageHandisoftSuperXml,keyExtractPath,pchar(Trim(ldlg.eFilename.Text)), Session.IniFile);
         WritePrivateProfilestring(SageHandisoftSuperXml,keySeparateClientFiles,pchar(intToStr(Integer(ldlg.rbSplit.Checked))),Session.IniFile);
...
begin
   Result := 0;
   if Index = SAGE_HANDISOFT_SUPER_XML then
      case Feature of
...
begin
   ...
   if OutputFileName > '' then begin
      // Can Have a Go...
      FilePerClient := Bool(GetPrivateProfileInt(SageHandisoftSuperXml,keySeparateClientFiles,0,Session.IniFile));
...
begin
   ...
   with ExtractFieldHelper do begin
     ...
     AddFieldNode(LTransactionNode, 'Type', GetField(f_SFTransID));
     AddFieldNode(LTransactionNode, 'Sub_Type', GetField(f_SFTransCode));
     ...
     AddFieldNode(LTransactionNode, 'UnFranked_Amount', GetField(f_UnFranked));
     AddFieldNode(LTransactionNode, 'Franked_Amount', GetField(f_Franked));
     AddFieldNode(LTransactionNode, 'Franking_Credit', GetField(f_Imp_Credit));
...
begin
   ExtractFieldHelper.SetFields(Session.Data);
   case Session.Index of
     SAGE_HANDISOFT_SUPER_XML:  WriteXMLFields(Session);
   end;
   Result := er_OK;
end;
...
begin
   ...
   ef_SessionStart  : case Session.Index of
                        SAGE_HANDISOFT_SUPER_XML: Result := TrapException(Session,XMLSessionStart);
                      end;
...
//   ef_Dissected     : Result := TrapException(Session,Transaction);
   ...
   ef_CanConfig    : case Session.Index of
                       SAGE_HANDISOFT_SUPER_XML: begin // Also Doubles as get initial settings...
                            Result := er_OK;
                       end;
                     end;
   ef_DoConfig     : case Session.Index of
                       SAGE_HANDISOFT_SUPER_XML: Result := DoXMLConfig(Session);
                     end;
