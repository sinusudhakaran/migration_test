...
begin
   ...
   case Index of
   PRO_SUPER_XML : begin
          EType.Index := PRO_SUPER_XML;
          EType.Code  := ProSuperXml;
          EType.Description := 'ProSuper XML';
...
begin
   ...
   if Session.IniFile <> nil then
     ...
     PRO_SUPER_XML : Result :=  GetPrivateProfileText(ProSuperXml,keyExtractPath, '', string(Session.IniFile));
     end;
end;
...
begin
   ...
   try
      ...
      ldlg.rbSplit.Checked := Bool(GetPrivateProfileInt(ProSuperXml,keySeparateClientFiles,0,Session.IniFile));
      if ldlg.ShowModal = mrOK then begin
         WritePrivateProfileString(ProSuperXml,keyExtractPath,pchar(Trim(ldlg.eFilename.Text)), Session.IniFile);
         WritePrivateProfilestring(ProSuperXml,keySeparateClientFiles,pchar(intToStr(Integer(ldlg.rbSplit.Checked))),Session.IniFile);
...
begin
   ...
   if OutputFileName > '' then begin
      // Can Have a Go...
      FilePerClient := Bool(GetPrivateProfileInt(ProSuperXml,keySeparateClientFiles,0,Session.IniFile));
...
begin
   ExtractFieldHelper.SetFields(Session.Data);
   case Session.Index of
   PRO_SUPER_XML:  WriteXMLFields(Session);
   end;
   Result := er_OK;
end;
...
begin
   ...
   ef_SessionStart  : case Session.Index of
                        PRO_SUPER_XML: Result := TrapException(Session,XMLSessionStart);
                      end;
...
//   ef_Dissected     : Result := TrapException(Session,Transaction);
   ...
   ef_CanConfig    : case Session.Index of
                       PRO_SUPER_XML: begin // Also Doubles as get initial settings...
                            Result := er_OK;
                       end;
                     end;
   ef_DoConfig     : case Session.Index of
                       PRO_SUPER_XML: Result := DoXMLConfig(Session);
                     end;
