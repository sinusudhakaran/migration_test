...
begin
   Result := False;
   case Index of
   SIMPLE_CLASS_SUPER : begin
          EType.Index := SIMPLE_CLASS_SUPER;
          EType.Code  := ClassSuperBasicCode;
          EType.Description := 'ClassSuper';
...
begin
   ...
   if Session.IniFile <> nil then
     case Session.Index of
       SIMPLE_CLASS_SUPER : Result :=  GetPrivateProfileText(ClassSuperBasicCode,
                                                             keyExtractPath, '',
                                                             string(Session.IniFile));
     end;
end;
...
function DoClassSuperConfig (var Session: TExtractSession): Integer; stdcall;
var ldlg: TXMLConfig;
begin
   ...
   try
      ...
      ldlg.Caption := 'ClassSuper Export Setup';
      ldlg.eFilename.Text := GetExtractPath(Session);
      ldlg.rbSplit.Checked := Bool(GetPrivateProfileInt(ClassSuperBasicCode,
                                                        keySeparateClientFiles,0,
                                                        Session.IniFile));
      if ldlg.ShowModal = mrOK then begin
         WritePrivateProfileString(ClassSuperBasicCode,
                                   keyExtractPath,pchar(Trim(ldlg.eFilename.Text)),
                                   Session.IniFile);
         WritePrivateProfilestring(ClassSuperBasicCode,
                                   keySeparateClientFiles,
                                   pchar(intToStr(Integer(ldlg.rbSplit.Checked))),
                                   Session.IniFile);
...
begin
   Result := 0;
   if Index = SIMPLE_CLASS_SUPER then
      case Feature of
...
function ClassSuperSessionStart(var Session: TExtractSession): Integer; stdcall;
var
  Path: string;
begin
   Result := er_OK;
   ...
   while not ValidFilePath(Path) do begin
     if DoClassSuperConfig(Session) = er_OK then begin
       OutputFileName := FindExtractPath(Session);
       Path := ExtractFilePath(OutputFileName);
     end else begin
   ...
   if OutputFileName > '' then begin
      // Can Have a Go...
      FilePerClient := Bool(GetPrivateProfileInt(ClassSuperBasicCode,
                                                 keySeparateClientFiles,0,
                                                 Session.IniFile));
...
procedure WriteClassSuperFields(var Session: TExtractSession);
var LTrans: IXMLNode;
...
begin
   ExtractFieldHelper.SetFields(Session.Data);
   case Session.Index of
   SIMPLE_CLASS_SUPER:  WriteClassSuperFields(Session);
   end;
   Result := er_OK;
end;
...
begin
   ...
   ef_SessionStart  : case Session.Index of
                        SIMPLE_CLASS_SUPER: Result := TrapException(Session,ClassSuperSessionStart);
                      end;
   ...
   ef_CanConfig    : case Session.Index of
                       SIMPLE_CLASS_SUPER: begin // Also Doubles as get initial settings...
                            Result := er_OK;
                       end;
                     end;
   ef_DoConfig     : case Session.Index of
                       SIMPLE_CLASS_SUPER: Result := DoClassSuperConfig(Session);
                     end;
