...
begin
   ...
   case Index of
   SIMPLEXML : begin
          EType.Index := SIMPLEXML;
          EType.Code  := XMLBasiccode;
          EType.Description := 'Generic XML';
...
begin
   ...
   if Session.IniFile <> nil then
     ...
     SIMPLEXML : Result :=  GetPrivateProfileText(XMLBasicCode,keyExtractPath, '', string(Session.IniFile));
     end;
end;
...
begin
   ...
   try
      ...
      ldlg.rbSplit.Checked := Bool(GetPrivateProfileInt(XMLBasicCode,keySeparateClientFiles,0,Session.IniFile));
      if ldlg.ShowModal = mrOK then begin
         WritePrivateProfileString(XMLBasicCode,keyExtractPath,pchar(Trim(ldlg.eFilename.Text)), Session.IniFile);
         WritePrivateProfilestring(XMLBasicCode,keySeparateClientFiles,pchar(intToStr(Integer(ldlg.rbSplit.Checked))),Session.IniFile);
...
begin
   ...
   if OutputFileName > '' then begin
      // Can Have a Go...
      FilePerClient := Bool(GetPrivateProfileInt(XMLBasicCode,keySeparateClientFiles,0,Session.IniFile));
...
begin
   ExtractFieldHelper.SetFields(Session.Data);
   case Session.Index of
   SIMPLEXML:  WriteXMLFields(Session);
   end;
   Result := er_OK;
end;
...
begin
   ...
   ef_SessionStart  : case Session.Index of
                        SIMPLEXML: Result := TrapException(Session,XMLSessionStart);
                      end;
   ...
   ef_CanConfig    : case Session.Index of
                       SIMPLEXML: begin // Also Doubles as get initial settings...
                            Result := er_OK;
                       end;
                     end;
   ef_DoConfig     : case Session.Index of
                       SIMPLEXML: Result := DoXMLConfig(Session);
                     end;
