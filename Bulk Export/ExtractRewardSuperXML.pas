...
begin
   Result := False;
   case Index of
   REWARD_SUPER_XML : begin
          EType.Index := REWARD_SUPER_XML;
          EType.Code  := RewardSuperXml;
          EType.Description := 'RewardSuper XML';
...
begin
   ...
   if Session.IniFile <> nil then
     ...
     REWARD_SUPER_XML : Result :=  GetPrivateProfileText(RewardSuperXml,keyExtractPath, '', string(Session.IniFile));
     end;
end;
...
begin
   ...
   try
      ...
      ldlg.Caption := 'Banklink XML Setup';
      ldlg.eFilename.Text := GetExtractPath(Session);
      ldlg.rbSplit.Checked := Bool(GetPrivateProfileInt(RewardSuperXml,keySeparateClientFiles,0,Session.IniFile));
      if ldlg.ShowModal = mrOK then begin
         WritePrivateProfileString(RewardSuperXml,keyExtractPath,pchar(Trim(ldlg.eFilename.Text)), Session.IniFile);
         WritePrivateProfilestring(RewardSuperXml,keySeparateClientFiles,pchar(intToStr(Integer(ldlg.rbSplit.Checked))),Session.IniFile);
...
begin
   Result := 0;
   if Index = REWARD_SUPER_XML then
      case Feature of
...
function XMLSessionStart(var Session: TExtractSession): Integer; stdcall;
var
  Path: string;
begin
   Result := er_OK;
   ...
   if OutputFileName > '' then begin
      // Can Have a Go...
      FilePerClient := Bool(GetPrivateProfileInt(RewardSuperXml,keySeparateClientFiles,0,Session.IniFile));
...
begin
   ...
   AddFieldNode(FAccountNode,'Account_Contra_Code',CurrentContra);
...
begin
   ...
   with ExtractFieldHelper do begin
     ...
     AddFieldNode(LTransactionNode,'Narration',CleanTextField(GetField(f_Narration)));
     AddFieldNode(LTransactionNode,'Reference',CleanTextField(GetField(f_Reference)));

     //Chart Code
     if Session.ExtractFunction = ef_Dissected then begin
       //Can't output code if dissected 
       AddFieldNode(LTransactionNode, 'Code', '');
       AddFieldNode(LTransactionNode, 'Code_Desc', '');
     end else begin
       //Output code if if exists
     ...
     AddFieldNode(LTransactionNode,'Quantity',GetField(f_Quantity));

     // Supper fields

     AddFieldNode(LTransactionNode,'CGT_Transaction_Date',GetField(f_CGTDate));
     ...
     AddFieldNode(LTransactionNode,'Tax_Free_Distribution',GetField(f_TF_Dist));

     AddFieldNode(LTransactionNode,'Tax_Exempt_Distribution',GetField(f_TE_Dist));
     AddFieldNode(LTransactionNode,'Tax_Defered_Distribution',GetField(f_TD_Dist));
     AddFieldNode(LTransactionNode,'TFN_Credit',GetField(f_TFN_Credit));
     AddFieldNode(LTransactionNode,'Foreign_Income',GetField(f_Frn_Income));
     AddFieldNode(LTransactionNode,'Foreign_Credit',GetField(f_Frn_Credit));

     AddFieldNode(LTransactionNode,'Other_Expenses',GetField(f_OExpences));

     AddFieldNode(LTransactionNode,'Indexed_Capital_Gain',GetField(f_CGI));
     AddFieldNode(LTransactionNode,'Discount_Capital_Gain',GetField(f_CGD));
     AddFieldNode(LTransactionNode,'Other_Capital_Gain',GetField(f_CGO));
     AddFieldNode(LTransactionNode,'Member_Component',CleanTextField(GetField(f_MemComp)));
...
begin
   ExtractFieldHelper.SetFields(Session.Data);
   case Session.Index of
   REWARD_SUPER_XML:  WriteXMLFields(Session);
   end;
   Result := er_OK;
end;
...
begin
   ...
   ef_SessionStart  : case Session.Index of
                        REWARD_SUPER_XML: Result := TrapException(Session,XMLSessionStart);
                      end;
...
//   ef_Dissection    : Result := TrapException(Session,Transaction);
   ...
   ef_CanConfig    : case Session.Index of
                       REWARD_SUPER_XML: begin // Also Doubles as get initial settings...
                            Result := er_OK;
                       end;
                     end;
   ef_DoConfig     : case Session.Index of
                       REWARD_SUPER_XML: Result := DoXMLConfig(Session);
                     end;
