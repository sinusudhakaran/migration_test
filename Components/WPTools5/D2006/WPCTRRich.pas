...
//******************************************************************************
// WPCTRRich - WPTools 5 editor control wrapper to provide the best possible
...
{$IFNDEF DISABLEPRINTER}Printers, {$ENDIF}
  Classes, Controls, Graphics, ExtDlgs, Dialogs, WPRTEDefs, WPRTEPaint,
...
{$ENDIF}
 ...
 {:: This options are used by function PrintPageOnCanvas }
  TPrintPageMode = set of (ppmUseBorders {ignored, always true!}, ppmPaintTransparent,
    ppmPrintFrame, ppmStretchWidth,
    ppmStretchHeight, ppmIgnoreSelection, ppmDrawSmallAsBlock, ppmUseEvents, ppmForPDFOutput);
...
{$IFNDEF T2H}
    ...
    FFastCharAttrIDX: Cardinal;
    FFastCharAttrOld: TAttr; // Buffer the changes
{$IFDEF WINCOM} public {$ENDIF}
    FCommandDiabled: array[0..15] of Cardinal;
    FCommandChecked: array[0..15] of Cardinal;
{$IFDEF WINCOM} protected {$ENDIF}
...
{$ENDIF}
  public
    ...
    procedure DoUpdateEditState; override;
    ...
    {:: This function can be used to print the text. Normally it prints all pages. To print only the
    odd or even pages set the Property PrintParameter.PageSides to wpsOdd or wpsEven. <br>
    The function can also print pages from a list of pages (1,2,5,6). The list has to be passed
    as string parameter. The property PrintParameter.PageSides will be ignored in this case.
    PrintParameter.PageSides will be also ignored by function <see method="PrintPages">(from_page, to_page). }
    function Print(PageRange: string = ''): Boolean;
    {:: @depreciated This procedure is provided for compatibility to WPTools 4 only.
        We recommend to use <see method="PaintPageOnCanvas"> }
    function PrintPageOnCanvas(toCanvas: TCanvas; r: TRect; PageNumber: Integer;
      mode: TPrintPageMode; przooming: Integer): Boolean;
    ...
    // Fast routines - more or less depreciated
    ...
    {:: Creates a new paragraph after current. When inside a table,
        then jump to next cell which will then be cleared. }
    ...
    {:: Insert a paragraph at the current position and return it as result value.
    Please also see <see method="InputParagraphChild"> to create nested paragraphs.<br>
     <br>
     Please note that you need to call Refresh or ReformatAll after having used InputParagraph!  }
    function InputParagraph: TParagraph;
    ...
    procedure FastApplyPProp(par: TParagraph; const prp: TParProps;
      ...
      ppoIndentFirst, ppoindentleft, ppoindentright,
        ...
        ppoalign, ppoborderType, ppocolor, pposhading, ppoNewPage, ppoParID]); //new V5
...
{$IFDEF IWPGUTTER}FGutter: TWPCustomGutter; {$ENDIF}
  ...
  private
    ...
    procedure SetToolBar(x: TWPCustomToolCtrl);
...
{$ENDIF}
  public
    ...
    property WPToolBar: TWPCustomToolCtrl read FToolBar write SetToolBar;
...
{$IFNDEF T2H}
    ...
    {$IFDEF WINCOM}public{$ENDIF} FCursor: TWPRTFDataCursor;
{$ENDIF}
  protected
{$IFNDEF T2H}
...
{$ENDIF}
  ...
  public
    ...
    { This property will	set a flag to create \oulinelevel tags (in test	phase yet) }
...
begin
  ...
  if not PendingUpdate(WPUPD_REFORMAT) then
  begin
...
begin
  ...
  begin
    ...
    if assigned(FToolBar) then
      FToolBar.EnableIcon(0, FGroup, Fnum, state);
  end;
end;
...
procedure TWPCustomRichText.DoUpdateEditState;
var
  selection_marked: Boolean;
  clipboard_not_empty: Boolean;
  sel: TWPSelectionContents;
begin
  inherited DoUpdateEditState;
  sel := Memo.Cursor.SelectionContents;
  selection_marked := TextCursor.IsSelected;
  clipboard_not_empty := Clipboard.HasFormat(FormatWPT) or
    Clipboard.HasFormat(FormatWPT) or Clipboard.HasFormat(FormatHTM) or
    Clipboard.HasFormat(FormatRTF)
    or Clipboard.HasFormat(CF_TEXT)
    or (not (wpcoDontPasteGraphics in ClipboardOptions) and
  ...
  if (WPToolBar <> nil) or (ActionList <> nil) then
  begin
...
begin
  ...
  aPar := Memo.Cursor.active_paragraph;
...
begin
  ...
  UpdateIcon(WPI_GR_PRINT, WPI_CO_Print, not WPNoPrinterInstalled);
  UpdateIcon(WPI_GR_PRINT, WPI_CO_PrintDialog, not WPNoPrinterInstalled);
  ...
  begin
    ...
    else FToolBar.UpdateSelection(wptSize, '', -1);
      ...
      begin
        ...
        begin
          if group = WPI_GR_DISK then
            case num of
              WPI_CO_New:
                if OpenDialog(wpdiaCanClose) then
                begin
                  OpenDialog(wpStartNewDocument);
                  desel := TRUE;
                  Typ := wptNone;
                end;
              WPI_CO_OPEN:
              ...
              case num of
                ...
                WPI_CO_NUMBERS:
                  ...
                  begin
                    ...
                    // This code locates a numbered line. If it finds the number style
                    // it assignes the style to the current line
{$IFDEF LOCATENEARNUMBER} (*
                    while par <> nil do
                    begin
                      if ((par^.numlevel > 0) or (par^.number > 0)) then
                      begin
                        numsty := NumberStyles.FindNumberStyle(par^.number, par^.numlevel);
                        if (numsty <> nil) and not (numsty.Style in [wp_none, wp_bullet]) then break;
                      end;
                      par := par^.prev;
                    end;
                    if (par = nil) then
...
begin
  ...
  if Result = false then
    case DialogType of
      ...
      wpdiaFind: Result := FindDialog;
      wpdiaReplace: Result := ReplaceDialog;
...
begin
  ...
  Font.Style := fst;
  if CurrAttr.Color <> 0 then Font.Color := CurrAttr.NrToColor(CurrAttr.Color)
  else Font.Color := clNone;
  Font.Size := Round(CurrAttr.Size);
  Result := Font;
end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        begin
          Result := LoadFromFile(dia.Filename, false);
    ...
    begin
      ...
      try
       ...
       {$IFDEF USEEXECDIA} if ExecDialog(dia) then {$ELSE}
        if dia.Execute then {$ENDIF}
        begin
          Result := LoadFromFile(dia.Filename, true);
          if Result then LastFileName := dia.Filename;
        end;
      finally
        dia.Free;
...
begin
  ...
  try
    ...
    if dia.Execute then {$ENDIF}
    begin
      if ExtractFileExt(dia.FileName) = '' then
      begin
      ...
      Result := SaveToFile(dia.Filename);
      if Result then
     ...
     if dia.Execute then {$ENDIF}
      begin
        ...
        begin
          ...
          txtobj := TextObjects.Insert(Result);
          if txtobj <> nil then
          begin
...
function TWPCustomRichText.Print(PageRange: string = ''): Boolean;
var str: TStringList;
  printing, nh, nh2: Boolean;
  i, j, a, b: Integer;
  s: string;
  procedure BeginPrintNow;
  begin
    if not printing then BeginPrint;
    printing := TRUE;
  end;
begin
  printing := FALSE;
  nh := not InsertPointAttr.Hidden;
{$IFDEF ALLOWWORDWRAPPRINT}
  nh2 := FALSE;
{$ELSE}
  nh2 := WordWrap;
{$ENDIF}
  if nh2 or (nh and (wpAlwaysHideFieldmarkers in PrintParameter.PrintOptions)) then
  begin
    if wpAlwaysHideFieldmarkers in PrintParameter.PrintOptions then
      InsertPointAttr.Hidden := TRUE;
    if nh2 then WordWrap := FALSE;
    ReformatAll;
  end else
  ...
  try
    if PageRange = '' then
    begin
      if PrintParameter.PageSides = wpsAll then Result := PrintPages(1, MaxInt)
      else if PrintParameter.PageSides = wpsOdd then
      begin
        Result := TRUE;
        BeginPrintNow;
        try
          for i := 1 to PageCount do
            if (i and 1) <> 0 then PrintPages(i, i);
        finally
          if printing then EndPrint;
        end;
      end else if (PrintParameter.PageSides = wpsEven) and (PageCount > 1) then
      begin
        Result := TRUE;
        BeginPrintNow;
        try
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if j > 0 then // a-b
          begin
            a := StrToInt(Copy(s, 1, j - 1));
            b := StrToInt(Trim(Copy(s, j + 1, Length(s) - j)));
            if (a > 0) or (b > 0) then
            begin
              BeginPrintNow;
              Result := TRUE;
...
function TWPCustomRichText.PrintPageOnCanvas(toCanvas: TCanvas; r: TRect; PageNumber: Integer;
  mode: TPrintPageMode; przooming: Integer): Boolean;
var PaintMode: TWPPaintModes;
  pw, ph: Integer;
begin
  if (PageNumber < 0) or (PageNumber >= FMemo.PaintPageCount) then Result := FALSE
  else
  begin
    PaintMode := [];
    if (ppmPaintTransparent in mode) then
       include(PaintMode, wppWhiteIsTransparent);
    if not (ppmIgnoreSelection in mode) then
      include(PaintMode, wppShowSelection);
    if (ppmDrawSmallAsBlock in mode) then
      include(PaintMode, wppTextAsBlocks);
    if (ppmPrintFrame in mode) then
      include(PaintMode, wppShowMargins);
    if (ppmForPDFOutput in mode) then
      include(PaintMode, wppInPaintForwPDF);

    if not (ppmUseEvents in mode) then
      include(PaintMode, wppNoWatermarks);
    if ppmStretchWidth in mode then pw := r.Right - r.Left else pw := 0;
    if ppmStretchHeight in mode then ph := r.Bottom - r.Top else ph := 0;
    if (przooming = 0) or (przooming = 100) then
      FMemo.PaintRTFPage(PageNumber, r.Left, r.Top, pw, ph,
        toCanvas, PaintMode, WPScreenPixelsPerInch, WPScreenPixelsPerInch)
    else
      FMemo.PaintRTFPage(PageNumber, r.Left, r.Top, pw, ph,
        toCanvas, PaintMode, MulDiv(WPScreenPixelsPerInch, 100, przooming),
        MulDiv(WPScreenPixelsPerInch, przooming, 100));
    Result := TRUE;
  end;
end;
...
function TWPCustomRichText.FindDialog: Boolean;
begin
  if FReplaceDialog <> nil then FReplaceDialog.CloseDialog;
  if FFindDialog = nil then FFindDialog := TWPFindDialog.Create(Application);
...
function TWPCustomRichText.ReplaceDialog: Boolean;
begin
...
begin
  ...
  begin
    if AComponent = WPToolBar then WPToolBar := nil
    else if AComponent = WPRuler then WPRuler := nil
    else if AComponent = VRuler then VRuler := nil
    else if AComponent = WPGutter then WPGutter := nil
    else if AComponent = ActionList then ActionList := nil;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Finder.Next(TReplaceDialog(Sender).FindText) then
      begin
        FReplaceDialog_last_was_search := TRUE;
  ...
  try
    ...
    begin
      ...
      begin
        if Next(TReplaceDialog(Sender).FindText) then
        begin
          Changing;
          FReplaceDialog_last_was_search := TRUE;
...
function TWPCustomRichText.BookmarkEdit: Boolean;
var obj: TWPTextObj;
  s: string;
begin
  obj := BookmarkAtCP;
...
begin
  ...
  begin
    if TextCursor.active_paragraph.Cell <> nil then
    begin
      Result := TextCursor.active_paragraph.nextcell;
      Result.ClearText;
    end else
    begin
...
function TWPCustomRichText.InputParagraph: TParagraph;
begin
  if IsEmpty then
  begin
    CheckActiveText;
    ...
    if ActivePosInPar < ActivePar.CharCount then
    begin
      Result := ActivePar.SplitAt(ActivePosInPar, true);
      Result.SplitAt(0, true);
    end
    else Result := ActivePar.SplitAt(ActivePosInPar, true);
  include(Result.prop, paprMustInit);
  Memo.Cursor.MoveTo(Result, 0, false);
end;
...
begin
  if (FFastCharAttrOld.Font <> Attr.Font) or
    (FFastCharAttrOld.Color <> Attr.Color) or
    (FFastCharAttrOld.BGColor <> Attr.BGColor) or
    (FFastCharAttrOld.Style <> Attr.Style) or
    (FFastCharAttrOld.CharSet <> Attr.CharSet) or
    (FFastCharAttrOld.Size <> Attr.Size)
    then
  begin
    ...
    try
      ...
      FFastCharAttrOld := Attr;
      FFastCharAttrIDX := FFastCharAttr.CharAttr;
    finally
      FFastCharAttr.Free;
    end;
  end;
  Result := FFastCharAttrIDX;
end;

procedure TWPCustomRichText.FastInputText(const prp: TParProps);
var par: TParagraph;
  charattr: Cardinal;
{$IFNDEF CLR}i: Integer; pa: PTAttr; {$ENDIF}
begin
  par := ActivePar;
...
begin
  if Cursor.IsSelected then
  begin
    if not Cursor.SelectedTextAttr.AGet(WPAT_Code, Result) then Result := Value;
  end
  else
  begin
    if not Cursor.WritingTextAttr.AGet(WPAT_Code, Result) then Result := Value;
  end;
end;
...
{$IFNDEF DONT_REPORT_DEFAULT_ATTR} // also see DONT_REPORT_DEFAULT_ATTR_STYLE and DONT_REPORT_DEFAULT_ATTR_COLOR
  if (Result = '') and (Cursor.active_paragraph <> nil) and
    (Cursor.active_paragraph.ABaseStyle <> nil) then
    Result := Cursor.active_paragraph.ABaseStyle.AGetFontName;
  if Result = '' then
...
{$IFNDEF DONT_REPORT_DEFAULT_ATTR}
  if (Result = 0) and (Cursor.active_paragraph <> nil) and
    (Cursor.active_paragraph.ABaseStyle <> nil) then
    Result := Cursor.active_paragraph.ABaseStyle.AGetDef(
