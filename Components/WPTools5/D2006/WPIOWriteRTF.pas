...
{$IFNDEF CLRUNIT}unit WPIOWriteRTF;
{$ELSE}unit WPTools.IO.WriteRTF; {$ENDIF}
//******************************************************************************
...
{$IFDEF T2H}
 {:: This class is used to save text in RTF format. This class is
     used when the format name (property TextSaveFormat) is "RTF".
     You can specify the format name also in most save procedures (SaveToFile, SaveToFile and AsANSIString).
     If required you can append options to the format string
     for example "RTF-onlybody,-noimages".
 }
type TWPRTFWriter = class(TWPCustomTextWriter);
{$ELSE}
...
{$ENDIF}
  ...
  protected
    ...
    function MakeStringRTFConformW(const Str: WideString): string;
...
function TWPRTFWriter.WriteParProps(ParStyle: TWPTextStyle; UseHistory, CellProps, DoGetInherited: Boolean): Boolean;
  procedure WriteParAttr(DoGetInherited : Boolean; const rtftag: AnsiString; ID: Integer; mult: Integer; Checkmode: Boolean; defvalue: Integer = 0);
  var attrval: Integer;
    gotit: Boolean;
  begin
    attrval := 0;
...
var val, d: Integer;
  ...
  begin
    ...
    begin
      if soSaveOldPNNumbering in FStoreOptions then // Reader should ignore!
        WriteString('\pntext')
...
begin
  ...
  begin
    ...
    begin
      if not OptNoNumberProps then
      begin
      // We save PN numbering. This does not save outline info and should
      // only be used if compatibility to very old RTF reader is required
...
{$ENDIF}
  ...
  // ---------------------------------------------------------------------------
  WriteParAttr(DoGetInherited,'\li', WPAT_IndentLeft, 0, true); // check for 0!
  WriteParAttr(DoGetInherited,'\fi', WPAT_IndentFirst, 0, true); // check for 0!
  WriteParAttr(DoGetInherited,'\ri', WPAT_IndentRight, 0, true); // check for 0!
  WriteParAttr(DoGetInherited and not CellProps,'\sb', WPAT_SpaceBefore, 0, true); // check for 0!
  WriteParAttr(DoGetInherited and not CellProps,'\sa', WPAT_SpaceAfter, 0, true); // check for 0!
...
begin
  ...
  if (paprPagenrRestart in par.prop) then
...
{$ENDIF}
  ...
  begin
    WriteSectionProps(par, (paprNewPage in par.prop));
  end else
  ...
  begin
    ...
    else if par.AGet(WPAT_BANDPAR_STR, i) then
...
{$ENDIF}
    ...
    FNeedSpace := TRUE;
{$IFDEF CLR}
    FCurrParAttr[not FCurrParId] := emptyattr;
{$ELSE}
...
{$IFDEF CLR}
      FCurrCharAttr[not FCurrParId] := emptyca;
{$ELSE}
...
begin
  ...
  begin
    ...
    begin
      tbl := TableLevel;
      TableLevel := 0;
      WriteSectionProps(tablepar,(paprNewPage in tablepar.prop));
      TableLevel := tbl;
    end
    else // Word only allows to have page breaks this way
      if (paprNewPage in tablepar.prop) then
      begin
...
function TWPRTFWriter.WriteRow(rowpar: TParagraph; Style: TWPTextStyle;
  ...
  var par, refpar, aPar: TParagraph;
    x, {flags,} wid, BrdFlags, val, allval, col: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        WriteStringAndValue('\trwWidth', val div 2); // We work with /1000, RTF /200
      end else
      begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          WriteStringAndValue('\clwWidth', wid); //V5.15 val); // Twips
        end else
      ...
      // The X Offset
        ...
        if (paprRowMerge in par.prop) or
          (paprColMerge in par.prop) then
          val := par.StartSpan.AGetDefInherited(WPAT_VertAlignment, 0)
        else val := par.AGetDefInherited(WPAT_VertAlignment, 0);
  ...
  begin
    ...
    begin
      ...
      if (paprNewSection in rowpar.prop) and
        ...
        not (soNoSectionFormatInRTF in FStoreOptions) then
      begin
        WriteSectionProps(rowpar, (paprNewPage in rowpar.prop) or rowpar.HasProp(paprNewPage));
      end else
      // Word only allows to have page breaks this way
        if (paprNewPage in rowpar.prop) or rowpar.HasProp(paprNewPage) then
        begin
...
begin
  ...
  begin
    sty := WritingParStyle; //= WritingPar.ABaseStyle;
...
{$IFDEF PARSTYLE_SAVEMODE}
    ...
    begin
      sty := WritingParStyle; //= WritingPar.ABaseStyle  //V5.17.4
    end;
{$ENDIF};
...
var FontSize, j, c, submask, font_nr: Integer; fl: Single;
  sty: TWPRTFStyleElement;
  s: string;
{$IFDEF CLR}emptyca: TWPCharAttr; {$ENDIF}
begin
  if FNeedPlain then
  begin
    if not FIgnoreNextPlain then
...
{$IFDEF CLR}
    OldCharAttr := emptyca;
    // if (_rtfdataobj=nil) or (_rtfdataobj.Kind=wpIsBody) then  //V5.19.6 - commented out
    // The following line disturbs saving of text boxes
    // CurrCharAttr := emptyca; //V5.19.2   commented out: //V5.19.3a
{$ELSE}
    FillChar(OldCharAttr,SizeOf(OldCharAttr),0);
...
{$IFDEF DONT_WRITE_PARSTYLE_ATTR}if (WriteCharAttrIndex < 0) and {$ELSE}if {$ENDIF}
  ...
  begin
    if not SourceStyle.AGetInherited(WPAT_CharCharset, c) then
      c := 1; // DEFAULT_CHARSET;
...
{$ENDIF}
  ...
  begin
    ...
    if ((bits and WPSTY_LOWERCASE) <> 0) <> ((oldbits and WPSTY_LOWERCASE) <> 0)
      then WriteStringAndValue('\lower', Ord((bits and WPSTY_LOWERCASE) <> 0)); //<--- NON RTF!  WPLOWER
    if ((bits and WPSTY_BUTTON) <> 0) <> ((oldbits and WPSTY_BUTTON) <> 0)
      then WriteStringAndValue('\wpbut', Ord((bits and WPSTY_BUTTON) <> 0)); //<--- NON RTF!   afsButton Style
    ...
    if ((bits and WPSTY_USERDEFINED) <> 0) <> ((oldbits and WPSTY_USERDEFINED) <> 0)
      then WriteStringAndValue('\wpuser', Ord((bits and WPSTY_USERDEFINED) <> 0)); //<--- NON RTF!   WPUSER

  end;
  WriteCharProperty('\wphigh', WPAT_CharHighlight, 0, false); //<--- NON RTF! WPTOOLS - CUSTOM HIGLIGHT!
...
function TWPRTFWriter.WriteChar(aChar: WideChar): Boolean;
var c: AnsiChar;
begin
    ...
    begin
      ...
      begin
        ...
        begin
         ...
         {    if writecharset <>0 and
              UnicodeToMultibyte erfolgreich
               (auch DBCS!)   }
...
{$ENDIF}
 ...
 {if (soWriteBackgroundImageInRTF in FStoreOptions) then
  ...
  begin
    ...
    {$IFDEF WINCOM}WriteString('#c');{$ENDIF}
    WriteString(';}');
  end;
...
{$ENDIF}
      ...
      {	1. Now save new	wpobj format }
            ...
            begin
              ...
              ImageObj.WriteRTFData(Self, txtobj, AllowBinary);
...
function TWPRTFWriter.MakeStringRTFConformW(const Str: WideString): string;
var
  i: Integer;
begin
  Result := '';
