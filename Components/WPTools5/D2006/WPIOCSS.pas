{$IFNDEF CLRUNIT}unit WPIOCSS;
{$ELSE}unit WPTools.IO.CSS; {$ENDIF}
{ ------------------------------------------------------------------------------
  WPCubed Complete Generic CSS Parser
  ------------------------------------------------------------------------------
  written by Julian Ziersch in July and August  2003
  Copyright (C) 2003 - all rights reserved by WPCubed GmbH
...
type TWPCSSName = (
 ...
 wpcss_page_break_before, // Pagebreak: always
 wpcss_pageRotate,
...
// Each Style element
  ...
  public
    ...
    function    GetTwipValue(atomnr : Integer; const defaultvalue : Integer):Integer;
...
// Each Style:
  ...
  public
    constructor Create; {$IFDEF CLR} virtual; {$ENDIF}
    destructor Destroy; override;
  ...
  public
    constructor Create(StyleClass : TWPCSSParserStyleClass);
    destructor  Destroy; override;
    procedure   Clear;
...
('XXnameXX',
 ...
 'page-break-before', // Pagebreak: always
 'page-rotate',
...
constructor TWPCSSParser.Create(StyleClass : TWPCSSParserStyleClass);
begin
  inherited Create;
...
constructor TWPCSSParserStyle.Create;
begin
  inherited Create;
...
function TWPCSSParserStyleElement.GetTwipValue(atomnr : Integer; const defaultvalue : Integer):Integer;
begin
  if (FAtoms.Count>atomnr) and
     ...
     Result := TWPCSSParserAtom(FAtoms[0]).GetAsTwips
  else Result := defaultvalue;
...
begin
  case Typ of
       ...
       wp_urlValue: Result := 'url(' + AnsiQuotedStr(sValue,'"') + ')';
    else Result := '';
  end;
end;
...
begin
  case Typ of
       ...
       wp_pxValue: Result  := Round(fValue * 1440 / CSSPixelsPerInch);
       wp_inValue: Result  := Round(fValue * 1440);
       wp_mmValue: Result  := Round(fValue * 1440 / 25.4);
       wp_cmValue: Result  := Round(fValue * 1440 / 2.54);
       wp_PHValue: Result  := 0; //  ???
       wp_FValue:  Result  := Round(fValue * 1440);
       wp_IValue:  Result  := Round(iValue * 1440);
...
begin
  ...
  begin
    ...
    begin
      typ := wp_urlValue;
      i := Pos(')', x);
      sValue := AnsiDequotedStr(Copy(x, i+5, i - 5 - 1), '"');
    end
      // Detect Type RGB
...
wpcss_font:
          ...
          for j := 0 to element.AtomCount - 1 do
            case element.Atoms[j].Typ of
              wp_ptValue, wp_pxValue, wp_inValue, wp_mmValue, wp_cmValue :
                begin
                   par.ASet(WPAT_CharFontSize, MulDiv(element.Atoms[j].AsTwips,100,20));
                end;
              wp_strValue:
                begin
      ...
      wpcss_font_size:
      begin
          par.ASet(WPAT_CharFontSize, MulDiv( element.GetTwipValue(0, 11*20),100,20));
      end;
      wpcss_font_style:
        begin
      ...
      wpcss_line_height:
        begin
          {
          par.spacebetween :=
            element.GetTwipValue(0, par.spacebetween);  }
        end;
      ...
      wpcss_vertical_align:
        begin
          if element.IsEqualTo('top') then
            par.ASet( WPAT_Alignment, Integer(paralVertTop) )
          else if element.IsEqualTo('middle') then
            par.ASet( WPAT_Alignment, Integer(paralVertCenter) )
          else if element.IsEqualTo('bottom') then
            par.ASet( WPAT_Alignment, Integer(paralVertBottom) );
        end;
  ...
  begin
    ...
    case CSS of
            ...
            case element.Atoms[j].Typ of
              wp_ptValue, wp_pxValue, wp_inValue, wp_mmValue, wp_cmValue :
                begin
                   ...
                   a.Values[WPAT_CharFontSize] := element.Atoms[j].AsTwips *5;    // 100*pt!
                end;
              wp_strValue:
                begin
      ...
      begin
          ...
          a.Values[WPAT_CharFontSize] := element.GetTwipValue(0, 11*20) * 5; // 100*pt!
      end;
      wpcss_font_style:
        begin
          if element.IsEqualTo('normal') then
