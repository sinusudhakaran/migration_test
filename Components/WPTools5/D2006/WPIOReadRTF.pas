...
{$IFNDEF CLRUNIT}unit WPIOReadRTF;
{$ELSE}unit WPTools.IO.ReadRTF; {$ENDIF}
//******************************************************************************
...
// 17.8.2005 Last Update (better table reading, faster images)
//******************************************************************************
...
{$IFDEF USEIN_PDFCONTROL}{$DEFINE DONT_FIX_STYLES}{$DEFINE DONT_FIX_DEFSTYLE}{$ENDIF}
...
{$IFDEF WPDEBUG}, Dialogs{$ENDIF}
{$IFDEF CLRHashTable}, System.Collections{$ENDIF};

{$IFDEF T2H}
 {:: This class is used to read text which was saved in RTF format. This class is
 used when the format name (property TextLoadFormat) is "RTF". }
type TWPRTFReader = class(TWPCustomTextReader);
{$ELSE}
...
{$IFDEF READTAGOPT}Name: string; {$ENDIF}
  ...
  TWPShapeProps = record
    ...
    shpbxmethod: Integer; //0=Page, 1=Margin, 2=Column
    shpName: string;
    WPPosmode: Integer;
    wpFrameMode: Integer;
    wpExtra: Integer;
...
{$IFNDEF CLR}destructor Destroy; override; {$ENDIF}
  ...
  //  Spacing: array[0..3] of Integer; // L T R B  (trspdl ... )
    ...
    RowAlign: Integer; // 0=left, 1= ceneter, 2 = right
...
{$IFDEF FIXUP_CHARSTYLES}FHasCharStyles: Boolean; {$ENDIF}
    ...
    FInfoItemName, FCurrentMergePar, FCurrentElementName: string;
    FWrittenByGenerator: string;
...
{$ENDIF}
{$ENDIF}
{$IFDEF NEWTRHANDL}FOpt_SpectTR: Boolean; {$ENDIF} //experimental
    // Load Color Table
    ...
    class function UseForContents(const First500Char: string): Boolean; override; // AUTO ...
    property WrittenByGenerator: string read FWrittenByGenerator;
...
begin
{$IFDEF WPDEBUG}
  Assert(WPRTFTagList.IndexOf(sKeyword) < 0, 'Keyword ' + sKeyword + ' already in list!');
{$ENDIF}
  Result := TWPRTFTagParams.Create;
{$IFDEF READTAGOPT}
{$IFDEF WPDEBUG}
  if lowercase(sKeyword) <> sKeyword then
    raise Exception.Create('RTF tag ' + sKeyword + ' is not lowercase!');
{$ENDIF}
  ...
  begin
   ...
   { if FStack.Props.InNestCell and not FStack.FPrevious.Props.InNestCell then
    ...
    begin
      ...
      FFirstLoadedFirstPar := nil; //<--- WE WANT ONLY BODY!!!
...
begin
  ...
  if not FInSpecialText and FRestartPageNr then
  begin
...
{$IFNDEF NO_RTFSPLIT_TABLES}{$IFDEF NEWTRHANDL}if not FOpt_SpectTR then {$ENDIF}
      begin
...
{$ENDIF}
      ...
      begin
{$IFDEF NEWTRHANDL}if FOpt_SpectTR then par := FCurrentParagraph else {$ENDIF}
...
begin
  if FShape.shpright < FShape.shpleft then
    aLeft := FShape.shpright //< Mirrored!
  else aLeft := FShape.shpleft;

  if FShape.shpbottom < FShape.shptop then
    aTop := FShape.shpbottom //< Mirrored!
  else aTop := FShape.shptop;

  case FShape.shpbxmethod of
    1 : txtobj.RelX := aLeft + FShape.dx;    // Paragraph
    2 : txtobj.RelX := aLeft + Layout.margl; // Column
  ...
  else txtobj.Wrap := wpwrNone; // Text through object ! (Mode=3)
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FReadingStyle.ASet(WPAT_NumberLEVEL, FPNGroup.LEVEL); end;
      end else
      begin
        fontstyle := FRTFProps.NumberStyles.AddNumberStyle(numstyle,
          FPNTEXTB, FPNTEXTA, numfont,
  ...
  begin
    if FPNGroup.Reading then
      FPNTEXTA := FCurrentString.GetString
    else
      if FReadingStyle <> nil then
        FReadingStyle.ASet(WPAT_NumberTEXTA, FReadingStyle.AStringToNumber(FCurrentString.GetString))
      else if FCurrentParagraph <> nil then
        FCurrentParagraph.ASet(WPAT_NumberTEXTA, FCurrentParagraph.AStringToNumber(FCurrentString.GetString));
    FCurrentString.Clear;
  end
  else if FStack.Destination = rdsPNTextB then // also uses StringBuilder!
  begin
    if FPNGroup.Reading then
      FPNTEXTB := FCurrentString.GetString
    else
      if FReadingStyle <> nil then
        FReadingStyle.ASet(WPAT_NumberTEXTB, FReadingStyle.AStringToNumber(FCurrentString.GetString))
      else if FCurrentParagraph <> nil then
        FCurrentParagraph.ASet(WPAT_NumberTEXTB, FCurrentParagraph.AStringToNumber(FCurrentString.GetString));
    FCurrentString.Clear;
  end
  ...
  begin
    ...
    begin
      FCurrentList.FListName := FCurrentString.GetString;
      FCurrentString.Clear;
    end;
  end
  else if FStack.Destination = rdsffname then
  begin
    FCurrFieldName := FCurrentExString.GetString;
    FCurrentExString.Clear;
  end
  else if FStack.Destination = rdsffdeftext then
  begin
    FCurrFieldText := FCurrentExString.GetString;
    FCurrentExString.Clear;
  end
  else if FStack.Destination = rdsffformat then
  begin
    FCurrFieldFormat := FCurrentExString.GetString;
    FCurrentExString.Clear;
  end
  ...
  begin
    FPictureSource := FCurrentExString.GetString;
    FCurrentExString.Clear;
  end
...
{$ENDIF}
            ...
            begin
              if FIgnoreNextPict then //V5.17.3
              begin
...
class function TWPRTFReader.UseForContents(const First500Char: string): Boolean;
begin
...
{$IFDEF NEWTRHANDL}FOpt_SpectTR := (Pos('-spec_tr,', aoptstr) > 0); {$ENDIF}
...
{$IFDEF CLRHashTable}
      ...
      Result := TWPRTFTagParams(readtags[ss]);
{$ELSE CLR}
{$IFDEF READTAGOPT_HASH}
...
{$ENDIF}
  ...
  procedure AppendChar(aCh: Integer);
  var by: Byte;
...
{$ENDIF}
        if (FCurrentCodePage <> 0) and (FCurrentCodePage <> 1252) and (aCh < 256) then
          PrintAChar(Char(aCh), FCurrentCodePage)
        ...
        begin
          ...
          begin
            ...
            rdsBookmark, rdsMergePar, rdsFootnote, rdsAnySVString, rdsInfoItemName,
...
{$IFDEF READTAGOPT}
{$IFDEF CLRHashTable}
  readtags := HashTable.Create;
  for i := 0 to WPRTFTagList.Count - 1 do
  begin
    str := TWPRTFTagParams(WPRTFTagList.Objects[i]).Name;
    readtags.Add(str, TWPRTFTagParams(WPRTFTagList.Objects[i]));
    Assert(readtags.Contains(str), 'HASHTABLE ERROR');
  end;
{$ELSE}
...
{$ENDIF}
  ...
  FUNICharCount := 1;
  FNeedStackAssign := FALSE;
  FCurrentCodePage := WPGetCodePage(0); // Default!
  FIsNewSection := FALSE;
  FRestartPageNr := FALSE;
  FInSpecialText := FALSE;
  FWrittenByGenerator := '';
...
{$IFDEF WPDEBUG}
      ...
      begin
      //   ShowMessage('Too few "}" in RTF code');
      end;
{$ENDIF}
...
{$IFNDEF DONT_FIX_DEFSTYLE}
              ...
              begin
                ...
                begin
                  ...
                  if parsty.AGet(WPAT_CharCharset, n) then defstylesheet.ADel(WPAT_CharCharset);
...
{$ENDIF}
      ...
      if FHasReportGroups then CreateGroupTree;
    finally
...
{$ENDIF}
              ...
              Attr.SetFontCharSet(FFontTbl.FontCharSet[FDefaultFontNr]);
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  FReadingStyle.ASet(WPAT_CharCharset, FFontTbl.FontCharSet[Value]); //V5.17.3
                end
                else
                begin
                  Attr.SetFont(FFontTbl.Fontnr[Value]);
                  Attr.SetFontCharSet(FFontTbl.FontCharSet[Value]);
...
{$ENDIF}
          ...
          //  CreateImplizitPar;
                ...
                begin
                  TableDepth := 0;
                  Assert(TableVar[TableDepth].FLastCellInRow <> nil, 'Where is the cell in level 0');
...
{$ENDIF}
                ...
                if not (paprIsTable in FCurrentParagraph.prop) then ParLevelUp;
      ...
      // -------------------------------------------------------------------------
          ...
          // trowd sometimes comes BEFORE itap - ResetValues on Demand
                FStack.FInTROWD := TRUE; //<--- V5.11.2
                FStack.Props.InTable := TRUE; //<--- V5.11.2
                FStack.ASetBaseStyle(0);
            ...
            ipropTableCellDefaults:
              begin
{$IFDEF WPDEBUG}FRTFDataCollection.StatusMsg(WPST_ReadInfo, 'CellProps'); {$ENDIF}
              end;
...
{$ENDIF}
            ...
            ipropTableCellX:
              ...
              begin
                ...
                inc(tstack.FRowTbl.AllWidth, Value - tstack.FRowTbl.CellLast);
  ...
  // Borders ... trbrdrt
    ...
    // reserved
      ...
      // ---------------------------------------------------------------- Shapes
            ...
            ipropshpbypage: FShape.shpbypage := TRUE;
            ...
            ipropshpbxcolumn:
               if (WrittenByWPToolsVersion<5) or (WrittenByWPToolsVersion>5.2) then
                   FShape.shpbxmethod := Value; //V5.20.1
      // -------------------------------------------------------- Page Numbering
            ipropPgnX: { not used }
              begin
              end;
...
{$ENDIF}
  ...
  // ---------------------------------------------------------------- PAGE FRAME
            ipropPageFrame:
              begin
                if (FCurrentSectionProps <> nil) and not (loIgnorePageSize in FLoadOptions) then
                begin
...
{$ENDIF}
            ...
            ipropNewSection:
              begin
                ...
                if not FStack.Props.InTable then //---> must go into table row!  //V5.17.5
                begin
              {  NewParagraph;
                include(FCurrentParagraph.prop, paprNewPage);
                FIsNewSection := FALSE; //V5.17.5
                }
                end;
                FCurrentSectionProps :=
                  RTFDataCollection.GetSectionProps(FNewSectionID);
...
{$IFDEF WPDEBUG}
    on e: Exception do
    begin
      FRTFDataCollection.StatusMsg(WPST_ReadInfo, e.Message
        + ' at ' + IntToStr(ReadPosition));
      raise;
    end;
{$ENDIF}
  end;
...
{$IFDEF CLR}emptygrp: TWPCurrentPNGroup; {$ENDIF}
begin
...
{$ENDIF}
      ...
      idestFldResult:
        begin
          FWaitForInsertPoint := FALSE; //<-- V4 nevers used FldResult
          if FStack.Destination = rdsField then
          begin
      ...
      idestInfoItem:
        begin
            // 'Title', 'Subject', 'Author', 'Keywords', 'Operator', 'DocComm', 'Generator'
            // '\comment is automatically converted to "DocComm" !
        ...
        begin
          ...
          begin
            if idestCode = idestHeader then
              rtfdataobj := FRTFDataCollection.Append(wpIsHeader,
                TWPPagePropertyRange(Value), FCurrentElementName)
            else rtfdataobj := FRTFDataCollection.Append(wpIsFooter,
                TWPPagePropertyRange(Value), FCurrentElementName);
...
{$ENDIF}
      ...
      idestPNTextA:
        begin
          if loIgnoreNumberDefs in LoadOptions then
            FStack.Destination := rdsSkip
          else
          begin
...
begin
  ...
  begin
    ...
    // trrh:
    if not OptIgnoreRowHeight then
    begin
  ...
  i := 0;
{$IFDEF WPDEBUG}
  if tstack.FRowTbl.CellCount = 0 then
    FRTFDataCollection.StatusMsg(WPST_ReadInfo, 'Cellcount = 0 in TableDepth ' + IntToStr(TableDepth));
{$ENDIF}
  ...
  begin
{$IFDEF NEWTRHANDL}
    if not FOpt_SpectTR then par := tstack.FTableParent else begin
      par := tstack.FTableRow;
      if tstack.FRowTbl.BoxWidth > 0 then
        par.ASet(WPAT_BoxWidth, tstack.FRowTbl.BoxWidth)
      else if tstack.FRowTbl.BoxWidth_PC > 0 then
        par.ASet(WPAT_BoxWidth_PC, tstack.FRowTbl.BoxWidth_PC * 2)
      else if tstack.FRowTbl.AllWidth <> 0 then
        par.ASet(WPAT_BoxWidth, tstack.FRowTbl.AllWidth); end;
{$ELSE}
    par := tstack.FTableParent; {$ENDIF}
    if par <> nil then par.ASet(WPAT_BoxMarginLeft, tstack.FRowTbl.RowLeft);
  end;

  if (tstack.FTableParent <> nil) and (row<>nil) and (row.PrevPar = nil) then
  begin
...
begin
  ...
  begin
    if not (loIgnoreFonts in FLoadOptions) and not OptIgnoreFonts then
    begin
...
{$ENDIF}
      // -------------------------------------------------------------
        ...
        if (FFontTbl.aktnr = FDefaultFontNrDEFF) and not
          OptIgnoreFonts then //V5.17.4
        begin
        ...
        begin
          ...
          if (FFontTbl.aktCharset = 0) and (
            (CompareText(FFontTbl.aktFont, 'Symbol') = 0) or
            (CompareText(Copy(FFontTbl.aktFont, 1, 9), 'Wingdings') = 0) or // ..1, ..2, ...
            (CompareText(FFontTbl.aktFont, 'Webdings') = 0)) then
...
// AddRgSymRTF('bin', 0, FALSE, kwdSpec, ipfnBin);
  ...
  AddRgSymRTF('shpbypara', 0, false, kwdProp, ipropshpbypara);
...
{$ENDIF}
 ...
 // Shading
  ...
  AddRgSymRTF('pgbrdrhead', 1, FALSE, kwdProp, ipropPageFrame);
  AddRgSymRTF('pgbrdrfoot', 1, FALSE, kwdProp, ipropPageFrame);
  AddRgSymRTF('pgbrdrt', 1, FALSE, kwdProp, ipropPageFrame);
  AddRgSymRTF('pgbrdrb', 1, FALSE, kwdProp, ipropPageFrame);
  AddRgSymRTF('pgbrdrl', 1, FALSE, kwdProp, ipropPageFrame);
  AddRgSymRTF('pgbrdrr', 1, FALSE, kwdProp, ipropPageFrame);
