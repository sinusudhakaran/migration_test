// ***************************************************************
//  madCrypt.pas              version:  1.2a  ·  date: 2009-02-09
//  -------------------------------------------------------------
//  blowfish encryption
//  -------------------------------------------------------------
...
// 2009-02-09 1.2a Delphi 2009 support
// 2003-11-10 1.2  Encode/Decode (base64) functions added
// 2003-08-06 1.1  (1) this algorythm is now 100% blowfish compatible
//                 (2) automatic string padding added
//                 (3) added "UniqueString" to string functions
// 2002-10-04 1.0  initial release
...
procedure Encrypt (buf: pointer; len: dword; const password: AnsiString; iv: int64 = 0); overload;
procedure Decrypt (buf: pointer; len: dword; const password: AnsiString; iv: int64 = 0); overload;

// encrypt/decrypt a string
procedure Encrypt (var data: AnsiString; const password: AnsiString; iv: int64 = 0); overload;
procedure Decrypt (var data: AnsiString; const password: AnsiString; iv: int64 = 0); overload;

// ***************************************************************

// this is the old version, which worked but was not 100% blowfish compatible
procedure OldEncrypt (buf: pointer; len: dword; password: AnsiString); overload;
procedure OldDecrypt (buf: pointer; len: dword; password: AnsiString); overload;
procedure OldEncrypt (var data: AnsiString; password: AnsiString); overload;
procedure OldDecrypt (var data: AnsiString; password: AnsiString); overload;
...
function Encode (data: AnsiString) : AnsiString; overload;
...
procedure cryptBuf(buf: pointer; len: dword; encrypt: boolean; const password: AnsiString;
...
var SArr : TSArr;
  ...
  procedure SetPassword(password: PAnsiChar; len: dword);
...
begin
  PArr := CPArr;
  SArr := CSArr;
  SetPassword(PAnsiChar(password), Length(password));
  pc1 := buf;
...
procedure Encrypt(buf: pointer; len: dword; const password: AnsiString; iv: int64 = 0);
begin
  if len > 0 then
    cryptBuf(buf, len, true, password, iv);
end;

procedure Decrypt(buf: pointer; len: dword; const password: AnsiString; iv: int64 = 0);
begin
  if len > 0 then
    cryptBuf(buf, len, iv <> 0, password, iv);
end;

procedure Encrypt(var data: AnsiString; const password: AnsiString; iv: int64 = 0);
var i1, i2 : integer;
begin
  if data <> '' then begin
    UniqueString(data);
...
procedure Decrypt(var data: AnsiString; const password: AnsiString; iv: int64 = 0);
var i1, i2 : integer;
    b1     : boolean;
begin
  i1 := Length(data);
...
procedure OldEncrypt(buf: pointer; len: dword; password: AnsiString);
begin
  cryptBuf(buf, len, true, password, 0, true);
end;

procedure OldDecrypt(buf: pointer; len: dword; password: AnsiString);
begin
  cryptBuf(buf, len, false, password, 0, true);
end;

procedure OldEncrypt(var data: AnsiString; password: AnsiString);
begin
  UniqueString(data);
  cryptBuf(pointer(data), Length(data), true, password, 0, true);
end;

procedure OldDecrypt(var data: AnsiString; password: AnsiString);
begin
  UniqueString(data);
...
function Encode(data: AnsiString) : AnsiString; overload;
begin
  result := Encode(PAnsiChar(data), Length(data));
end;
