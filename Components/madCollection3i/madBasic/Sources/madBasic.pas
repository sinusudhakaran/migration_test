// ***************************************************************
//  madBasic.pas              version:  1.6g  ·  date: 2009-02-09
//  -------------------------------------------------------------
...
type
  ...
  IBasic = interface ['{53F8CE42-2C8A-11D3-A52D-00005A180D69}']
    ...
    procedure SetLastErrorStr (str : AnsiString);
    procedure SetLastError    (no  : cardinal;
                               str : AnsiString = '');
    property  LastErrorNo     : cardinal   read GetLastErrorNo  write SetLastErrorNo;
    property  LastErrorStr    : AnsiString read GetLastErrorStr write SetLastErrorStr;

    // multi purpose string
    function  GetStrBuf : AnsiString;
    procedure SetStrBuf (buf: AnsiString);
    property  StrBuf    : AnsiString read GetStrBuf write SetStrBuf;
...
type
  ...
  public
    ...
    constructor Create (valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString);
...
constructor TIBasic.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString);
begin
  inherited Create;
  FValid := valid;
  FSuccess := FValid;
...
function TIBasic.GetLastErrorStr : AnsiString;
begin
...
procedure TIBasic.SetLastErrorStr(str: AnsiString);
begin
  FLastErrorStr := str;
...
procedure TIBasic.SetLastError(no: cardinal; str: AnsiString = '');
begin
  FLastErrorNo  := no;
  FLastErrorStr := str;
  FSuccess := FLastErrorNo = 0;
...
procedure TIBasic.SetStrBuf(buf: AnsiString);
begin
  FStrBuf := buf;
end;
...
constructor TIList.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString);
begin
...
constructor TIBasicListSelection.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                                        const parentList: ICustomBasicList; copyParentList: boolean);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
