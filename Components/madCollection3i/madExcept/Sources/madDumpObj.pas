// ***************************************************************
//  madDumpObj.pas            version:  1.0a  ·  date: 2009-02-09
//  -------------------------------------------------------------
...
function DumpObj(obj: TObject) : string;
  ...
  function DumpObject_(obj: TObject; indent: string) : string;
    ...
    const CTypeKindNames : array [TTypeKind] of PAnsiChar =
      ('Unknown', 'Integer', 'Char', 'Enum', 'Float',
       'ShortString', 'Set', 'Class', 'Method', 'WChar', 'String', 'WideString',
       'Variant', 'Array', 'Record', 'Interface', 'Int64', 'DynArray' {$ifdef UNICODE}, 'UnicodeString' {$endif});
    const CMethodKindNames : array [TMethodKind] of PAnsiChar =
    ...
    begin
      ...
      if tk = tkClass then begin
        ...
        if tk = tkMethod then begin
          td := GetTypeData(propInfo^.PropType^);
          if td <> nil then begin
            result := result + string(CMethodKindNames[td^.MethodKind]);
            buf := @td^.ParamList;
            result := result + ' (';
          ...
          result := result + string(CTypeKindNames[tk]);
          case tk of
