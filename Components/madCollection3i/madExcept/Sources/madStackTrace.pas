// ***************************************************************
//  madStackTrace.pas         version:  2.1i  ·  date: 2009-02-09
//  -------------------------------------------------------------
//  stack tracing
//  -------------------------------------------------------------
...
// 2009-02-09 2.1i Delphi 2009 support
...
{$ifdef meLog}
  var meLog_         : procedure (str: AnsiString) = nil;
      meIndentLog_   : procedure = nil;
      meUnindentLog_ : procedure = nil;
{$endif}
...
type
  TStackItem = record
                 ...
                 FunctionName : AnsiString;  // function/method name
               end;
...
function StackTrace (hideUglyItems     : boolean        = false;
                     ...
                     pMinDebugInfos    : TPDAAnsiString = nil;
                     dontAddTopItem    : boolean        = false) : AnsiString;
...
function FastMM_LogStackTrace(preparedStack: pointer; stackTraceDepth: integer; buf: PAnsiChar;
                              hideUglyItems, showRelativeAddrs, showRelativeLines, dumbTrace: boolean) : PAnsiChar;
function StackAddrToStr (addr: pointer; relAddr: boolean = false; relLine: boolean = false) : AnsiString;
function InternalError (func: AnsiString; relAddr, relLine: boolean) : AnsiString;
...
{$ifdef meLog}
  procedure meLog(str: AnsiString);
  begin
    if @meLog_ <> nil then
      meLog_(str);
  end;
  procedure IndentLog;
  begin
    if @meIndentLog_ <> nil then
      meIndentLog_;
  end;
  procedure UnindentLog;
  begin
    if @meUnindentLog_ <> nil then
      meUnindentLog_;
  end;
{$endif}
...
var
  ...
  functionCache : array of record
    ...
    name     : AnsiString;     // function name, if we can find out
...
function AddFunction(code: pointer; name: AnsiString; module: integer;
...
begin
  result := false;
  index  := -1;
  {$ifdef meLog}meLog('AddModule adding ' + IntToHexEx(module));{$endif}
  ...
  if GetModuleFileNameA(module, arrCh, MAX_PATH) <> 0 then begin
    nh := GetImageNtHeaders(module);
    if nh <> nil then
      with nh^.OptionalHeader do begin
        // the module has a valid PE image header, so add to our cache
        result := true;
        {$ifdef meLog}meLog('AddModule adding ' + IntToHexEx(module) + ' -> "' + arrCh + '"');{$endif}
...
(*          if (i1 <> -1) and
  ...
  {$ifdef meLog}
    if result then
         meLog('AddModule added ' + IntToHexEx(module) + ' -> ' + IntToStrEx(index))
    else meLog('AddModule adding ' + IntToHexEx(module) + ' failed.');
  {$endif}
end;
...
// if the parsing was successful, also try to find a name for the function
  ...
  function GetFunctionName(module: integer; code: pointer) : AnsiString;
...
begin
  {$ifdef meLog}
    result := -1;
    meLog('AddFunction adding ' + IntToHexEx(dword(code)) + ', "' + name + '", module ' + IntToStrEx(module));
    IndentLog;
    try
  {$endif}
  Finalize(fi);
  ...
  if (code <> nil) or force then begin
    ...
    {$ifdef meLog}meLog('AddFunction parsing ' + IntToHexEx(dword(code)));{$endif}
    ...
    if fi.IsValid then begin
      ...
      {$ifdef meLog}meLog('AddFunction adding ' + IntToHexEx(dword(code)) + ' -> ' + IntToStrEx(result));{$endif}
      inc(functionCount);
      ...
      for i1 := 0 to high(fi.FarCalls) do
        if not fi.FarCalls[i1].Call then begin
          {$ifdef meLog}meLog('AddFunction check sub jump ' + IntToHexEx(dword(fi.FarCalls[i1].Target))); IndentLog;{$endif}
          AddFunction(fi.FarCalls[i1].Target, '', -1, approved, false, tryRead_);
          {$ifdef meLog}UnindentLog;{$endif}
        end;
...
{          i2 := Length(pfi^.FarCalls);
  ...
  {$ifdef meLog}
      ...
      UnindentLog;
      if result <> -1 then
           meLog('AddFunction added ' + IntToHexEx(dword(code)) + ' -> ' + IntToStrEx(result))
      else meLog('AddFunction adding ' + IntToHexEx(dword(code)) + ' failed.');
    end;
  {$endif}
end;
          ...
          if beforeInstr <> nil then
            ...
            for i2 := 0 to high(functionCache[func].fi.FarCalls) do
              if functionCache[func].fi.FarCalls[i2].CodeAddr2 = beforeInstr then begin
                ...
                if (i3 <> -1) and
                   ( (PosText(AnsiString('@RaiseExcept'), functionCache[i3].Name) > 0) or
                     (PosText(AnsiString('Error'),        functionCache[i3].Name) > 0)    ) then begin
                  // bingo
                  result := true;
...
type
  ...
  TInternalStackItem = record
    ...
    FinalOutput  : record                     // for creating the final string output
                     ...
                     UnitName   : AnsiString;
                     Line       : integer;    // line number (0 = unknown)
                     PublicName : AnsiString; // function name
...
var stackArr : array of TInternalStackItem;  // this dynamic array holds all possible stack items
  ...
  // collect all important information and store it into the "FinalOutput" record
    ...
    procedure AddMinDebugInfoItem(moduleName: AnsiString; mf: TMapFile);
    var s1 : AnsiString;
        i1 : integer;
    begin
  ...
  var s1, s2, s3, s4, s5, s6 : AnsiString;
    ...
    for i1 := high(stackArr) downto 0 do begin
      ...
      with stackArr[i1], FinalOutput do
        if (not invalid) and
          ...
          s1 := ModuleName;
          Move(PAnsiChar(s1)^, PAnsiChar(s3)^, Length(s1));
          for i3 := Length(s1) + 1 to Length(s3) do
            s3[i3] := ' ';
          Move(PAnsiChar(UnitName)^, PAnsiChar(s4)^, Length(UnitName));
          for i3 := Length(UnitName) + 1 to Length(s4) do
            s4[i3] := ' ';
          if PublicLine > 0 then begin
            s1 := IntToStrEx(PublicLine);
            Move(PAnsiChar(s1)^, s5[i5 - Length(s1)], Length(s1));
          end else
            s1 := '';
          ...
          if (PublicLine > 0) or (Line > 0) then begin
            ...
            if showRelativeLines and (PublicLine > 0) then
              s1 := '+' + s1;
            Move(PAnsiChar(s1)^, s6[i6 - Length(s1)], Length(s1));
            b1 := false;
          end else
            s1 := '';
  ...
  function InternalError2(func: AnsiString) : AnsiString;
  begin
...
begin
  ...
  if not IsBadReadPtr(pointer(stackBottom), stackTop - stackBottom) then begin
    ...
    try
      ...
      {$ifdef meLog}meLog('StackTrace CollectPossibleStackItems'); IndentLog;{$endif}
      try
      ...
      // remove recursive stack areas
      {$ifdef meLog}UnindentLog; meLog('StackTrace RemoveRecursiveStackAreas'); IndentLog;{$endif}
      try
      ...
      {$ifdef meLog}UnindentLog; meLog('StackTrace InDeepChecks'); IndentLog;{$endif}
      try
      ...
      {$ifdef meLog}UnindentLog; meLog('StackTrace RealizeOptimalLinkChains'); IndentLog;{$endif}
        try
          i2 := high(stackArr);
      ...
      {$ifdef meLog}UnindentLog; meLog('StackTrace RemoveRecursiveAreas'); IndentLog;{$endif}
      try
      ...
      {$ifdef meLog}UnindentLog; meLog('StackTrace FinalFinalOutput'); IndentLog;{$endif}
      try
        FillFinalOutput;
      except
      ...
      if (pAbort <> nil) and pAbort^ then exit;
      // take all the collected informations and put them together nicely
      {$ifdef meLog}UnindentLog; meLog('StackTrace ComposeResult'); IndentLog;{$endif}
      try
        ComposeResult;
      except
