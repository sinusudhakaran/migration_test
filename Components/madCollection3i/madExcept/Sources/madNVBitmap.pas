// ***************************************************************
//  madNVBitmap.pas           version:  1.0d  ·  date: 2009-02-09
//  -------------------------------------------------------------
...
// 2009-02-09 1.0d Delphi 2009 support
// 2006-09-11 1.0c (1) grayscale pngs were sometimes distorted
...
type
  ...
  INVBitmap = interface ['{3EE757B5-CC56-4610-A917-E8731737D5BE}']
    ...
    function AsBmpStr                                    : AnsiString;
    function AsPngStr (format: TPngFormat = pf256Colors) : AnsiString;

    function SaveBmp (bmpFile: AnsiString                                  ) : boolean;
    function SavePng (pngFile: AnsiString; format: TPngFormat = pf256Colors) : boolean;

    function Zoom (width, height: integer) : INVBitmap;
  end;

// load a bitmap file
function LoadBitmap (bmpFile: AnsiString) : INVBitmap;
...
function TINVBitmap.AsBmpStr : AnsiString;
...
function TINVBitmap.AsPngStr(format: TPngFormat = pf256Colors) : AnsiString;
type TPalette = array [0..255] of packed record r, g, b : byte end;
//     TBigPal  = array [0..255] of packed record r, g, b, a : byte end;

  function CreatePng(width, height: dword; palette, bitmap: AnsiString) : AnsiString;
    ...
    function CompressBitmap : AnsiString;
    ...
    procedure AddPngPart(var png: AnsiString; name, data: AnsiString);
    var pdw : ^dword;
        crc : dword;
    begin
  ...
  procedure FindOptimalPalette(bmp: PAnsiChar; pixels: integer; var palette: TPalette; noOfCols: integer);
  ...
  function GetGrayPal(const values: array of byte) : AnsiString;
  var i1, i2 : integer;
  begin
...
function TINVBitmap.SaveBmp(bmpFile: AnsiString) : boolean;
var fh : dword;
    c1 : dword;
    s1 : AnsiString;
begin
  fh := CreateFileA(PAnsiChar(bmpFile), GENERIC_WRITE, 0, nil, CREATE_ALWAYS, 0, 0);
  if fh <> INVALID_HANDLE_VALUE then begin
    s1 := AsBmpStr;
    result := WriteFile(fh, pointer(s1)^, length(s1), c1, nil) and (c1 = dword(length(s1)));
    CloseHandle(fh);
    if not result then
      DeleteFileA(PAnsiChar(bmpFile));
  end else
    result := false;
end;

function TINVBitmap.SavePng(pngFile: AnsiString; format: TPngFormat) : boolean;
var s1     : AnsiString;
    fh, c1 : dword;
begin
  result := false;
...
function DcToBmp(sdc: dword; ix, iy, iw, ih: integer; drawCursor, thisAppOnly: boolean) : INVBitmap;
  ...
  begin
    ...
    if (pid = GetCurrentProcessId) and IsWindowVisible(window) and (not IsIconic(window)) then begin
      ...
      if (r1.Left < r1.Right) or (r1.Top < r1.Bottom) then begin
  ...
  function CalculateRegion(window: dword; var snr: TSnapshotRec) : bool; stdcall;
    ...
    function WindowToRgn(window: dword) : dword;
    var r1 : TRect;
    begin
      result := 0;
      if IsWindowVisible(window) and (not IsIconic(window)) then begin
        GetWindowRect(window, r1);
        if (r1.Left < r1.Right) or (r1.Top < r1.Bottom) then begin
          OffsetRect(r1, -snr.ix, -snr.iy);
          result := CreateRectRgnIndirect(r1);
        end;
      end;
    end;
  ...
  begin
    ...
    if pid = GetCurrentProcessId then begin
      c1 := WindowToRgn(window);
      if c1 <> 0 then begin
        while window <> 0 do begin
          window := GetNextWindow(window, GW_HWNDPREV);
          c2 := WindowToRgn(window);
          if c2 <> 0 then begin
            CombineRgn(c1, c1, c2, RGN_DIFF);
            DeleteObject(c2);
          end;
        end;
...
begin
  ...
  if thisAppOnly then begin
    ...
    if snr.rect.Left < maxInt then begin                
      ...
      if GetRgnBox(snr.region, r1) = NULLREGION then begin
...
begin
  result := nil;
  sbmp := LoadImageA(0, PAnsiChar(bmpFile), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE or LR_CREATEDIBSECTION);
