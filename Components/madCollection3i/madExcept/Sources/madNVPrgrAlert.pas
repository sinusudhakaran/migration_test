// ***************************************************************
//  madNVPrgrAlert.pas        version:  1.0a  ·  date: 2009-02-09
//  -------------------------------------------------------------
...
type
  IProgressAlert = interface ['{45B8448C-47B8-4F1C-886E-95ED5FD41E5F}']
    ...
    procedure AddArea (weight: int64; text: AnsiString = '');
...
function NewProgressAlert (title     : AnsiString;
                           bmpModule : dword  = 0;
                           bmpName   : AnsiString = '') : IProgressAlert;
...
type
  TIProgressAlert = class (TInterfacedObject, IProgressAlert)
    ...
    constructor Create (title: AnsiString; bmpModule: dword; bmpName: AnsiString);
...
constructor TIProgressAlert.Create(title: AnsiString; bmpModule: dword; bmpName: AnsiString);
begin
  inherited Create;
  FTitle     := title;
...
function TIProgressAlert.GetTitle       : AnsiString; begin result := FTitle       end;
...
procedure TIProgressAlert.SetTitle(value: AnsiString);
begin
  if FTitle <> value then begin
    FTitle := value;
    SetWindowTextA(FMainWnd, PAnsiChar(FTitle));
  end;
end;

procedure TIProgressAlert.AddArea(weight: int64; text: AnsiString = '');
var i1 : integer;
begin
  i1 := length(FAreas);
...
begin
  if (value <> FCurrentArea) and (value >= 0) and (value <= Length(FAreas)) then begin
    ...
    if (FCurrentArea < length(FAreas)) and (FLabel <> 0) then
      SetWindowTextA(FLabel, PAnsiChar(FAreas[FCurrentArea].text));
  end;
end;
...
begin
  ...
  if FBar <> 0 then
    if GetWindowThreadProcessId(FBar, nil) = GetCurrentThreadId then begin
      SendMessageA(FBar, PBM_SETPOS, value, 0);
      ProcessMessages;
    end else
      PostMessageA(FBar, PBM_SETPOS, value, 0);
end;

function TIProgressAlert.Show(parentWindow: dword = 0; activate: boolean = true) : boolean;

  function DrawGraphic(module: dword; name: AnsiString; hDC, size: dword) : boolean;
...
begin
  ...
  if result then begin
    ...
    FMainWnd := CreateWindowExA(WS_EX_DLGMODALFRAME or WS_EX_TOPMOST or WS_EX_TOOLWINDOW,
                                wndClass.lpszClassname, PAnsiChar(FTitle),
    ...
    for i1 := FCurrentArea to high(FAreas) do begin
      ...
      DrawTextA(FTmpDC, PAnsiChar(FAreas[i1].text), -1, r2, DT_CALCRECT or DT_WORDBREAK);
    ...
    if FindResourceA(FBmpModule, PAnsiChar(FBmpName), PAnsiChar(RT_BITMAP)) <> 0 then begin
      inc(r1.Left,  32 + 8);
      inc(r1.Right, 32 + 8);
      FBmpWnd := CreateWindow('Button', '', WS_VISIBLE or WS_CHILD or BS_OWNERDRAW or WS_DISABLED,
                              9, 9, 32, 32, FMainWnd, 0, HInstance, nil);
      SetWindowLongA(FBmpWnd, GWL_USERDATA, integer(self));
    end else
      FBmpWnd := 0;
    ...
    iw := r1.Right + 22;
    ih := 12 + r1.Bottom + 12;
    FLabel := CreateWindowA('Static', PAnsiChar(FAreas[FCurrentArea].text), WS_VISIBLE or WS_CHILD or SS_LEFT,
                                      r1.Left, r1.Top, r1.Right, r1.Bottom, FMainWnd, 0, HInstance, nil);
    SendMessageA(FLabel, WM_SETFONT, integer(FFont), 0);
    ...
    SendMessageA(FBar, PBM_SETRANGE, 0, 1000 shl 16);
...
begin
  if FMainWnd <> 0 then begin
    if FBmpWnd <> 0 then
      SetWindowLongA(FBmpWnd, GWL_USERDATA, 0);
    if GetWindowThreadProcessId(FMainWnd, nil) <> GetCurrentThreadId then begin
      SetWindowLongA(FMainWnd, GWL_USERDATA, 1);
      SendMessageA(FMainWnd, WM_SYSCOMMAND, SC_CLOSE, 0);
    end else
...
function NewProgressAlert(title: AnsiString; bmpModule: dword = 0; bmpName: AnsiString = '') : IProgressAlert;
begin
