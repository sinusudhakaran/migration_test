// ***************************************************************
//  madExceptWizard.pas       version:  3.0f  ·  date: 2009-02-09
//  -------------------------------------------------------------
//  IDE integration
//  -------------------------------------------------------------
...
// 2009-02-09 3.0f Delphi 2009 support
// 2008-01-14 3.0e (1) support for BCB2007 added
//                 (2) double click in callstack on "db.pas" didn't work
//                 (3) multiple plugins in one mep file was problematic
// 2007-03-28 3.0d support for D2007 ".dproj" added
...
type
  ...
  TDfmValue = record
    ...
    vBool : boolean;
    vStr  : AnsiString;
    list  : AnsiString;  // list of options for dtEnum
    prep  : AnsiString;  // prepend string for dtEnum
  end;
  TDfmObject = record
    name  : AnsiString;
    typ   : AnsiString;
  ...
  private
    ...
    FAssis            : TDAAnsiString;
    FSelProp          : AnsiString;
    FActiveProp       : array [0..2] of integer;
    FPropList         : TListBox;
    FCurIcon          : TPaintBox;
    FIcons            : array [0..10] of AnsiString;
    FHook             : dword;
    FIconPanel        : TPanel;
    FHookWndProc      : pointer;
    FSmtpServer       : AnsiString;
    FSmtpPort         : dword;
    FSmtpAccount      : AnsiString;
    FSmtpPassword     : AnsiString;
    FHttpServer       : AnsiString;
    FHttpPort         : dword;
    FHttpAccount      : AnsiString;
    FHttpPassword     : AnsiString;
    FBugReportSendAs  : AnsiString;
    FBugReportZip     : AnsiString;
    FScreenShotSendAs : AnsiString;
    FScreenShotZip    : AnsiString;
    FPropCol          : integer;
    ...
    function  EditAssistant(var assis: AnsiString) : boolean;
  ...
  public
    ...
    procedure FillAssisCombo (combo: TComboBox; assis: AnsiString = '*'; renameFrom: AnsiString = ''; renameTo: AnsiString = '');
    procedure FillAssisCombos (renameFrom: AnsiString = ''; renameTo: AnsiString = '');
...
begin
  GetModuleFileNameA(0, arrCh, MAX_PATH);
  BCB := PosText(AnsiString('bcb.exe'), arrCh) > 0;
end;
...
begin
  ...
  for i1 := 0 to high(Plugins) do
    PluginList.Items.Add(string(Plugins[i1].description));
end;
...
procedure TFMadExceptProjectSettings.ImgsPaint(Sender: TObject);
  ...
  function DrawGraphic(bmp: AnsiString; size, dc, backCol: dword; enabled: boolean) : boolean;
  ...
  begin
    ...
    if Length(bmp) < 200 then begin
      bmp := UpStr(bmp);
      c1 := FindResourceA(HInstance, PAnsiChar(bmp), pointer(RT_BITMAP));
...
procedure FillProps(var dfmObj: TDfmObject; defaultButton: boolean);
  ...
  procedure AddProp(var dfmObj: TDfmObject; name: AnsiString; typ: TDfmTyp;
                    vInt: integer; vBool: boolean; vStr, list: AnsiString; prep: AnsiString = '');
  ...
  begin
    ...
    for i1 := 0 to high(dfmObj.props) - 1 do
      if madStrings.CompareText(name, AnsiString(dfmObj.props[i1].name)) <= 0 then begin
        i3 := i1;
        break;
      end;
...
function DfmToStruct(dfm: AnsiString) : TDfm;
var index : integer;
  ...
  procedure ConvertObject;
    ...
    var i1, i2 : integer;
        s1     : AnsiString;
    begin
      SetString(s1, PAnsiChar(dfm) + index, ord(dfm[index]));
      ...
      case dfm[index - 1] of
        ...
        #$c      : begin
                     if i2 <> -1 then begin
                       ...
                       SetString(dfmObj.props[i1].vStr, PAnsiChar(dfm) + index + 3, TPInteger(@dfm[index])^);
                     end;
        ...
        #$7      : begin
                     if i2 <> -1 then begin
                       ...
                       SetString(dfmObj.props[i1].vStr, PAnsiChar(dfm) + index, ord(dfm[index]));
                     end;
  ...
  begin
    ...
    SetString(result[i1].typ,  PAnsiChar(dfm) + index, ord(dfm[index]));
    inc(index, ord(dfm[index]) + 1);
    SetString(result[i1].name,  PAnsiChar(dfm) + index, ord(dfm[index]));
...
begin
  result := nil;
  index := 5;
  if PosStrIs1(AnsiString('TPF0'), dfm) then
    ConvertObject;
end;

function StructToDfm(const dfm: TDfm) : AnsiString;
var i1, i2 : integer;
    s1     : AnsiString;
begin
  result := 'TPF0';
  for i1 := 0 to high(dfm) do begin
    ...
    result := result + AnsiChar(Length(s1          )) + s1 +
                       AnsiChar(Length(dfm[i1].name)) + dfm[i1].name;
    for i2 := 0 to high(dfm[i1].props) do
      with dfm[i1].props[i2] do
        if (name <> 'Name') and ((typ <> dtEnum) or (vStr <> '')) then begin
          result := result + AnsiChar(Length(name)) + name;
          case typ of
            dtInt  : begin
            ...
            dtEnum : result := result + #$7 + AnsiChar(Length(vStr)) + vStr;
          end;
        end;
    result := result + #0;
    if i1 > 0 then
...
procedure TFMadExceptProjectSettings.FillAssisCombo(combo: TComboBox; assis: AnsiString = '*'; renameFrom: AnsiString = ''; renameTo: AnsiString = '');
var i1 : integer;
begin
  if (assis = '*') and (combo.ItemIndex > 0) then begin
    assis := AnsiString(combo.Items[combo.ItemIndex]);
  ...
  for i1 := 0 to high(FAssis) do begin
    combo.Items.Add(string(SubStr(FAssis[i1], 1)));
    if IsTextEqual(SubStr(FAssis[i1], 1), assis) then
      combo.ItemIndex := i1 + 1;
  end;
end;

procedure TFMadExceptProjectSettings.FillAssisCombos(renameFrom: AnsiString = ''; renameTo: AnsiString = '');
var b2 : boolean;
begin
...
begin
  with settings do begin
    ...
    for i1 := 0 to high(FAssis) do begin
      ...
      with AssisList.Items.Add do begin
        Caption := string(SubStr(FAssis[i1], 1));
        SubItems.Add(string(SubStr(FAssis[i1], 2)));
        s1 := '';
        for i2 := 3 to SubStrCount(FAssis[i1]) do
          s1 := s1 + ', ' + SubStr(FAssis[i1], i2);
        System.Delete(s1, 1, 2);
        SubItems.Add(string(s1));
      end;
    end;
    ...
    for i1 := 0 to high(FForms) do begin
      ...
      with FormList.Items.Add do begin
        Caption := string(FForms[i1][0].name);
        ImageIndex := 1;
      end;
    end;
     ...
     autoSendPrgrBoxCheck.Checked := AutoSendPrgrBox;
      ...
      autoShowBugRepCheck.Checked := AutoShowBugReport;
        showUglyBtnsCheck.Checked := NoOwnerDrawButtons;
             mailAddrEdit.Text    := string(MailAddr);
  ...
  if directCheck.Checked then begin
          ...
          backgroundCheck.Checked := SendInBackground;        
           ...
           bugRepFileEdit.Text    := string(BugReportFile);
        ...
        showRelLinesCheck.Checked := ShowRelativeLines;
           formatDisCheck.Checked := FormatDisassembly;
            limitDisCheck.Checked := LimitDisassembly <> 0;
              filter1Edit.Text    := string(Filter1Classes);
              filter2Edit.Text    := string(Filter2Classes);
   ...
   filter3NoHandlersCheck.Checked := GeneralNoHandlers;
    ...
    for i1 := 0 to high(SettingsStrings) do
      with StringList.Items.Add do begin
        Caption := string(SettingsStrings[i1].description);
        s1 := TPAnsiString(integer(@settings) + SettingsStrings[i1].offset)^;
    ...
    for i1 := 1 to SubStrCount(AdditionalAttachs) do begin
      ...
      with AttachList.Items.Add do begin
        ImageIndex := 9;
        Caption := string(SubStr(s1, 1, '>'));
        SubItems.Add(string(SubStr(s1, 2, '>')));
        SubItems.Add(string(SubStr(s1, 3, '>')));
      end;
    end;
  end;
end;
...
procedure TFMadExceptProjectSettings.FormToSettings(var settings: TProject);
  ...
  function CorrectFilterClasses(classes: AnsiString) : AnsiString;
  begin
    result := classes;
    ReplaceStr(result, ';', ',');
    ReplaceStr(result, '|', ',');
...
begin
  with settings do begin
    ...
    if sendAssisCombo.ItemIndex > 0 then
         SendAssistant    := AnsiString(sendAssisCombo.Items[sendAssisCombo.ItemIndex])
    else SendAssistant    := '';
    if saveAssisCombo.ItemIndex > 0 then
         SaveAssistant    := AnsiString(saveAssisCombo.Items[saveAssisCombo.ItemIndex])
    else SaveAssistant    := '';
    if printAssisCombo.ItemIndex > 0 then
         PrintAssistant   := AnsiString(printAssisCombo.Items[printAssisCombo.ItemIndex])
    else PrintAssistant   := '';
    AutoShowBugReport     :=           autoShowBugRepCheck.Checked;
    NoOwnerDrawButtons    :=             showUglyBtnsCheck.Checked;
    MailAddr              :=       AnsiString(mailAddrEdit.Text);
    ...
    BugReportFile         :=     AnsiString(bugRepFileEdit.Text);
    ...
    Filter1Classes      := CorrectFilterClasses(AnsiString(filter1Edit.Text));
    Filter2Classes      := CorrectFilterClasses(AnsiString(filter2Edit.Text));
    ...
    if Filter1AssisCombo.ItemIndex > 0 then
         Filter1Assis := AnsiString(Filter1AssisCombo.Items[Filter1AssisCombo.ItemIndex])
    else Filter1Assis := '';
    if Filter2AssisCombo.ItemIndex > 0 then
         Filter2Assis := AnsiString(Filter2AssisCombo.Items[Filter2AssisCombo.ItemIndex])
    else Filter2Assis := '';
    if Filter3AssisCombo.ItemIndex > 0 then
         GeneralAssis := AnsiString(Filter3AssisCombo.Items[Filter3AssisCombo.ItemIndex])
    else GeneralAssis := '';
    ...
    for i1 := 0 to high(SettingsStrings) do begin
      ...
      if li <> nil then begin
        s1 := AnsiString(li.SubItems[0]);
        ReplaceStr(s1, '%LF%', #$D#$A);
        TPAnsiString(integer(@settings) + SettingsStrings[i1].offset)^ := s1;
      end;
    end;
    i2 := 0;
    li := AttachList.FindCaption(0, '<bug report>', false, true, false);
    if li <> nil then begin
      FBugReportSendAs := AnsiString(li.SubItems[0]);
      FBugReportZip    := AnsiString(li.SubItems[1]);
      inc(i2);
    end;
    li := AttachList.FindCaption(0, '<screen shot>', false, true, false);
    if li <> nil then begin
      FScreenShotSendAs := AnsiString(li.SubItems[0]);
      FScreenShotZip    := AnsiString(li.SubItems[1]);
      inc(i2);
    end;
    ...
    for i1 := i2 to AttachList.Items.Count - 1 do
      with AttachList.Items[i1] do
        AdditionalAttachs := AdditionalAttachs + '|' + AnsiString(Caption) + '>' + AnsiString(SubItems[0]) + '>' + AnsiString(SubItems[1]);
...
function CompareProjectSettings(const p1, p2: TProject) : boolean;
  ...
  function DAStrEqual(const das1, das2: TDAAnsiString) : boolean;
  var i1 : integer;
  begin
...
function GetVersionFromResources : AnsiString;
var arrCh      : array [0..MAX_PATH] of AnsiChar;
    c1, c2, c3 : dword;
    s1         : AnsiString;
begin
  ...
  s1 := arrCh;
  KillStr(s1, 'Wizard');
  c1 := GetModuleHandleA(PAnsiChar(s1));
  if c1 = 0 then
    c1 := GetModuleHandleA(arrCh);
  if c1 <> 0 then begin
    c2 := FindResourceA(c1, 'DESCRIPTION', pointer(RT_RCDATA));
    if c2 <> 0 then begin
      c3 := LoadResource(c1, c2);
      if c3 <> 0 then begin
        result := AnsiString(UnicodeString(PWideChar(LockResource(c3))));
        UnlockResource(c3);
        FreeResource(c3);
      end;
    end;
  ...
  if result <> '' then begin
    Delete(result, 1, Pos(AnsiString(' '), result));
    Delete(result, Pos(AnsiString('-'), result), maxInt);
    TrimStr(result);
    result := result + ' ';
  end;
end;
...
function EditProjectSettings(var project: TProject) : boolean;
  ...
  function EncryptPassword(password, account, server: AnsiString) : AnsiString;
  begin
    result := password;
  ...
  with TFMadExceptProjectSettings.Create(nil), project do
    try
      ...
      try
        ...
        if FPrj = nil then
             Caption := string(s1) + 'default settings...'
        else Caption := string(s1) + 'settings  ·  ' + ExtractFileName(IOTAProject(FPrj).FileName);
 ...
 filter3NoScreenshotCheck.Enabled := b2;
  ...
  if attachBugRepCheck.Checked <> (li1 <> nil) then
    if not attachBugRepCheck.Checked then begin
      FBugReportSendAs := AnsiString(li1.SubItems[0]);
      FBugReportZip    := AnsiString(li1.SubItems[1]);
      li1.Delete;
    end else
      with AttachList.Items.Insert(0) do begin
        ImageIndex := 7;
        Caption := '<bug report>';
        SubItems.Add(string(FBugReportSendAs));
        SubItems.Add(string(FBugReportZip));
      end;
  if attachScreenShotCheck.Checked <> (li2 <> nil) then
    if not attachScreenShotCheck.Checked then begin
      FScreenShotSendAs := AnsiString(li2.SubItems[0]);
      FScreenShotZip    := AnsiString(li2.SubItems[1]);
      li2.Delete;
    end else
      with AttachList.Items.Insert(ord(li1 <> nil)) do begin
        ImageIndex := 8;
        Caption := '<screen shot>';
        SubItems.Add(string(FScreenShotSendAs));
        SubItems.Add(string(FScreenShotZip));
      end;
  FillAssisCombos;
  CheckAssisButtons;
...
begin
  ...
  if li <> nil then begin
    s1 := AnsiString(StringMemo.Text);
    ReplaceStr(s1, #$D#$A, '%LF%');
    li.SubItems[0] := string(s1);
  end;
end;

procedure TFMadExceptProjectSettings.ReleaseIconPanel(var Message: TMessage);

  function LoadBitmap(fileName: AnsiString; size: dword) : AnsiString;
  var i1     : cardinal;
      c1, c2 : dword;
      pbih   : PBitmapInfoHeader;
      s1     : AnsiString;
  ...
  begin
    result := '';
    c1 := CreateFileA(PAnsiChar(fileName), GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_EXISTING, 0, 0);
    if c1 <> INVALID_HANDLE_VALUE then begin
      ...
      if (s1 <> '') and ReadFile(c1, pointer(s1)^, Length(s1), c2, nil) and (c2 = dword(Length(s1))) then begin
        if IsTextEqual(ExtractFileExt(string(fileName)), '.ico') then begin
          b1 := false;
              ...
              with items[i1] do
                if (width = size) and ((height = size) or (height = size * 2)) then begin
                  ...
                  if (bitCount = 32) and (imageSize > size * size * 4) and (imageSize < size * size * 2 * 4) then begin
                    ...
                    SetString(result, PAnsiChar(pbih), sizeOf(pbih^) + size * size * 4);
                    b1 := false;
                    break;
                  end;
                end;
...
//            icon := LoadImageA(0, PAnsiChar(fileName), IMAGE_ICON, size, size, LR_LOADFROMFILE);
        end else begin
  ...
  if Message.wParam = -2 then
    with TOpenDialog.Create(nil) do begin
      ...
      if Execute then begin
        s1 := LoadBitmap(AnsiString(FileName), FCurIcon.Width);
        if s1 <> '' then begin
...
begin
  ...
  s1 := AnsiString((Sender as TSpeedButton).Name);
  case i1 of
...
procedure TFMadExceptProjectSettings.ImgsClick(Sender: TObject);
  ...
  function GetResIcon(name: AnsiString; var bmp: TBitmap; size: integer) : boolean;
  var graphic : dword;     // icon or bitmap handle
  ...
  begin
    ...
    SetLength(buf, size * size);
    c1 := FindResourceA(HInstance, PAnsiChar(name), pointer(RT_BITMAP));
...
begin
  ...
  if FIconPanel = nil then begin
    ...
    with FIconPanel do begin
      ...
      img := AnsiString((Sender as TPaintBox).Name);
      ...
      i1 := 0;
      while GetResIcon(img + IntToStrEx(i1 + 1), bmp, FCurIcon.Width) do
        with TSpeedButton.Create(FIconPanel) do begin
          Name := string(img + IntToStrEx(i1 + 1));
          Tag := i1;
          Left := 2 + i1 * (iw + 8);
          Top := 2;
          Width := iw + 8;
...
function CutFileExt(var path: AnsiString) : AnsiString;
var i1 : integer;
begin
  i1 := PosStr(AnsiString('.'), path, maxInt, 1);
  if i1 <> 0 then begin
...
function PrjFileName(const prj: IUnknown) : AnsiString;
begin
  try
    result := AnsiString(IOTAProject(prj).FileName);
    if IsTextEqual(ExtractFileExt(string(result)), '.bdsproj') or
       IsTextEqual(ExtractFileExt(string(result)), '.dproj'  ) or
       IsTextEqual(ExtractFileExt(string(result)), '.cbproj' ) then
      try
        result := AnsiString(IOTAProject(prj).GetModuleFileEditor(0).GetFileName);
      except
      end;
  except result := '' end;
end;

procedure GetProjectRoot(const project: TProject; var root: AnsiString);
begin
...
begin
  if FCurrentPrj > -1 then
    with IOTAProject(Projects[FCurrentPrj].FPrj).ProjectOptions do begin
      log('current project: ' + Projects[FCurrentPrj].FCurrentFileName + '/' + AnsiString(IOTAProject(Projects[FCurrentPrj].FPrj).FileName));
      try
...
procedure ExpandVars(var str: AnsiString);
var i1, i2 : integer;
    s1, s2 : AnsiString;
begin
  s1 := str;
  repeat
    ...
    i1 := ExpandEnvironmentStringsA(PAnsiChar(s1), nil, 0);
    if i1 > 0 then begin
      SetLength(s2, i1);
      if ExpandEnvironmentStringsA(PAnsiChar(s1), PAnsiChar(s2), i1) > 0 then
        s1 := PAnsiChar(s2);
    end;
  until str = s1;
end;

function GetLibraryStr(str: AnsiString) : AnsiString;
var s1     : AnsiString;
    hk1    : HKEY;
    c1, c2 : dword;
begin
  result := '';
  s1 := AnsiString((BorlandIDEServices as IOTAServices).GetBaseRegistryKey + '\Library');
  if (s1 <> '') and (s1[1] = '\') then
    Delete(s1, 1, 1);
  if RegOpenKeyExA(HKEY_CURRENT_USER, PAnsiChar(s1), 0, KEY_QUERY_VALUE, hk1) = 0 then
    try
      c2 := 0;
      if RegQueryValueExA(hk1, PAnsiChar(str), nil, nil, nil, @c2) = 0 then begin
        c1 := REG_SZ;
        SetLength(result, c2 + 1);
        if (RegQueryValueExA(hk1, PAnsiChar(str), nil, @c1, pointer(result), @c2) = 0) and (c2 > 1) then
             SetLength(result, c2 - 1)
        else result := '';
      end;
...
function GetRootDir : AnsiString;
var s3     : AnsiString;
    hk1    : HKEY;
    c1, c2 : dword;
...
begin
  result := '';
  s3 := AnsiString((BorlandIDEServices as IOTAServices).GetBaseRegistryKey);
  Delete(s3, 1, 1);
  if RegOpenKeyExA(HKEY_LOCAL_MACHINE, PAnsiChar(s3), 0, KEY_QUERY_VALUE, hk1) = 0 then
    try
      c2 := MAX_PATH + 1;
...
function FindUnitPath(const project: IOTAProject; unitName: AnsiString) : AnsiString;

  function CheckPaths(paths: AnsiString) : AnsiString;
  var i1     : integer;
      s1, s2 : AnsiString;
  begin
    result := '';
    ...
    for i1 := 1 to SubStrCount(paths, ';') do begin
      ...
      if s1 <> '' then begin
        ...
        if GetFileAttributesA(PAnsiChar(s1 + '.dpr')) <> dword(-1) then begin
          result := s1 + '.dpr';
          exit;
        end;
        if GetFileAttributesA(PAnsiChar(s1 + '.pas')) <> dword(-1) then begin
          result := s1 + '.pas';
          exit;
        end;
        if GetFileAttributesA(PAnsiChar(s1 + '.inc')) <> dword(-1) then begin
          result := s1 + '.inc';
          exit;
        end;
        if GetFileAttributesA(PAnsiChar(s1 + '.asm')) <> dword(-1) then begin
          result := s1 + '.cpp';
          exit;
        end;
        if GetFileAttributesA(PAnsiChar(s1 + '.bpr')) <> dword(-1) then begin
          result := s1 + '.bpr';
          exit;
        end;
        if GetFileAttributesA(PAnsiChar(s1 + '.cpp')) <> dword(-1) then begin
          result := s1 + '.cpp';
          exit;
        end;
        if GetFileAttributesA(PAnsiChar(s1 + '.c')) <> dword(-1) then begin
          result := s1 + '.c';
          exit;
        end;
        if GetFileAttributesA(PAnsiChar(s1 + '.h')) <> dword(-1) then begin
          result := s1 + '.h';
          exit;
        end;
      end;
...
begin
  ...
  if IsTextEqual(ExtractFileName(PrjFileName(project)), unitName + '.bpr') and
     (GetFileAttributesA(PAnsiChar(PrjFileName(project))) <> dword(-1)) then begin
  ...
  for i1 := 0 to project.GetModuleCount - 1 do
    try
      with project.GetModule(i1) do
        if IsTextEqual(Name, unitName) and (GetFileAttributesA(PAnsiChar(AnsiString(FileName))) <> dword(-1)) then begin
          result := FileName;
          exit;
        end;
    except end;
...
function OpenUnitLine(const project: IOTAProject; unitName: AnsiString; line: integer) : boolean;
  ...
  function HandleSourceEditor(const editor: IOTASourceEditor) : boolean;
    ...
    var arrCh : array [0..MAX_PATH] of AnsiChar;
    begin
      result := (GetClassNameA(window, arrCh, MAX_PATH) = 0) or (arrCh <> 'TEditControl');
      if not result then
  ...
  begin
    ...
    if module <> nil then begin
      ...
      if IsTextEqual(ExtractFileExt(s1), '.bdsproj') or
         IsTextEqual(ExtractFileExt(s1), '.dproj'  ) or
         IsTextEqual(ExtractFileExt(s1), '.cbproj' ) then
        try
...
procedure ExtractExtFromDpr(s1: AnsiString; var ext: AnsiString; noDpk: boolean);
var i1, i2 : integer;
    s2     : AnsiString;
begin
  i1 := 1;
...
function FindMapFile : AnsiString;

  function RetAddBackslash(const path: AnsiString) : AnsiString;
  begin
      ...
      with Projects[FCurrentPrj] do begin
        ...
        if binary <> '' then begin
          ...
          if isDpk and (not BCB) then begin
            ...
            if (output = '0') and
               (GetPrivateProfileStringA('Directories', 'PackageDLLOutputDir', '0',
                                         arrCh, MAX_PATH, PAnsiChar(root + '.dof')) > 0) then begin
          ...
          if isDpk then begin
            ...
            if ((output = '') or (output = '0')) and
               (RegOpenKeyExA(HKEY_CURRENT_USER, PAnsiChar(s1), 0, KEY_QUERY_VALUE, hk1) = 0) then
              try
                c2 := MAX_PATH + 1;
          ...
          if ext = '' then begin
            ...
            c1 := CreateFileA(PAnsiChar(PrjFileName(FPrj)),
        ...
        if binary <> '' then begin
          ...
          if GetFileAttributesA(PAnsiChar(mapFile)) <> dword(-1) then
            result := mapFile;
        end;
      end;
...
procedure JumpToBugReportLine(line: AnsiString);

  function IsOrdinal(str: AnsiString; hexAllowed: boolean) : boolean;
  var i1 : integer;
  begin
    ...
    for i1 := 1 to Length(str) do
      if not ((str[i1] in ['0'..'9']) or (hexAllowed and (str[i1] in ['a'..'f', 'A'..'F']))) then begin
        result := false;
        break;
      end;
  end;
...
var addrAbs, addrOff        : pointer;
    ...
    sModule, sUnit, sPublic : AnsiString;
    i1                      : integer;
    s1                      : AnsiString;
begin
  CheckCurrentProject;
  ...
  for i1 := 1 to SubStrCount(line, ' ') do begin
    s1 := SubStr(line, i1, ' ');
    if s1 <> '' then begin
      if IsOrdinal(s1, i1 < 3) then begin
        if s1[1] = '+' then begin
          Delete(s1, 1, 1);
          if lineAbs <> 0 then
...
{$ifdef TestProjectOptions}
  procedure PostWarning(project, warning: AnsiString; logWarning: boolean = true); forward;
      ...
      for i1 := 0 to high(names) do
        try
          case names[i1].Kind of
            ...
            tkLString:
              try
                PostWarning('', names[i1].Name + ' (AnsiString = ' + IntToStr(ord(names[i1].Kind)) + ') -> "' + project.ProjectOptions.Values[names[i1].Name] + '"');
              except
                PostWarning('', names[i1].Name + ' (AnsiString = ' + IntToStr(ord(names[i1].Kind)) + ') -> exception');
              end;
            tkString:
              try
                ss := project.ProjectOptions.Values[names[i1].Name];
                PostWarning('', names[i1].Name + ' (AnsiString = ' + IntToStr(ord(names[i1].Kind)) + ') -> "' + ss + '"');
              except
                PostWarning('', names[i1].Name + ' (AnsiString = ' + IntToStr(ord(names[i1].Kind)) + ') -> exception');
              end;
            else
...
function ShowDelphiForm(className: AnsiString) : boolean;
var i1 : integer;
begin
  result := false;
...
procedure PostWarning(project, warning: AnsiString; logWarning: boolean = true);
begin
  if logWarning then
    log('warning "' + warning + '"');
...
function Reg_ValExists(key: HKEY; const path, name: AnsiString) : boolean;
var len : dword;
begin
  result := false;
  if RegOpenKeyExA(key, PAnsiChar(path), 0, KEY_QUERY_VALUE, key) = 0 then begin
    result := (RegQueryValueExA(key, PAnsiChar(name), nil, nil, nil, @len) = 0) and (len > 1);
    RegCloseKey(key);
  end;
end;

function ExtractUsesClause(var project: TProject) : AnsiString;
...
begin
  ...
  try
    ...
    SetLength(s1, $8000);
    GetReader(ed, reader);
    SetLength(s1, reader.GetText(0, PAnsiChar(s1), $8000 - 1));
    reader := nil;
    i1 := 1;
    while i1 < Length(s1) do
      case s1[i1] of
...
function ModifyProjectUses(var project: TProject; unitName, afterUnitName: AnsiString; addUnit: boolean) : boolean;
                  ...
                  if (pos > 0) and ((s1[i1] = ';') or (not b2)) then begin
                    if addUnit then begin
                      with ed.CreateWriter do begin
                        CopyTo(pos);
                        Insert(PAnsiChar(#$D#$A + #$D#$A + 'uses ' + unitName + ';'));
                      end;
                      result := true;
                    end;
                    exit;
                    ...
                    if addUnit and (afterUnitName = '') then begin
                      with ed.CreateWriter do begin
                        if s1[i1 + 4] in [#$D, #$A] then begin
                          CopyTo(i1 + 5);
                          Insert(PAnsiChar('  ' + unitName + ',' + #$D#$A));
                        end else begin
                          CopyTo(i1 + 3);
                          Insert(PAnsiChar(' ' + unitName + ','));
                        end;
                      end;
                      result := true;
                      exit;
...
function CheckProjectLinks(var project: TProject; unitName: AnsiString; checkOnly, addUnit: boolean) : boolean;
  ...
  function IsUnitContained(s1: AnsiString) : boolean;
  var i1, i2 : integer;
  begin
    i1 := 1;
    while i1 < Length(s1) do
          ...
          if (not Reg_ValExists(HKEY_CURRENT_USER,  'Software\madshi\madExcept', 'dontTouchUses')) and
            ...
            if addUnit then begin
              with ed.CreateWriter do begin
                ...
                if ExtractFileExt(unitName) <> '' then
                     Insert(PAnsiChar('#include <'     + unitName + '>' + #$D#$A))
                else Insert(PAnsiChar('#pragma link "' + unitName + '"' + #$D#$A));
              end;
              result := true;
            end else begin
              if ExtractFileExt(unitName) <> '' then
...
procedure CheckProjectUses(var project: TProject);
  ...
  procedure CheckPathOption(option, s2: AnsiString);
  ...
  procedure CheckUsesClause(uss, unit_: AnsiString; addUnit: boolean);
  const CMMs : PAnsiChar = 'ShareMem|SimpleShareMem|HPMM|MultiMM|nxReplacementMemoryManager|dbisammm|FastMM3|FastMM4|RecyclerMM|SmartHeapMM';

    function FindFirstNonMM : AnsiString;
    var i1 : integer;
        s1 : AnsiString;
    begin
      result := '';
...
begin
  ...
  del := '';
  for i1 := 0 to high(Plugins) do
    if ( (@Plugins[i1].procOldA <> nil) and GetProcInfo(@Plugins[i1].procOldA, s1, s2) ) or
       ( (@Plugins[i1].procOldW <> nil) and GetProcInfo(@Plugins[i1].procOldW, s1, s2) ) or
       ( (@Plugins[i1].procNewA <> nil) and GetProcInfo(@Plugins[i1].procNewA, s1, s2) ) or
       ( (@Plugins[i1].procNewW <> nil) and GetProcInfo(@Plugins[i1].procNewW, s1, s2) ) then begin
      b1 := false;
      ...
      if (not SubTextExists(add, s1, ',')) and b1 and project.Enabled then
        add := add + ',' + s1;
    end;
  for i1 := 0 to high(Plugins) do
    if ( (@Plugins[i1].procOldA <> nil) and GetProcInfo(@Plugins[i1].procOldA, s1, s2) ) or
      ...
      if (not SubTextExists(add, s1, ',')) and (not (b1 and project.Enabled)) then
        del := del + ',' + s1;
    end;
  Delete(add, 1, 1);
  Delete(del, 1, 1);
  ...
  if (s1 <> '') and (s1[2] in ['b'..'c', 'B'..'C']) then begin
    for i1 := 1 to SubStrCount(add, ',') do
      if CheckProjectLinks(project, SubStr(add, i1, ','), false, true) then
        SetProjectModified;
    for i1 := 1 to SubStrCount(del, ',') do
      if CheckProjectLinks(project, SubStr(del, i1, ','), false, false) then
        SetProjectModified;
    if project.Enabled and CheckProjectLinks(project, 'ExcMagic', true, false) then
      PostWarning(PrjFileName(project.FPrj), 'Exceptional Magic seems to be enabled in addition to madExcept', false);
    ...
    for i1 := 1 to SubStrCount(add, ',') do
      CheckUsesClause(uss, SubStr(add, i1, ','), true);
    for i1 := 1 to SubStrCount(del, ',') do
      CheckUsesClause(uss, SubStr(del, i1, ','), false);
    if project.Enabled and SubTextExists(uss, 'ExcMagic', ',') then
      PostWarning(PrjFileName(project.FPrj), 'Exceptional Magic seems to be enabled in addition to madExcept', false);
    b1 := project.Enabled and
  ...
  if (not Reg_ValExists(HKEY_CURRENT_USER,  'Software\madshi\madExcept', 'dontTouchDefines')) and
    ...
    {$ifdef ver180}
      ...
      if (s2 <> '') and (s2[2] in ['b'..'c', 'B'..'C']) then begin
        try
          if IOTAProject(project.FPrj).ProjectOptions.Values['DebugInfo'] <> -1 then ;
          s1 := IOTAProject(project.FPrj).ProjectOptions.Values['bcc32.D.value'];
        except
          // BCB 2007 or newer
          s1 := IOTAProject(project.FPrj).ProjectOptions.Values['BCC_Defines'];
        end;
      end else
      ...
      {$ifdef ver180}
        ...
        if (s2 <> '') and (s2[2] in ['b'..'c', 'B'..'C']) then begin
          try
            ...
            // BCB 2007 or newer
            IOTAProject(project.FPrj).ProjectOptions.Values['BCC_Defines'] := s1;
          end;
        end else
    ...
    for i1 := 0 to high(Projects) do
      if Projects[i1].FPrj = FPrj then
        with Projects[i1] do begin
          ...
          if GetFileAttributesA(PAnsiChar(FCurrentFileName + '.mes')) <> maxCard then
            FModified := true;
        ...
        if Enabled then begin
          with IOTAProject(FPrj).ProjectOptions do begin
            ...
            try
              ...
              // BCB 2007 or newer
              try
                Values['BCC_SourceDebuggingOn'] := 'true';
                Values['BCC_DebugLineNumbers' ] := 'true';
                Values['ILINK_FullDebugInfo'  ] := 'true';
              except
                PostWarning(PrjFileName(FPrj), 'Checking compiler/linker options failed.', false);
              end;
            end;
          end;
        end;
      ...
      with Projects[FCurrentPrj] do begin
        ...
        if binary <> '' then begin
          ...
          if ext = '' then begin
            ...
            c1 := CreateFileA(PAnsiChar(PrjFileName(FPrj)), GENERIC_READ, FILE_SHARE_READ, nil,
        ...
        if binary <> '' then begin
          if Enabled then begin
            ...
            mapFile := mapFile + '.map';
            log('map file: "' + mapFile + '"');
            c1 := FindFirstFileA(PAnsiChar(binary), wfd);
            if c1 <> INVALID_HANDLE_VALUE then begin
            ...
            madFile := output + '.mad';
            SetFileAttributesA(PAnsiChar(madFile), 0);
            DeleteFile(madFile);
          end;
...
procedure AfterCompileHack(package: AnsiString);
var ext : AnsiString;
begin
          ...
          if Owners[i1] = Projects[i2].FPrj then
            with Projects[i2] do begin
              ...
              if (not FModified) and
                 (GetFileAttributesA(PAnsiChar(s1 + '.amf')) = maxCard) and
                 (GetFileAttributesA(PAnsiChar(s1 + '.mes')) = maxCard) then begin
                Enabled := true;
                FModified := true;
                SetProjectModified;
              end;
...
begin
  ...
  if s1 <> '' then begin
    ...
    {$ifdef ver120}fh := FindFirstFileA(PAnsiChar(s1 + '\*40.bpl'), wfd);{$endif}
    {$ifdef ver130}fh := FindFirstFileA(PAnsiChar(s1 + '\*50.bpl'), wfd);{$endif}
    {$ifdef ver140}fh := FindFirstFileA(PAnsiChar(s1 + '\*60.bpl'), wfd);{$endif}
    {$ifdef ver150}fh := FindFirstFileA(PAnsiChar(s1 + '\*70.bpl'), wfd);{$endif}
    {$ifdef ver170}fh := FindFirstFileA(PAnsiChar(s1 + '\*90.bpl'), wfd);{$endif}
    {$ifdef ver180}fh := FindFirstFileA(PAnsiChar(s1 + '\*100.bpl'), wfd);{$endif}
    {$ifdef ver200}fh := FindFirstFileA(PAnsiChar(s1 + '\*120.bpl'), wfd);{$endif}
...
begin
  ...
  if pluginList <> nil then begin
    for i1 := pluginList.Count - 1 downto 0 do
      UnloadPackage(GetModuleHandleA(PAnsiChar(AnsiString(pluginList[i1]))));
    pluginList.Free;
    pluginList := nil;
  end;
...
function TypToImageIndex(typ: AnsiString) : integer;
begin
...
procedure TFMadExceptProjectSettings.PropPaintPaint(Sender: TObject);
  ...
  function PropToText(i1, i2, i3: integer) : AnsiString;
  begin
...
procedure TFMadExceptProjectSettings.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
  ...
  begin
    ...
    if (GetFileAttributesA(PAnsiChar(s1)) <> maxCard) and
       (FindExecutableA(PAnsiChar(s1), nil, arrCh) > 32) then
      ShellExecuteA(0, nil, arrCh, PAnsiChar('"' + s1 + '"'), nil, SW_SHOWNORMAL);
  end;
...
begin
  ...
  if (formItem <> nil) and (compItem <> nil) and
     (MessageBoxA(Handle, PAnsiChar('Do you really want to delete the component "' + AnsiString(compItem.Caption) + '"?'),
...
begin
  formItem := FormList.Selected;
  if (formItem <> nil) and
     (MessageBoxA(Handle, PAnsiChar('Do you really want to delete the form "' + AnsiString(formItem.Caption) + '"?'),
                  'Question...', MB_YESNO or MB_ICONQUESTION) = IDYES) then
    for i1 := 0 to high(FForms) do
...
begin
  ...
  if formItem <> nil then
    for i1 := 0 to high(FForms) do
      if IsTextEqual(FForms[i1, 0].name, formItem.Caption) then begin
        Enabled := false;
        CreateAssistant('Unnamed Assistant', [StructToDfm(FForms[i1])], nil).ShowModal(Handle);
        Enabled := true;
        BringToFront;
        break;
      end;
...
function TFMadExceptProjectSettings.EditAssistant(var assis: AnsiString) : boolean;
var astr1, astr2 : TDAAnsiString;
    i1, i2       : integer;
begin
...
begin
  ...
  if (li <> nil) and (li.Index < Length(FAssis)) and
     (MessageBoxA(Handle, PAnsiChar('Do you really want to delete the assistant "' + AnsiString(li.Caption) + '"?'),
...
begin
  ...
  if (li <> nil) and (li.Index < Length(FAssis)) then begin
    ...
    if astr <> nil then begin
      Enabled := false;
      CreateAssistant(SubStr(FAssis[li.Index], 2), astr, nil).ShowModal(Handle);
      Enabled := true;
      BringToFront;
    end;
  end;
