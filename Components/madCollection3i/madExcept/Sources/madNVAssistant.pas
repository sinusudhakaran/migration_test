// ***************************************************************
//  madNVAssistant.pas        version:  1.0c  ·  date: 2009-02-09
//  -------------------------------------------------------------
...
// 2009-02-09 1.0c Delphi 2009 support
...
type
  ...
  INVEdit = interface (INVItem) ['{22AC205D-0FED-425B-BEBB-9FBAB850C1DC}']
    ...
    procedure SetOutputName (value: AnsiString);
  ...
  INVImage = interface (INVItem) ['{A0DE7AC1-F790-4BDB-B6B2-453AF25A6B7B}']
    ...
    property  File_     : AnsiString read GetFile;
  ...
  INVForm = interface ['{9C62303F-3C4A-4E2F-9C14-74C518871995}']
    ...
    procedure SetMessage        (value: AnsiString);
    ...
    function  GetActiveControl  : AnsiString;
    procedure SetActiveControl  (value: AnsiString);
    ...
    property  Message        : AnsiString               read GetMessage_      write SetMessage;
    ...
    property  ActiveControl  : AnsiString               read GetActiveControl write SetActiveControl;
    ...
    function  nvEdit     (item: AnsiString) : INVEdit;
    function  nvLabel    (item: AnsiString) : INVLabel;
    function  nvButton   (item: AnsiString) : INVButton;
    function  nvCheckBox (item: AnsiString) : INVCheckBox;
    function  nvImage    (item: AnsiString) : INVImage;
  ...
  INVAssistant = interface ['{14A3AFE4-B39F-4745-8F85-08FFABE8771C}']
    ...
    function  GetForm        (name  : AnsiString) : INVForm;
    function  GetModalResult : TNVModalResult;
    function  GetOnAction    : TNVActionHandler;
    procedure SetOnAction    (value: TNVActionHandler);
    ...
    property  Form        [name  : AnsiString] : INVForm read GetForm;
    property  ModalResult : TNVModalResult               read GetModalResult;
    property  OnAction    : TNVActionHandler             read GetOnAction write SetOnAction;
...
function CreateAssistant (title      : AnsiString;
                          const dfms : array of AnsiString;
                          exception  : IUnknown {$ifndef bcb} = nil {$endif} ) : INVAssistant;
function LoadAssistant (module      : dword;
                        title       : AnsiString;
                        const forms : array of AnsiString;
                        exception   : IUnknown {$ifndef bcb} = nil {$endif} ) : INVAssistant;
var
...
type
  ...
  private
    ...
    FOutputName    : AnsiString;
    constructor Create (parent: TNVForm; name: AnsiString);
    function  GetTNVItem : TNVItem;
    procedure SetProperty (prop, value: AnsiString); virtual;
    ...
    procedure CreateWindow (ix, iy: integer; clss, text: AnsiString; style: dword; tabStop: boolean);
  ...
  private
    ...
    FInvisible   : boolean;
    constructor Create (parent: TNVForm; name, caption: AnsiString);
    procedure SetProperty (prop, value: AnsiString); override;
  ...
  private
    ...
    FData           : pointer;
    FSkip           : boolean;
    constructor Create (parent: TNVAssistant; title, dfm: AnsiString;
    ...
    function  FindClass  (item: AnsiString; guid: TGuid) : INVItem;
  ...
  private
    ...
    FException   : IUnknown;
    FData        : pointer;
    constructor Create (title: AnsiString; const dfms: array of AnsiString; exception: IUnknown);
    function  GetTitle : AnsiString;
    procedure SetTitle (value: AnsiString);
    function  GetFormCount : integer;
    function  GetForms (index: integer) : INVForm;
...
function BooleanValue(value: AnsiString) : boolean;
begin
  result := IsTextEqual(value, AnsiString('yes')) or IsTextEqual(value, AnsiString('true')) or (value = '1');
end;

function IntegerValue(value: AnsiString) : integer;
begin
  result := StrToIntEx(false, PAnsiChar(value), Length(value));
end;
...
function GetFont(font: AnsiString; size: integer; bold: boolean) : dword;
...
begin
  ...
  result := CreateFontA(-size, 0, 0, 0, weight, 0, 0, 0, DEFAULT_CHARSET,
                        OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                        DEFAULT_PITCH or FF_DONTCARE, PAnsiChar(font));
  if result = 0 then
    result := CreateFontA(-size, 0, 0, 0, weight, 0, 0, 0, DEFAULT_CHARSET,
                          OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
...
function GetTextExtent(dc: dword; text: AnsiString; defaultX: integer = 0; defaultY: integer = 0) : TPoint;
// calculate the extent of a specific text on the specified dc
begin
  if not GetTextExtentPoint32A(dc, PAnsiChar(text), Length(text), TSize(result)) then begin
    result.X := defaultX;
    result.Y := defaultY;
  end;
end;
...
procedure WrText(dc: dword; ix, iy: integer; text, font: AnsiString; size: integer;
...
begin
  ...
  if (iw <> 0) and GetTextExtentPoint32A(dc, PAnsiChar(text), Length(text), te) then begin
    ...
    for ix2 := -1 to +1 do
      for iy2 := -1 to +1 do
        if (ix2 <> 0) or (iy2 <> 0) then
          TextOutA(dc, ix + ix2, iy + iy2, PAnsiChar(text), length(text));
    SetTextColor(dc, color);
  end;
  TextOutA(dc, ix, iy, PAnsiChar(text), length(text));
...
constructor TNVItem.Create(parent: TNVForm; name: AnsiString);
begin
  inherited Create;
  FParent  := parent;
  FName    := name;
...
procedure TNVItem.SetProperty(prop, value: AnsiString);

  function StrToOutputType(value: AnsiString) : TOutputType;
  begin
    if PosText(AnsiString('ownSection'), value) > 0 then
         result := otOwnSection
...
begin
  ...
  if FHandle <> 0 then begin
    SendMessageA(FHandle, WM_SETFONT, integer(FParent.FFont), 0);
...
procedure TNVItem.CreateWindow(ix, iy: integer; clss, text: AnsiString; style: dword; tabStop: boolean);
...
begin
  ...
  with FRect do
    FHandle := windows.CreateWindowA(PAnsiChar(clss), PAnsiChar(text), style + WS_CHILD,
...
function TNVItem.GetOutputName : AnsiString;  begin result := FOutputName end;
function TNVItem.GetName       : AnsiString;  begin result := FName       end;
...
procedure TNVItem.SetOutputName(value: AnsiString);
begin
  FOutputName := value;
end;
...
begin
  if FHandle <> 0 then begin
    ...
    if Self is TNVEdit then
      SendMessageA(FHandle, EM_SETSEL, 0, -1);
  end;
end;                                  

// ***************************************************************

constructor TNVButton.Create(parent: TNVForm; name, caption: AnsiString);
begin
  inherited Create(parent, name);
  FCaption := caption;
end;

procedure TNVButton.SetProperty(prop, value: AnsiString);
begin
...
function TNVButton.GetCaption : AnsiString;
begin
  if FCaption = '' then
       result := FName
...
procedure TNVButton.SetCaption(value: AnsiString);
begin
  FCaption := value;
end;
...
procedure TNVLabel.SetProperty(prop, value: AnsiString);
begin
...
begin
  ...
  DrawTextA(fontDC, PAnsiChar(GetCaption), length(GetCaption), result, DT_CALCRECT or DT_WORDBREAK);
end;
...
function TNVLabel.GetCaption : AnsiString;
begin
  if FCaption = '' then
       result := FName
  else result := FCaption;
...
procedure TNVLabel.SetCaption(value: AnsiString);
begin
  FCaption := value;
end;
...
procedure TNVEdit.SetProperty(prop, value: AnsiString);
begin
...
function TNVEdit.GetText : AnsiString;
begin
  if FHandle <> 0 then begin
    SetLength(FText, SendMessageA(FHandle, WM_GETTEXTLENGTH, 0, 0) + 1);
    SetLength(FText, SendMessageA(FHandle, WM_GETTEXT, Length(FText), integer(FText)));
  end;
  result := FText;
end;

procedure TNVEdit.SetText(value: AnsiString);
begin
  FText := value;
  if (value <> GetText) and (FHandle <> 0) then
    SendMessageA(FHandle, WM_SETTEXT, 0, integer(value));
  if FParent <> nil then
...
procedure TNVCheckBox.SetProperty(prop, value: AnsiString);
begin
  if      prop = 'Caption' then FCaption := value
...
begin
  ...
  CreateWindow(ix, iy, 'Button', PAnsiChar(FCaption), BS_NOTIFY or BS_OWNERDRAW, true);
  if FChecked then
    SendMessageA(FHandle, BM_SETCHECK, BST_CHECKED, 0);
  inherited;
end;
...
begin
  ...
  if itemState and ODS_DISABLED <> 0 then
    WrText(dc, r2.Left + 2, r2.Top + 1, PAnsiChar(FCaption), 'Tahoma', 11, false, false, GetSysColor(COLOR_WINDOW));
  WrText(dc, r2.Left + 1, r2.Top, PAnsiChar(FCaption), 'Tahoma', 11, false, false, c1);
  if itemState and ODS_FOCUS <> 0 then
    DrawFocusRect(dc, r2);
end;

function  TNVCheckBox.GetCaption : AnsiString;        begin result := FCaption end;
procedure TNVCheckBox.SetCaption (value: AnsiString); begin FCaption := value  end;
...
procedure TNVImage.SetProperty(prop, value: AnsiString);
begin
...
function TNVImage.GetFile      : AnsiString; begin result := FFile                     end;
...
constructor TNVForm.Create(parent: TNVAssistant; title, dfm: AnsiString;
                           exception: IUnknown; currentStep, maxStep: integer);
  ...
  function ConvertObject(var index: integer) : boolean;
    ...
    function ConvertProperty(const item: INVItemEx; var index: integer) : boolean;
      ...
      procedure ReadProperty(const item: INVItemEx; prop, val: AnsiString);
      ...
      begin
        if item = nil then begin
          ...
          if      prop = 'MinWidth'      then MinWidth       := IntegerValue(val)
            ...
            if      IsTextEqual(val, AnsiString('madExcept.HandleContactForm'   )) then FOnAction := HandleContactFormProc
            else if IsTextEqual(val, AnsiString('madExcept.HandleScreenshotForm')) then FOnAction := HandleScreenshotFormProc
            else begin
              s1 := SubStr(val, 2, '.');
              if s1 = '' then begin
                s1 := val;
    ...
    begin
      ...
      case dfm[index - 1] of
        ...
        #$6      : begin
                     SetString(s2, PAnsiChar(dfm) + index, TPByte(@dfm[index])^);
                     ReadProperty(item, s1, s2);
                     inc(index, 1 + TPByte(@dfm[index])^);
                   end;
        #$7      : begin
                     SetString(s2, PAnsiChar(dfm) + index, ord(dfm[index]));
                     ReadProperty(item, s1, s2);
                     inc(index, 1 + ord(dfm[index]));
                   end;
        #$8, #$9 : ReadProperty(item, s1, IntToStrEx(ord(dfm[index - 1] = #$9)));
        #$c      : begin
                     SetString(s2, PAnsiChar(dfm) + index + 3, TPInteger(@dfm[index])^);
                     ReadProperty(item, s1, s2);
                     inc(index, 4 + TPInteger(@dfm[index])^);
                   end;
        else       begin
                     MessageBoxA(0, PAnsiChar('Internal error: Invalid assistant resource (' + IntToHexEx(integer(dfm[index - 1])) + ').'), 'madExcept', 0);
                     result := false;
                   end;
      end;
    end;

  var typ, name : AnsiString;
      item      : INVItemEx;
  begin
    SetString(typ, PAnsiChar(dfm) + index, ord(dfm[index]));
    inc(index, ord(dfm[index]) + 1);
    SetString(name, PAnsiChar(dfm) + index, ord(dfm[index]));
    if name = 'AbortBtn' then
      name := 'CancelBtn';
    inc(index, ord(dfm[index]) + 1);
    item := nil;
    typ[1] := 'T';
    if typ = AnsiString(TNVButton.ClassName) then begin
      ...
      else if typ = AnsiString(TNVLabel   .ClassName) then item := AddItem(TNVLabel   .Create(self, name))
      else if typ = AnsiString(TNVEdit    .ClassName) then item := AddItem(TNVEdit    .Create(self, name))
      else if typ = AnsiString(TNVCheckBox.ClassName) then item := AddItem(TNVCheckBox.Create(self, name))
      else if typ = AnsiString(TNVImage   .ClassName) then item := AddItem(TNVImage   .Create(self, name));
  ...
  case msg of
    ...
    WM_COMMAND    : begin
                      ...
                      if dword(wParam) shr 16 = BN_SETFOCUS then
                        PostMessageA(dword(lParam), BM_SETSTYLE, GetWindowLong(dword(lParam), GWL_STYLE) or BS_DEFPUSHBUTTON, 1);
    ...
    else if IsWindowUnicode(window) then
           result := DefWindowProcW(window, msg, wParam, lParam)
         else
           result := DefWindowProcA(window, msg, wParam, lParam);
  end;
...
function TNVForm.FindClass(item: AnsiString; guid: TGuid) : INVItem;
...
function  TNVForm.GetName                : AnsiString;        begin result := FName                  end;
function  TNVForm.GetTitle               : AnsiString;        begin result := FTitle                 end;
procedure TNVForm.SetTitle         (value: AnsiString);       begin FTitle := value                  end;
function  TNVForm.GetMessage_            : AnsiString;        begin result := FMessage               end;
procedure TNVForm.SetMessage       (value: AnsiString);       begin FMessage := value                end;
...
function  TNVForm.GetActiveControl       : AnsiString;        begin result := FActiveControl         end;
procedure TNVForm.SetActiveControl (value: AnsiString);       begin FActiveControl := value          end;
...
function TNVForm.nvEdit    (item: AnsiString) : INVEdit;     begin result := INVEdit    (FindClass(item, INVEdit    )) end;
function TNVForm.nvLabel   (item: AnsiString) : INVLabel;    begin result := INVLabel   (FindClass(item, INVLabel   )) end;
function TNVForm.nvButton  (item: AnsiString) : INVButton;   begin result := INVButton  (FindClass(item, INVButton  )) end;
function TNVForm.nvCheckBox(item: AnsiString) : INVCheckBox; begin result := INVCheckBox(FindClass(item, INVCheckBox)) end;
function TNVForm.nvImage   (item: AnsiString) : INVImage;    begin result := INVImage   (FindClass(item, INVImage   )) end;
...
var fontDC, oldFont : dword;
  ...
  begin
    ...
    Handle := CreateWindowExA(WS_EX_DLGMODALFRAME, wndClass.lpszClassname, PAnsiChar(FTitle),
    ...
    SetWindowLongA(Handle, GWL_WNDPROC, integer(wndProc));
  ...
  procedure SelectFont(font: AnsiString; size: integer; bold: boolean);
  begin
...
constructor TNVAssistant.Create(title: AnsiString; const dfms: array of AnsiString; exception: IUnknown);
var i1 : integer;
begin
  inherited Create;
  FTitle     := title;
...
function TNVAssistant.GetTitle : AnsiString;
begin
  result := FTitle;
end;

procedure TNVAssistant.SetTitle(value: AnsiString);
var i1 : integer;
begin
  FTitle := value;
  for i1 := 0 to high(FForms) do
...
function TNVAssistant.GetForm(name: AnsiString) : INVForm;
var i1 : integer;
begin
  result := nil;
  for i1 := 0 to high(FForms) do
...
function CreateAssistant(title: AnsiString; const dfms: array of AnsiString; exception: IUnknown) : INVAssistant;
begin
...
function LoadAssistant(module: dword; title: AnsiString; const forms: array of AnsiString; exception: IUnknown) : INVAssistant;
var dfms : TDAAnsiString;
    i1   : integer;
    c1   : dword;
begin
  ...
  if dfms <> nil then begin
    ...
    if FindResourceA(c1, PAnsiChar('TME' + UpStr(forms[0])), pointer(RT_RCDATA)) <> 0 then
      // switch to the language dll, but only if it contains the resources we need
      module := c1;
    for i1 := 0 to high(dfms) do
      dfms[i1] := ResToStr(module, pointer(RT_RCDATA), PAnsiChar('TME' + UpStr(forms[i1])));
  end;
