// ***************************************************************
//  madExceptPatcher.pas      version:  1.1d  ·  date: 2009-02-09
//  -------------------------------------------------------------
...
// 2009-02-09 1.1d Delphi 2009 support
// 2008-01-14 1.1c fixed big memory leak during patching
...
type
  TProject = record
    ...
    AutoDelay           : dword;
    // EXCEPTION FILTER
    Filter1Classes      : AnsiString;
    Filter2Classes      : AnsiString;
    ...
    Filter1Assis        : AnsiString;
    Filter2Assis        : AnsiString;
    GeneralAssis        : AnsiString;
    ...
    SendBtnIcon         : AnsiString;
    SaveBtnIcon         : AnsiString;
    PrintBtnIcon        : AnsiString;
    ShowBtnIcon         : AnsiString;
    ContinueBtnIcon     : AnsiString;
    CantContinueBtnIcon : AnsiString;
    RestartBtnIcon      : AnsiString;
    CloseBtnIcon        : AnsiString;
    BigIcon             : AnsiString;
    Send32Icon          : AnsiString;
    PleaseWaitIcon      : AnsiString;
    FocusedButton       : TMEButton;
    SendAssistant       : AnsiString;
    SaveAssistant       : AnsiString;
    PrintAssistant      : AnsiString;
    ...
    SmtpServer          : AnsiString;
    SmtpPort            : dword;
    SmtpAccount         : AnsiString;
    SmtpPassword        : AnsiString;
    HttpServer          : AnsiString;
    HttpPort            : dword;
    HttpAccount         : AnsiString;
    HttpPassword        : AnsiString;
    AttachBugReport     : boolean;
    AttachBugReportFile : boolean;
    DeleteBugReportFile : boolean;
    BugReportSendAs     : AnsiString;
    BugReportZip        : AnsiString;
    ScreenShotDepth     : integer;
    ScreenShotAppOnly   : boolean;
    ScreenShotSendAs    : AnsiString;
    ScreenShotZip       : AnsiString;
    AdditionalAttachs   : AnsiString;
    MailFrom            : AnsiString;
    // SAVE SETTINGS
    BugReportFile       : AnsiString;
    ...
    EnabledPlugins      : AnsiString;
    VersionVar          : AnsiString;
    // ASSISTANT CREATOR
    Assistants          : TDAAnsiString;
    Forms               : TDAAnsiString;
    // CUSTOM STRINGS
    TitleBar            : AnsiString;
    ExceptMsg           : AnsiString;
    FrozenMsg           : AnsiString;
    BitFaultMsg         : AnsiString;
    SendBtnCaption      : AnsiString;
    SaveBtnCaption      : AnsiString;
    PrintBtnCaption     : AnsiString;
    ShowBtnCaption      : AnsiString;
    ContinueBtnCaption  : AnsiString;
    RestartBtnCaption   : AnsiString;
    CloseBtnCaption     : AnsiString;
    OkBtnCaption        : AnsiString;
    DetailsBtnCaption   : AnsiString;
    PleaseWaitTitle     : AnsiString;
    PleaseWaitText      : AnsiString;
    MailSubject         : AnsiString;
    MailBody            : AnsiString;
    SendBoxTitle        : AnsiString;
    PrepareAttachMsg    : AnsiString;
    MxLookupMsg         : AnsiString;
    ConnectMsg          : AnsiString;
    AuthMsg             : AnsiString;
    SendMailMsg         : AnsiString;
    FieldsMsg           : AnsiString;
    SendAttachMsg       : AnsiString;
    SendFinalizeMsg     : AnsiString;
    SendFailureMsg      : AnsiString;
  end;
...
procedure LoadSettingsFromIni (fileName: AnsiString; var project: TProject);
procedure SaveSettingsToIni   (fileName: AnsiString; const project: TProject);
function  LoadDefaultSettings (var project: TProject) : boolean;
procedure SaveDefaultSettings (const project: TProject);

function GetProcInfo (proc: pointer; var unitName, procName: AnsiString) : boolean;

function PatchBinary (var project: TProject; binary, root, map: AnsiString;
                      compileSucceeded: boolean; var infos: AnsiString) : integer;
...
function LoadStrFromFile(file_: UnicodeString) : AnsiString;
var fh, c1 : dword;
begin
...
function EncodeStr(str: AnsiString) : AnsiString;
...
function DecodeStr(str: AnsiString) : AnsiString;
...
function RegGetStr(hk: HKEY; name: AnsiString; var value: AnsiString) : boolean;
var s1  : AnsiString;
    len : dword;
begin
  result := false;
  len := 0;
  if RegQueryValueExA(hk, PAnsiChar(name), nil, nil, nil, @len) = 0 then begin
    SetLength(s1, len);
    if RegQueryValueExA(hk, PAnsiChar(name), nil, nil, pointer(s1), @len) = 0 then begin
...
procedure LoadSettingsFromIni(fileName: AnsiString; var project: TProject);
  ...
  procedure IniGetDword(section, key: AnsiString; var value: dword);
  var arrCh : array [0..MAX_PATH] of AnsiChar;
  begin
    if (GetPrivateProfileStringA(PAnsiChar(section), PAnsiChar(key), '´', arrCh, MAX_PATH, PAnsiChar(fileName)) <> 1) or
       (arrCh[0] <> '´') then begin
      value := StrToIntDef(string(arrCh), value);
      log('[' + section + ']\' + key + ' = ' + IntToStrEx(value));
    end;
  end;

  function IniGetBoolean(section, key: AnsiString; var value: boolean) : boolean;
  var arrCh : array [0..MAX_PATH] of AnsiChar;
  begin
    result := false;
    if (GetPrivateProfileStringA(PAnsiChar(section), PAnsiChar(key), '´', arrCh, MAX_PATH, PAnsiChar(fileName)) <> 1) or
       (arrCh[0] <> '´') then begin
      result := true;
      value := StrToIntDef(string(arrCh), ord(value)) <> 0;
      log('[' + section + ']\' + key + ' = ' + booleanToChar(value));
    end;
  end;

  function IniGetStr(section, key: AnsiString; var value: AnsiString) : boolean;
  var pc : PAnsiChar;
  begin
    GetMem(pc, 32 * 1024);
    if (GetPrivateProfileStringA(PAnsiChar(section), PAnsiChar(key), '´', pc, 32 * 1024, PAnsiChar(fileName)) <> 1) or
       (pc[0] <> '´') then begin
      result := true;
      value := pc;
      if PosText(AnsiString('Assistent'), key) > 0 then
...
begin
  ...
  if RegOpenKeyEx(HKEY_LOCAL_MACHINE, 'Software\madshi\madExcept', 0, KEY_QUERY_VALUE, hk) = 0 then begin
    if RegGetStr(hk, 'centralMesFile', s1) and (s1 <> '') then begin
      if ExpandEnvironmentStringsA(PAnsiChar(s1), arrCh, MAX_PATH) > 0 then
        s1 := arrCh;
      if GetFileAttributesA(PAnsiChar(s1)) <> maxCard then begin
        log('Central MES file "' + s1 + '" shall be used!');
        DeleteR(s1, Length(ExtractFileExt(string(s1))));
        fileName := s1;
      end else
  ...
  if fileName <> '' then
    with project do begin
      if (GetPrivateProfileStringA('AppendMapFile', 'Enabled', '´', arrCh, MAX_PATH, PAnsiChar(fileName + '.amf')) <> 1) or
         (arrCh[0] <> '´') then begin
        FModified         := true;
        Enabled           := StrToIntDef(string(arrCh), ord(Enabled)) <> 0;
      ...
      s1 := LoadStrFromFile(UnicodeString(fileName));
      if (s1 <> '') and (Length(s1) < MAX_PATH) and (PosStr(AnsiString(#$D#$A), s1) = 0) then begin
        if (Length(s1) = 1) or
           ( ((s1[2] <> ':')                  ) and
             ((s1[1] <> '\') or (s1[2] <> '\'))     ) then
          if s1[1] = '\' then
                 s1 := AnsiString(ExtractFileDrive(string(fileName))) + s1
          else   s1 := AnsiString(ExtractFilePath (string(fileName))) + s1;
      ...
      if IniGetBoolean('BugReport',       'ListModules',                      b1                   ) and b1 and
         (not SubTextExists(EnabledPlugins, AnsiString('modules'))) then
      ...
      if IniGetBoolean('BugReport',       'ListHardware',                     b1                   ) and b1 and
         (not SubTextExists(EnabledPlugins, AnsiString('hardware'))) then
        if EnabledPlugins = '' then
      ...
      if IniGetStr('Assistants', 'Assistant1', s1) then begin
        ...
        while true do begin
          s1 := '';
          IniGetStr    ('Assistants',      'Assistant' + IntToStrEx(i1 + 1),   s1                   );
          if s1 = '' then
            break;
        ...
        if IniGetStr('Assistents', 'Assistent1', s1) then begin
          ...
          while true do begin
            s1 := '';
            IniGetStr    ('Assistents',      'Assistent' + IntToStrEx(i1 + 1),   s1                   );
            if s1 = '' then
              break;
            SetLength(Assistants, i1 + 1);
            Assistants[i1] := s1;
      ...
      if IniGetStr('Assistants', 'Forms1', s1) then begin
        ...
        while true do begin
          s1 := '';
          IniGetStr    ('Assistants',      'Forms' + IntToStrEx(i1 + 1),       s1                   );
          if s1 = '' then
            break;
        ...
        if IniGetStr('Assistents', 'Forms1', s1) then begin
          ...
          while true do begin
            s1 := '';
            IniGetStr    ('Assistents',      'Forms' + IntToStrEx(i1 + 1),       s1                   );
            if s1 = '' then
              break;
            SetLength(Forms, i1 + 1);
            Forms[i1] := s1;
...
procedure SaveSettingsToIni(fileName: AnsiString; const project: TProject);
  ...
  procedure IniSetDword(section, key: AnsiString; value: dword);
  begin
    WritePrivateProfileStringA(PAnsiChar(section), PAnsiChar(key), PAnsiChar(IntToStrEx(value)), PAnsiChar(fileName));
    log('[' + section + ']\' + key + ' := ' + IntToStrEx(value));
  end;

  procedure IniSetBoolean(section, key: AnsiString; value: boolean);
  begin
    WritePrivateProfileStringA(PAnsiChar(section), PAnsiChar(key), PAnsiChar(IntToStrEx(ord(value))), PAnsiChar(fileName));
    log('[' + section + ']\' + key + ' := ' + booleanToChar(value));
  end;

  procedure IniSetStr(section, key, value: AnsiString);
  begin
    if value <> '' then
      value := EncodeStr(value);
    WritePrivateProfileStringA(PAnsiChar(section), PAnsiChar(key), PAnsiChar(value), PAnsiChar(fileName));
    log('[' + section + ']\' + key + ' := "' + value + '"');
  end;

  procedure IniGetStr(section, key: AnsiString; var value: AnsiString);
  var pc : PAnsiChar;
  begin
    GetMem(pc, 32 * 1024);
    if (GetPrivateProfileStringA(PAnsiChar(section), PAnsiChar(key), '´', pc, 32 * 1024, PAnsiChar(fileName)) <> 1) or
  ...
  function IniDelVal(section, key: AnsiString) : boolean;
  var s1 : AnsiString;
  begin
    IniGetStr(section, key, s1);
    result := (s1 <> '') and WritePrivateProfileStringA(PAnsiChar(section), PAnsiChar(key), nil, PAnsiChar(fileName));
    log('del [' + section + ']\' + key + ' -> ' + booleanToChar(result));
  end;

  procedure IniDelSection(section: AnsiString);
  begin
    WritePrivateProfileStringA(PAnsiChar(section), nil, nil, PAnsiChar(fileName));
    log('del [' + section + ']\*');
  end;
...
begin
  ...
  if (fileName <> '') and (not project.MadExcept2Only) then
    with project do begin
      SetFileAttributesA(PAnsiChar(fileName + '.amf'), 0);
      DeleteFile(string(fileName + '.amf'));
      fileName := fileName + '.mes';
      s1 := LoadStrFromFile(UnicodeString(fileName));
      if (s1 <> '') and (Length(s1) < MAX_PATH) and (PosStr(AnsiString(#$D#$A), s1) = 0) then begin
        if (Length(s1) = 1) or
      ...
      IniSetBoolean('BugReport',       'ListModules',                      SubTextExists(EnabledPlugins, AnsiString('modules' )));
      IniSetBoolean('BugReport',       'ListHardware',                     SubTextExists(EnabledPlugins, AnsiString('hardware')));
      ...
      for i1 := 0 to high(Assistants) do
        IniSetStr    ('Assistants',      'Assistant' + IntToStrEx(i1 + 1),   Assistants[i1]       );
      for i1 := Length(Assistants) to maxInt - 1 do
        if not IniDelVal('Assistants', 'Assistant' + IntToStrEx(i1 + 1)) then
          break;
      if Assistants = nil then
        IniSetStr    ('Assistants',      'Assistant1',                       ''                   );
      for i1 := 0 to high(Forms) do
        IniSetStr    ('Assistants',      'Forms' + IntToStrEx(i1 + 1),       Forms[i1]            );
      for i1 := Length(Forms) to maxInt - 1 do
        if not IniDelVal('Assistants', 'Forms' + IntToStrEx(i1 + 1)) then
          break;
      if Forms = nil then
      ...
      WritePrivateProfileStringA(nil, nil, nil, PAnsiChar(fileName));
    end;
  unindentLog;
end;
...
var hk : HKEY;
  ...
  function RegGetBinary(len: dword; name: AnsiString; var value) : boolean;
  begin
    result := RegQueryValueExA(hk, PAnsiChar(name), nil, nil, @value, @len) = 0;
    if result then
      if len = 1 then log(name + ' = ' + booleanToChar(boolean(value)))
      else            log(name + ' = ' + IntToStrEx   (integer(value)));
  end;

  function RegGetDword(name: AnsiString; var value: dword) : boolean;
  begin
    result := RegGetBinary(4, name, value);
  end;

  function RegGetBoolean(name: AnsiString; var value: boolean) : boolean;
  begin
    result := RegGetBinary(1, name, value);
  end;

  function RegGetStr(name: AnsiString; var value: AnsiString) : boolean;
  begin
    result := madExceptPatcher.RegGetStr(hk, name, value);
    if PosText(AnsiString('Assistent'), name) > 0 then
      ReplaceStr(value, 'Assistent', 'Assistant');
  end;
      ...
      with project do begin
        ...
        if result then begin
          ...
          if RegGetStr('Assistant1', s1) then begin
            ...
            while true do begin
              s1 := '';
              RegGetStr    ('Assistant' + IntToStrEx(i1 + 1),   s1                   );
              if s1 = '' then
                break;
              SetLength(Assistants, i1 + 1);
              Assistants[i1] := s1;
            ...
            if RegGetStr('Assistent1', s1) then begin
              ...
              while true do begin
                s1 := '';
                RegGetStr    ('Assistent' + IntToStrEx(i1 + 1),   s1                   );
                if s1 = '' then
                  break;
                SetLength(Assistants, i1 + 1);
                Assistants[i1] := s1;
          ...
          if RegGetStr('Forms1', s1) then begin
            ...
            while true do begin
              s1 := '';
              RegGetStr    ('Forms' + IntToStrEx(i1 + 1),       s1                   );
              if s1 = '' then
                break;
              SetLength(Forms, i1 + 1);
              Forms[i1] := s1;
...
var hk : HKEY;
  ...
  procedure RegSetBin(len: dword; name: AnsiString; const value);
  var typ : dword;
  begin
    if len <> 4 then typ := REG_BINARY
    else             typ := REG_DWORD;
    RegSetValueExA(hk, PAnsiChar(name), 0, typ, @value, len);
    case len of
      1  : log(name + ' := ' + booleanToChar(boolean(value)));
      4  : log(name + ' := ' + IntToStrEx(integer(value)));
      else log(name + ' := <' + IntToStrEx(len) + ' bytes of binary data>');
    end;
  end;

  procedure RegSetDword(name: AnsiString; const value: dword);
  begin
    RegSetBin(4, name, value);
  end;

  procedure RegSetBoolean(name: AnsiString; const value: boolean);
  begin
    RegSetBin(1, name, value);
  end;

  procedure RegSetStr(name, value: AnsiString);
  begin
    RegSetValueExA(hk, PAnsiChar(name), 0, REG_SZ, PAnsiChar(value), Length(value) + 1);
    log(name + ' := "' + value + '"');
  end;

  function RegDelVal(name: AnsiString) : boolean;
  begin
    result := RegDeleteValueA(hk, PAnsiChar(name)) = 0;
  ...
  if c1 = 0 then
    try
      with project do begin
        ...
        RegSetBoolean('ListModules',                      SubTextExists(EnabledPlugins, AnsiString('modules' )));
        RegSetBoolean('ListHardware',                     SubTextExists(EnabledPlugins, AnsiString('hardware')));
        ...
        for i1 := 0 to high(Assistants) do
          RegSetStr    ('Assistant' + IntToStrEx(i1 + 1),   Assistants[i1]       );
        for i1 := Length(Assistants) to maxInt - 1 do
          if not RegDelVal('Assistant' + IntToStrEx(i1 + 1)) then
            break;
        if Assistants = nil then
          RegSetStr    ('Assistant1',                       ''                   );
        for i1 := 1 to maxInt do
          if not RegDelVal('Assistent' + IntToStrEx(i1)) then
            break;
        for i1 := 0 to high(Forms) do
          RegSetBin    (Length(Forms[i1]), 'Forms' + IntToStrEx(i1 + 1), Forms[i1][1]);
        for i1 := Length(Forms) to maxInt - 1 do
          if not RegDelVal('Forms' + IntToStrEx(i1 + 1)) then
            break;
        if Forms = nil then
          RegSetStr    ('Forms1',                         ''                   );
  ...
  if FindModule(proc, module, s1) then begin
    ...
    if result then begin
      ...
      if c1 <> 0 then begin
        ...
        if c2 <> 0 then begin
          pc1 := LockResource(c2);
          i1 := PosPChar(PAnsiChar(unitName), pc1, Length(unitName), SizeOfResource(module, c1), true);
          if i1 >= 0 then
...
function PatchBinary(var project: TProject; binary, root, map: AnsiString;
  ...
  procedure info(info: AnsiString);
  begin
    log('Info: ' + info);
    infos := infos + '|i' + info;
  end;

  procedure warning(warning: AnsiString);
  begin
    log('Warning: ' + warning);
  ...
  function VirtualGetImportEntry(nh: PImageNtHeaders; buf: pointer; module, proc: AnsiString) : cardinal;
  type
  ...
  procedure logMapEntry(var address; name: AnsiString);
  begin
  ...
  function AdjustIcon(root: UnicodeString; name_: AnsiString; handle, size: dword; icon: AnsiString) : boolean;

    function GetResIcon(bmp: AnsiString) : AnsiString;
    var c1, c2 : dword;
    begin
      result := '';
      c1 := FindResourceA(HInstance, PAnsiChar(UpStr(bmp)), pointer(RT_BITMAP));
      if (c1 <> 0) and (SizeOfResource(HInstance, c1) = 40 + size * size * 4) then begin
        c2 := LoadResource(HInstance, c1);
        if c2 <> 0 then begin
          SetString(result, PAnsiChar(LockResource(c2)), 40 + size * size * 4);
          UnlockResource(c2);
          FreeResource(c2);
        end;
      end;
  ...
  var name    : UnicodeString;
      icofile : UnicodeString;
      bmpfile : UnicodeString;
      s1      : AnsiString;
      b1      : boolean;
      i1      : integer;
      pbih    : PBitmapInfoHeader;
  begin
    name    := 'MEI' + UnicodeString(UpStr(name_));
        ...
        if GetFileAttributesW(pwidechar(icofile)) <> dword(-1) then begin
          ...
          if s1 <> '' then begin
            ...
            if b1 then begin
              ...
              warning('The file "mei' + name_ + '.ico" doesn''t contain a ' + IntToStrEx(size) + 'x' + IntToStrEx(size) + ' sized image.');
          end else begin
            UpdateResourceW(handle, PWideChar(RT_BITMAP), PWideChar(name), 0, nil, 0);
          ...
          if s1 <> '' then begin
            ...
            if (pbih^.biWidth = integer(size)) and (pbih^.biHeight = integer(size)) then begin
              ...
              warning('The bmp file "mei' + name_ + '.bmp" doesn''t have the required size (' + IntToStrEx(size) + 'x' + IntToStrEx(size) + ').');
          end else begin
            UpdateResourceW(handle, PWideChar(RT_BITMAP), PWideChar(name), 0, nil, 0);
            //info('resource "mei' + name_ + '" deleted');
          end;
  ...
  function FindExport(module: dword; nh: PImageNtHeaders; proc: AnsiString) : dword;
    ...
    try
      ...
      with ed^ do
        for i1 := 0 to NumberOfNames - 1 do
          if proc = PAnsiChar(module + VirtualToRaw(nh, TPACardinal(module + VirtualToRaw(nh, AddressOfNames))^[i1])) then begin
...
{  function Align(value, align: dword) : dword;
  ...
  function CreateMadExceptDfm(const mapFile: TMapFile; var error: integer; versionVarPtr: dword; maxDebugInfo: AnsiString) : AnsiString;

    procedure AddDword(const name: AnsiString; value: dword);
    begin
      result := result + AnsiChar(Length(name)) + name + #$4 + #0#0#0#0;
      TPCardinal(dword(result) + dword(Length(result)) - 4)^ := value;
    end;

    procedure AddBoolean(const name: AnsiString; value: boolean);
    begin
      result := result + AnsiChar(Length(name)) + name + AnsiChar(8 + ord(value));
    end;

    procedure AddStr(const name, value: AnsiString);
    begin
    ...
    procedure AddDADword(const name: AnsiString; const value: TDACardinal);
    begin
      result := result + AnsiChar(Length(name)) + name + #$a + #0#0#0#0;
      TPInteger(dword(result) + dword(Length(result)) - 4)^ := Length(value) * 4;
      SetLength(result, Length(result) + Length(value) * 4);
      Move(value[0], (PAnsiChar(result) + Length(result) - Length(value) * 4)^, Length(value) * 4);
    end;
  ...
  begin
    ...
    with project do begin
      ...
      if not NoOwnSettings then begin
        ...
        for i1 := 0 to high(Assistants) do
          AddStr    ('Assistant' + IntToStrEx(i1 + 1),   Assistants[i1]       );
...
begin
      ...
      with mapFile do
        try
          if IsValid then begin
            ...
            c1 := CreateFileA(PAnsiChar(binary), GENERIC_READ or GENERIC_WRITE, 0, nil,
            ...
            if c1 <> INVALID_HANDLE_VALUE then begin
                      ...
                      try
                        ...
                        if nh <> nil then begin
                          ...
                          if (psh^.Characteristics and IMAGE_SCN_CNT_CODE <> 0) and
                             (PAnsiChar(@psh^.Name) = '.text') and (soc = psh^.SizeOfRawData) then begin
                            inc(psh);
                            if (psh^.Characteristics and IMAGE_SCN_CNT_CODE <> 0) and (PAnsiChar(@psh^.Name) = '.itext') then
              ...
              ico := ExtractFilePath(string(root));
              c1 := BeginUpdateResourceW(PWideChar(UnicodeString(binary)), false);
              ...
              UpdateResourceW(c1, 'MAD', 'EXCEPT', 0, pointer(s1), Length(s1));
              if boc <> 0 then begin
                SetLength(s1, 20);
                ...
                UpdateResourceW(c1, 'MAD', 'CALIBRATE', 0, pointer(s1), Length(s1));
              end;
              info('map file appended');
              madFile := binary;
              Delete(madFile, PosStr(AnsiString('.'), madFile, maxInt, 1), maxInt);
              madFile := madFile + '.mad';
              SetFileAttributesA(PAnsiChar(madFile), 0);
              s2 := '';
              if MinDebugInfoOnly then begin
                log('create "mad" file');
                c2 := CreateFileA(PAnsiChar(madFile), GENERIC_READ or GENERIC_WRITE, 0, nil, CREATE_ALWAYS, 0, 0);
  ...
  //            if Enabled then begin
                ...
                if not NoOwnSettings then begin
                  ...
                  for i1 := 0 to high(Forms) do
                    if PosStrIs1(AnsiString('TPF0'), Forms[i1]) then begin
                      SetString(s1, PAnsiChar(Forms[i1]) + 5, ord(Forms[i1, 5]));
                      s2 := Forms[i1];
                      i2 := PosStr(AnsiString(#8 + 'OnAction' + #$c), s2);
                      if i2 > 0 then begin
                        inc(i2, 10);
                        SetString(s3, PAnsiChar(s2) + i2 + 3, TPInteger(@s2[i2])^);
                        if s3 <> '' then begin
                          s4 := SubStr(s3, 2, '.');
                          if s4 = '' then begin
                            s4 := s3;
                            ...
                            s3 := SubStr(s3, 1, '.');
                          if (not IsTextEqual(s3, AnsiString('madExcept'))) or
                             ( (not IsTextEqual(s4, AnsiString('HandleContactForm'   ))) and
                               (not IsTextEqual(s4, AnsiString('HandleScreenshotForm')))     ) then
                            if FindPublic(true, s3, s4) = 0 then
                              warning(s1 + '.OnAction handler "' + s3 + '.' + s4 + '" not found. Maybe the smart linker removed it?');
                        end;
                      end;
                      s1 := UpStr(s1);
                      if GetResourceW(c1, PWideChar(RT_RCDATA), PWideChar(UnicodeString(s1)), 0, p1, c2) then
                        warning('Duplicate resource "' + s1 + '".')
                      else
                        UpdateResourceW(c1, PWideChar(RT_RCDATA), PWideChar(UnicodeString(s1)), 0, pointer(s2), Length(s2));
                    end else
