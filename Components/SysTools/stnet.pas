...
begin
  ...
  try
    ...
    while MoreData do begin                                                          {!!.02}
      ErrorD := StNetServerDiskEnum(FServer, 0, Buffer, DWord(-1), EntriesRead,
                                    TotalEntries, NextIndex);                        {!!.02}
      if ((ErrorD = NERR_SUCCESS) or (ErrorD = ERROR_MORE_DATA)) then begin          {!!.02}
        try
          for Index := 0 to EntriesRead-1 do
            NewList.Add((TDriveArray(Buffer^)[Index]));

          MoreData := (ErrorD = ERROR_MORE_DATA);                                    {!!.02}
        finally
          StNetApiBufferFree(Buffer);
        end;
      end else begin
...
begin
  ...
  try
    while MoreData do begin                                                          {!!.02}
      ErrorD := StNetQueryDisplayInformation(FServer, 1, NextIndex, DWord(-1),       {!!.02}
                                             DWord(-1), EntriesRead, Buffer);        {!!.02}
      if ((ErrorD = NERR_SUCCESS) or (ErrorD = ERROR_MORE_DATA)) then begin          {!!.02}
        try
          for Index := 0 to EntriesRead-1 do begin
            ...
            NextIndex := TNetDisplayUserArray(Buffer^)[Index].usri1_next_index;      {!!.02}
          end;
          MoreData := (ErrorD = ERROR_MORE_DATA);                                    {!!.02}
        finally
          StNetApiBufferFree(Buffer);
...
begin
  ...
  try
    ...
    while MoreData do begin                                                          {!!.02}
      ErrorD := StNetQueryDisplayInformation(FServer, 3, NextIndex, DWord(-1),       {!!.02}
                                             DWord(-1), EntriesRead, Buffer);        {!!.02}
      if ((ErrorD = NERR_SUCCESS) or (ErrorD = ERROR_MORE_DATA)) then begin          {!!.02}
        try
          for Index := 0 to EntriesRead-1 do begin
            ...
            NewList.AddObject(NewItem.Name, NewItem);
            NextIndex := TNetDisplayGroupArray(Buffer^)[Index].grpi3_next_index;     {!!.02}
          end;
          MoreData := (ErrorD = ERROR_MORE_DATA);                                    {!!.02}
        finally
          StNetApiBufferFree(Buffer);
...
begin
  ...
  if Result = nil then begin
    ...
    APasswordLen := (Length(APassword)+1) * 2;                         {!!.01}
    StNetApiBufferAllocate(APasswordLen, Pointer(Data.usri1_password));{!!.01}
    try
      StringToWideChar(AName, Data.usri1_name, ANameLen);
      StringToWideChar(APassword, Data.usri1_password, APasswordLen);  {!!.01}

      Data.usri1_priv := USER_PRIV_USER;
      Data.usri1_flags := UF_SCRIPT;                                   {!!.01}
      if AGlobal then
        SetLongFlag(LongInt(Data.usri1_flags),                         {!!.01}
          UF_NORMAL_ACCOUNT)                                           {!!.01}
      else
        SetLongFlag(LongInt(Data.usri1_flags),                         {!!.01}
          UF_TEMP_DUPLICATE_ACCOUNT);                                  {!!.01}
      ...
      if ErrorD = NERR_SUCCESS then begin
        ...
        RaiseStWin32Error(EStNetException, ErrorD);                    {!!.01}
      end;
    finally
      StNetApiBufferFree(Data.usri1_password);                         {!!.01}
