...
{$IFDEF WIN16}
      asm
        push ss
        pop es
        lea si,UB1+MantissaDigits
        lea di,UB2+MantissaDigits
        mov cx,MantissaDigits
@1:     mov al,es:[si]
        sub al,es:[di]
        jnz @2
...
{$IFDEF WIN16}
  asm
    ...
    mov ds,ax
    mov es,ax
    mov cx,SigDigits             {number of digits in result}
    lea di,UB1+SigDigits         {di points to result}
    lea si,TB+2*MantissaDigits+1 {si points to numerator}
    mov dh,byte ptr DivDigits    {keep DivDigits in dh}
    std

@1: push cx                      {save result counter}
    push di                      {save result position}
    mov bx,si                    {save numerator position}
    xor dl,dl                    {dl = number of times divisor fits into numerator}

@2: cmp byte ptr [si+1],0        {check for remainder in numerator}
    jnz @4                       {divisor guaranteed to fit again}
    xor cx,cx
    mov cl,dh                    {cx = number of divisor digits}
    lea di,UB2+MantissaDigits    {last digit of divisor}
...
@4: inc dl                       {increment number of times divisor fits}
    mov di,bx                    {restore numerator position to di}
    xor cx,cx
    mov cl,dh                    {number of divisor digits}
    lea si,UB2+MantissaDigits    {last digit of divisor}
    dec cx
    sub si,cx                    {first significant digit of divisor}
    sub di,cx                    {first active digit of numerator}
    inc cx
    clc                          {no carry to start}
    cld                          {go forward again}

@5: lodsb                        {get digit from divisor}
    mov ah,al
...
@6: std                          {backwards again}
    mov si,bx                    {restore numerator position}
    jmp @2

@7: mov si,bx                    {restore numerator position}
...
{$IFDEF WIN16}
    asm
      ...
      mov ds,ax
      mov es,ax
      lea bx,UB1 {multiplier}
      lea di,TB  {result}
      mov cx,MantissaDigits
      cld

@1:   inc bx      {next multiplier digit}
      inc di      {next output digit}
      mov al,[bx] {get next multiplier digit}
      or al,al    {if zero, nothing to do}
      jz @3

      push cx     {save digit counter}
      mov dl,al   {save multiplier}
      lea si,UB2+1 {multiplicand}
      mov cx,MantissaDigits
      xor dh,dh

@2:   lodsb       {next multiplicand digit}
      mul dl      {multiply by multiplier, overflow in ah}
      aam
      add al,[di] {add previous result}
      aaa
      add al,dh   {add previous overflow}
      aaa
      stosb       {store temporary result}
      mov dh,ah   {save overflow for next time}
      loop @2
      mov [di],dh {save last overflow in next digit}
      sub di,MantissaDigits {reset output offset for next multiplier}
      pop cx

@3:   loop @1     {next multiplier digit}
      pop ds
    end;
{$ENDIF}
{$ELSE}
