...
type
  ...
  TStCode128CodeSubset = (csCodeA, csCodeB);
  ...
  protected {private}
    ...
    FBarNarrowToWideRatio : Integer;                                     {!!.02}
    ...
    procedure SetBarNarrowToWideRatio(Value: Integer);                   {!!.02}
    ...
    function GetDigits(const Characters : string) : Integer;
  ...
  published
    ...
    property BarNarrowToWideRatio : Integer                              {!!.02}
      read FBarNarrowToWideRatio                                         {!!.02}
      write SetBarNarrowToWideRatio                                      {!!.02}
      default bcDefNarrowToWideRatio;                                    {!!.02}
  ...
  begin
    for K := 1 to Length(S) do begin
      case S[K] of
        ...
        '1' : if Odd(K) then
                bcBarInfo.Add(FBarNarrowToWideRatio, [bkBar])            {!!.02}
              else
                bcBarInfo.Add(FBarNarrowToWideRatio, [bkSpace]);         {!!.02}
      end;
    end;
  end;
      ...
      begin
        ...
        while I < bcDigitCount do begin
          ...
          for J := 1 to 5 do begin
            if C1[J] = '1' then
              bcBarInfo.Add(FBarNarrowToWideRatio, [bkBar]) {wide bar}   {!!.02}
            else
              bcBarInfo.Add(1, [bkBar]);   {narrow bar}
            if C2[J] = '1' then
              bcBarInfo.Add(FBarNarrowToWideRatio, [bkSpace]){wide space}{!!.02}
            else
        ...
        bcBarInfo.Add(FBarNarrowToWideRatio,                             {!!.02}
          [bkGuard, bkBar]); {double-width bar}                          {!!.02}
        bcBarInfo.Add(1, [bkGuard, bkSpace]);
        bcBarInfo.Add(1, [bkGuard, bkBar]);
      end;
    bcCodabar :
...
begin
  ...
  FBarColor := clBlack;
  FBarToSpaceRatio := 1;
  FBarNarrowToWideRatio := bcDefNarrowToWideRatio;                       {!!.02}
  FBarWidth := 12;
  FShowCode := True;
...
function TStBarCode.GetDigits(const Characters : string) : Integer;
var
  I, J : Integer;
  S    : string[2];
begin
      ...
      begin
        ...
        for I := 1 to Result do begin
          case Characters[I] of
            ...
            '.'      : bcDigits[I] := 14;
            '/'      : bcDigits[I] := 15;
            '0'..'9' : bcDigits[I] := 16 + Ord(Characters[I])-Ord('0');
            ':'      : bcDigits[I] := 26;
            ';'      : bcDigits[I] := 27;
            '<'      : bcDigits[I] := 28;
            '='      : bcDigits[I] := 29;
            ...
            '?'      : bcDigits[I] := 31;
            '@'      : bcDigits[I] := 32;
            'A'..'Z' : bcDigits[I] := 33 + Ord(Characters[I])-Ord('A');
            '['      : bcDigits[I] := 59;
            '\'      : bcDigits[I] := 60;
            ']'      : bcDigits[I] := 61;
            '^'      : bcDigits[I] := 62;
            '_'      : bcDigits[I] := 63;
            #0, #31  : bcDigits[I] := 64 + Ord(Characters[I]);  {control characters}
            '`'      : bcDigits[I] := 64;
            'a'..'z' : bcDigits[I] := 65 + Ord(Characters[I])-Ord('a');
            '{'      : bcDigits[I] := 91;
            '|'      : bcDigits[I] := 92;
            '}'      : bcDigits[I] := 93;
            '~'      : bcDigits[I] := 94;
            #130     : bcDigits[I] := 98;  {é} {rest are manufactured characters}
            #131     : bcDigits[I] := 97;  {â}
            #132     : bcDigits[I] := 96;  {ä}
            #133     : bcDigits[I] := 98;  {à}
            #134     : bcDigits[I] := 100; {å}
            #135     : bcDigits[I] := 99;  {ç}
            #136     : bcDigits[I] := 103; {ê}
            #137     : bcDigits[I] := 104; {ë}
            #138     : bcDigits[I] := 105; {è}
            #139     : bcDigits[I] := 106; {ï}
          else
            RaiseStError(EStBarCodeError, stscInvalidCharacter);
          end;
        end;
...
procedure TStBarCode.SetBarNarrowToWideRatio(Value : Integer);           {!!.02}
begin                                                                    {!!.02}
  if Value <> FBarNarrowToWideRatio then begin                           {!!.02}
    FBarNarrowToWideRatio := Value;                                      {!!.02}
    CalcBarCode;                                                         {!!.02}
    Invalidate;                                                          {!!.02}
  end;                                                                   {!!.02}
end;                                                                     {!!.02}
