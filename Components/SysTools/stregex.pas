...
begin
  ...
  if (aNode^.StrPtr <> nil) then begin                                 {!!.01}
    New(Result^.StrPtr);                                               {!!.01}
    Result^.StrPtr^     := aNode^.StrPtr^;                             {!!.01}
  end else                                                             {!!.01}
    Result^.StrPtr      := nil;                                        {!!.01}
...
{$ENDIF}
  ...
  if (Len = 0) then
    ...
    for I := 0 to pred(SL.Count) do begin
      {$IFDEF MSWINDOWS}
      Move(SL[I][1], PStr[J], Length(TrimL(SL[I])));
      Inc(J, Length(TrimL(SL[I])));
      {$ENDIF}
      {$IFDEF WIN16}
      S := SL[I];
      Move(S[1], PStr[J], Length(TrimS(S)));
      Inc(J, Length(TrimS(S)));
      {$ENDIF}
    end;
    Result := True;
  end;
      ...
      if ((FReplacePatPtr <> nil) and (not CheckOnly)) then begin
        ...
        if Result then begin
          ...
          if (not (ooCountOnly in FOutputOptions)) then begin
            ...
            Tmp[StrLen(Tmp)-2] := #0;
            FOutTextStream.WriteLineZ(Tmp);                             {!!!.03}
          end;
...
{        if not(SubLineMatchOne(Buf, Flags, TagOn,
                                LocTag, I, PatRec)) then}              {!!.01}
        if not(SubLineMatchOne(Buf, Flags, TagOn,                      {!!.01}
                               I, LocTag, PatRec)) then                {!!.01}
          Done := True;
      end;
      {i points to the location that caused a non-match}
      {match rest of pattern against rest of input}
