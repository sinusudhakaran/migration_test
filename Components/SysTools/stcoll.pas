...
begin
  with N do begin
    ...
    if (pdCount = 0) and (colPageList.Count > 1) then begin
      ...
      if N = colCachePage then begin                           {!!.02}
        colCachePage := TPageDescriptor(colPageList.Head);
        if N = colCachePage then                               {!!.02}
          colCachePage := TPageDescriptor(N.FNext);            {!!.02}
      end;                                                     {!!.02}
      colPageList.Delete(N);
    end;
  end;
...
begin
  ...
  (LongInt(PageElements)*SizeOf(Pointer) > MaxBlockSize)} then
    RaiseContainerError(stscBadSize);                                  {!!.01}
