...
{$IFNDEF VERSION3}
  !! Error: This unit cannot be compiled with Delphi 1 or 2, or CBuilder 1
{$ENDIF}
...
type
  ...
  TUSER_INFO_1 = record
    usri1_name            : LPWSTR;
    usri1_password        : LPWSTR{LPSTR};                             {!!.01}
...
type
  ...
  TNetShareEnum                  = function(ServerName: LPWSTR; Level: DWORD; var Buffer: Pointer; PrefMaxLen: DWord; var EntriesRead, TotalEntries: DWord; var ResumeHandle: DWord): NET_API_STATUS; stdcall; {!!.02}
...
type
  ...
  TNetServerTransportAddEx       = function(ServerName: LPSTR; Level: DWord; Buffer: Pointer): NET_API_STATUS; stdcall;
...
type
  ...
  TLookupAccountNameA            = function(SystemName, AccountName: PAnsiChar; Sid: Pointer; var cbSid: DWord; DomainName: PAnsiChar; var cbDomainName: DWord; var peUse: DWord): Boolean; stdcall;
  TLookupAccountSidA             = function(SystemName: PAnsiChar; Sid: PSID; AccountName: PChar; var AccountLen: DWord; DomainName: PChar; var DomainLen; var peUse: DWord): Boolean; stdcall;
...
{ ============================================================================}
  ...
  function StNetLocalGroupEnum(ServerName: string; Level: DWord; var Buffer: Pointer; PrefMaxLen: DWord; var EntriesRead, TotalEntries: DWord; var ResumeHandle: DWord): NET_API_STATUS;   {!!.02}
  ...
  function StNetShareEnum(ServerName: string; Level: DWORD; var Buffer: Pointer; PrefMaxLen: DWord; var EntriesRead, TotalEntries: DWord; var ResumeHandle: DWord): NET_API_STATUS; {!!.02}
  ...
  function StNetServerTransportAddEx(ServerName: string; Level: DWord; Buffer: Pointer): NET_API_STATUS;
...
var
  ...
  _LookupAccountNameA            : TLookupAccountNameA            = nil;
  _LookupAccountSidA             : TLookupAccountSidA             = nil;
...
function StNetLocalGroupEnum(ServerName: string; Level: DWord;
                             var Buffer: Pointer; PrefMaxLen: DWord;
                             var EntriesRead, TotalEntries: DWord;
                             var ResumeHandle: DWord): NET_API_STATUS;               {!!.02}
var
  S : TLMWideChar;
begin
  Result := 0;
    ...
    if (@_NetLocalGroupEnum <> nil) then begin
      ...
      try
        ...
        Result := _NetLocalGroupEnum(S.Value, Level, Buffer, PrefMaxLen,
                                     EntriesRead, TotalEntries, ResumeHandle);       {!!.02}
      finally
        FreeMem(S.Value, S.Length);
      end;
    end else
...
begin
  ...
  if IsNetApi32 then begin
    ...
    if (@_NetServerTransportAddEx <> nil) then begin
      try
        Result := _NetServerTransportAddEx(PAnsiChar(ServerName), Level, Buffer);
      finally
      end;
    end else
      RaiseStWin32Error(EStNetException, GetLastError);
...
begin
  Result := False;
  if IsAdvApi32 then begin
    if (@_LookupAccountNameA = nil) then
      @_LookupAccountNameA := GetProcAddress(ADVAPI32, 'LookupAccountNameA');

    if (@_LookupAccountNameA <> nil) then begin
      { This just sets up the buffer sizes }
      BufLen := 0;
      Buffer := nil;
      Result := _LookupAccountNameA(PAnsiChar(SystemName), PAnsiChar(AccountName),
      ...
      if ErrorD = ERROR_INSUFFICIENT_BUFFER then begin
        ...
        try
          try
            Result := _LookupAccountNameA(PAnsiChar(SystemName),
                                          PAnsiChar(AccountName), Sid, SidLength,
                                          Buffer, BufLen, peUse);
            if Result then
              DomainName := StrPas(Buffer)
            else
...
begin
  Result := False;
  if IsAdvApi32 then begin
    if (@_LookupAccountSidA = nil) then
      @_LookupAccountSidA := GetProcAddress(ADVAPI32, 'LookupAccountSidA');

    if (@_LookupAccountSidA <> nil) then begin
      { This just sets up the buffer sizes }
      ALen := 0;
      DLen := 0;
      ...
      A := nil;
      D := nil;
      Result := _LookupAccountNameA(PAnsiChar(SystemName), Sid, nil, ALen, nil,
                                    DLen, peUse);
      ErrorD := GetLastError;
      if ErrorD = ERROR_INSUFFICIENT_BUFFER then begin
        ...
        try
          Result := _LookupAccountNameA(PAnsiChar(SystemName), Sid, A, ALen, D,
                                        DLen, peUse);
          if Result then begin
            AccountName := StrPas(A);
            DomainName  := StrPas(D);
          end;
        finally
