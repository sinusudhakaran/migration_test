{*********************************************************}
{* SysTools: StShlCtl.pas 3.03                           *}
{* Copyright (c) TurboPower Software Co 1996, 2001       *}
{* All rights reserved.                                  *}
{*********************************************************}
{* SysTools: Explorer Work-Alike components              *}
...
uses
  ...
  StFileOp, Registry, StDict, StdCtrls, Clipbrd, 
  StRegEx, {!!.01} ExtCtrls,
...
type
  ...
  TStCustomShellController = class;                                    {!!.02}

  TStShellItem = class(TObject)
  protected {private}
    { property variables }
    FColText        : TStringList;
    FController     : TStCustomShellController;                        {!!.02}
    FDate           : TDateTime;
    FSize           : Integer;
    FDisplayName    : string;
    FFileAttributes : DWORD;
    FFileAttributesStr : string;                                       {!!.02}
    FIconIndex      : Integer;
    FIsDesktop      : Boolean;
    FIsFile         : Boolean;
    ...
    function GetDate : TDateTime;                                      {!!.02}
    function GetFileAttributes : DWORD;                                {!!.02}
    function GetFileAttributesStr : string;                            {!!.02}
    function GetHasRemovableMedia : Boolean;
    function GetHasSubFolder : Boolean;
    function GetIconIndex : Integer;                                   {!!.02}
    ...
    function GetOpenIconIndex : Integer;                               {!!.02}
    function GetOverlayIconIndex : Integer;                            {!!.02}
    function GetLargeIcon : TIcon;
    function GetSize : Integer;                                        {!!.02}
    function GetSmallIcon : TIcon;
    function GetSmallOpenIcon : TIcon;

    { internal methods }
    procedure GetItemDetails;                                          {!!.02}
  public
    { properties }
    ...
    {!!.02 - Added AController parameter to constructors. }
    ...
    constructor CreateFromPath(Path : string;
      AController : TStCustomShellController);
    destructor  Destroy; override;
    function GetFolderSize(Recursive : Boolean;                        {!!.02}
      IncludeHidden : Boolean) : Cardinal;                             {!!.02}
  end;
  ...
  protected { private }
    ...
    { internal methods }
    procedure ClearTree;
    {!!.02 Some methods made virtual per customer requests. }
  ...
  public
    ...
    procedure SelectFolder(Path : string);                             {!!.02}
    procedure SelectSpecialFolder(Folder : TStSpecialRootFolder);      {!!.02}
    constructor Create(AOwner : TComponent); override;
    destructor Destroy; override;
  end;
  ...
  protected
    ...
    FSelectedItems     : TStShellItemList;                             {!!.02}
    FSpecialRootFolder : TStSpecialRootFolder;
    FShellVersion      : Double;
    FTreeView          : TStCustomShellTreeView;
    ...
    property SelectedItems : TStShellItemList                          {!!.02}
      read FSelectedItems;                                             {!!.02}
  ...
  public
    ...
    function  ShowPropertySheet : Boolean;
    procedure SortColumn(Index : Integer; SortDir : TStSortDirection); {!!.03}
  ...
  protected {private}
    ...
    procedure SelectItem(const SI : TStShellItem);                     {!!.03}
...
begin
  ...
  if Win32Platform = VER_PLATFORM_WIN32_NT then begin
    ...
    HFile := CreateFile(TempFile, GENERIC_WRITE, 0,                    {!!.02}
      nil, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, 0);
    CloseHandle(HFile);                                                {!!.02}
...
begin
  ...
  if Value = sdToggle then                                             {!!.03}
    RaiseStError(EStInvalidSortDir, stscInvalidSortDir);               {!!.03}
...
begin
  ...
  FIconIndex := -1;
  FSize := -1;
  if AController = nil then begin                                      {!!.02}
    FController := TStCustomShellController.Create(nil);               {!!.02}
    OwnController := True;                                             {!!.02}
  end else                                                             {!!.02}
    FController := AController;                                        {!!.02}
end;
...
begin
  ...
  { Defer getting the item info until actually needed. }               {!!.02}
  { Controller.GetItemInfo(FPidl, FIconIndex, FOpenIconIndex, }        {!!.02}
  {   FOverlayIconIndex, FDisplayName); }                              {!!.02}
  { Fix up display names for items on remote machines. }
  ...
  FIsFileFolder := FIsFileSystem and
    ((Attr and SFGAO_FOLDER) = SFGAO_FOLDER);
  FIsFile := (FIsFileSystem and not FIsFileFolder);                    {!!.02}
...
begin
  ...
  FIconIndex := -1;
  FSize := -1;
  SHGetDesktopFolder(DesktopFolder);                                   {!!.02}
...
begin
  ...
  GetParentFolder(FPidl, FParentFolder);
  FController       := AValue.FController;                             {!!.02}
...
procedure TStShellItem.GetItemDetails;                                 {!!.02}
var
  S    : string;
  Res  : DWORD;
  FD   : TWin32FindData;
...
function TStShellItem.GetFolderSize(                                   {!!.02}
...
function TStShellItem.GetDate : TDateTime;                             {!!.02}
begin
  if FSize = -1 then
    GetItemDetails;
  Result := FDate;
...
function TStShellItem.GetFileAttributes : DWORD;                       {!!.02}
begin
  if FSize = -1 then
    GetItemDetails;
...
function TStShellItem.GetFileAttributesStr : string;                   {!!.02}
begin
  if FSize = -1 then
    GetItemDetails;
...
function TStShellItem.GetIconIndex : Integer;                          {!!.02}
var
  S : string;
begin
...
function TStShellItem.GetOpenIconIndex : Integer;                      {!!.02}
var
  S : string;
begin
  if FIconIndex = -1 then
...
function TStShellItem.GetOverlayIconIndex : Integer;                   {!!.02}
var
  S : string;
begin
  if FIconIndex = -1 then
...
function TStShellItem.GetSize : Integer;                               {!!.02}
begin
  if FSize = -1 then
    GetItemDetails;
  Result := FSize;
...
begin
  ...
  if RecreatingWnd then begin                                          {!!.01}
    Node := Items[0];                                                  {!!.03}
    while Node <> nil do begin                                         {!!.03}
      SI := Folders[Integer(Node.Data)];                               {!!.03}
      if SI <> nil then                                                {!!.03}
        if SI.HasSubFolder then                                        {!!.03}
            Node.HasChildren := True;                                  {!!.03}
      Node := Node.GetNext;                                            {!!.03}
    end;                                                               {!!.03}
    Exit;                                                              {!!.01}
  end;
...
begin
  if not (csLoading in ComponentState)                                 {!!.02}
      or (csDesigning in ComponentState) then                          {!!.02}
...
begin
  ...
  if not (csLoading in ComponentState) and (FRootFolder <> '') then begin
    ...
    if (FSpecialStartInFolder <> sfNone) or (FStartInFolder <> '') then {!!.02}
      StartFolderSet := False;                                          {!!.02}
    ClearTree;
    FillTree;
  end;
end;
...
begin
  with Message do begin
    ...
    if (Msg = WM_PAINT) and (not StartFolderSet)
        and (not (csDesigning in ComponentState)) then begin           {!!.01}
...
procedure TStCustomShellTreeView.FindAndSelectNode(APidl : PItemIDList); {!!.02}
var
  I     : Integer;
  Node  : TTreeNode;
  List  : TList;
...
begin
  ...
  if Node <> nil then                                                  {!!.03}
    ...
    SendMessage(Handle, WM_SETREDRAW, 1, 0);                           {!!.03}
    RaiseStError(EStInvalidFolder, stscInvalidFolder);                 {!!.03}
  end;                                                                 {!!.03}
  SendMessage(Handle, WM_SETREDRAW, 1, 0);
  Selected := Node;
  FSelectedFolder := Folders[Integer(Node.Data)];                      {!!.03}
end;

procedure TStCustomShellTreeView.SelectFolder(Path : string);          {!!.02}
var
  Pidl  : PItemIDList;
begin
  ...
  ILFree(Pidl);
  FStartInFolder := Path;                                              {!!.03}

end;

procedure TStCustomShellTreeView.SelectSpecialFolder(                  {!!.02}
...
//    if Selected = Node then
    if (Node <> nil) and (Selected = Node) then  // !!! Test
...
begin
  ...
  if FSpecialRootFolder <> sfNone then begin
    ...
    if Pidl = nil then                                                 {!!.02}
      RaiseStError(EStShellError, stscShellVersionError);              {!!.02}
...
var
  ...
  SF            : TStShellFolder;
begin
  if ANode = nil then                                                  {!!.04}
    ANode := Items[0];                                                 {!!.04}
  {if ANode <> nil then begin}                                         {!!.04}
...
begin
  ...
  FSelectedItems := TStShellItemList.Create;                           {!!.02}
...
begin
  ...
  if not (csLoading in ComponentState) then begin
    ...
    if Assigned(FTreeView) and                                         {!!.01}
      ...
      if FTreeView.Folders.Count <> 0 then                             {!!.02}
...
begin
  ...
  FillList(TStShellFolder(SI));
  if Assigned(FTreeView) then                                          {!!.02}
    FTreeView.ListViewFolderChange(SI, False);                         {!!.02}
end;
  ...
  begin
    ...
    for I := 0 to Pred(Columns.Count) do begin
      ...
      if SI.IsFile and (ColText = '') and
         (Columns.Items[I].Caption = stTypeCol) then begin             {!!.02}
...
begin
  if (csDesigning in ComponentState) then                              {!!.02}
    Exit;                                                              {!!.02}
  SI.ColText.Clear;
...
begin
  if (Button = mbRight) and (Selected <> nil)
      and (loShellMenu in FOptions) then begin                         {!!.03}
    Popup := PopupMenu;
    PopupMenu := nil;
    Pt.x := X;
    Pt.y := Y;
...
var
  ...
  Item      : TListItem;
  {$ENDIF}
  FocusItemPidl : PItemIDList;                                             {!!.02}
  FocusItemStr  : string;
begin
  { Save the focused item so we can focus it later. }                  {!!.02}
  if ItemFocused <> nil then begin                                     {!!.02}
    SI := ShellItems[Integer(ItemFocused.Data)];                       {!!.02}
    if SI.IsFileSystem then                                            {!!.02}
      FocusItemStr := SI.Path;                                         {!!.02}
    FocusItemPidl := SI.Pidl;                                          {!!.02}
  end else                                                             {!!.02}
    FocusItemPidl := nil;                                              {!!.02}
  inherited;
  if ClickedColumn = Column.Index then begin
    ...
    if SelectedItems.Count > 0 then begin                              {!!.02}
      for I := 0 to Pred(Items.Count) do begin                         {!!.02}
        if FocusItemPidl <> nil then begin                             {!!.02}
          if FocusItemStr <> '' then begin                             {!!.02}
            if FocusItemStr = ShellItems[Integer(Items[I].Data)].Path then {!!.02}
              ItemFocused := Items[I];                                 {!!.02}
          end else                                                     {!!.02}
            if ILIsEqual(                                              {!!.02}
              ShellItems[Integer(Items[I].Data)].Pidl, FocusItemPidl) then {!!.02}
              ItemFocused := Items[I];                                 {!!.02}
        end;                                                           {!!.02}
        Items[I].Selected := False;                                    {!!.02}
        for J := 0 to Pred(SelectedItems.Count) do                     {!!.02}
          if SelectedItems[J].IsFileSystem then begin                  {!!.02}
            if SelectedItems[J].Path =                                 {!!.02}
                 ShellItems[Integer(Items[I].Data)].Path then begin    {!!.02}
              Items[I].Selected := True;                               {!!.02}
              Break;                                                   {!!.02}
            end;                                                       {!!.02}
          end else                                                     {!!.02}
            if ILIsEqual(ShellItems[Integer(Items[I].Data)].Pidl,      {!!.02}
                 SelectedItems[J].Pidl) then begin                     {!!.02}
              Items[I].Selected := True;                               {!!.02}
              Break;                                                   {!!.02}
          end;                                                         {!!.02}
      end;                                                             {!!.02}
    ...
    R := ClientRect;                                                   {!!.02}
    InvalidateRect(Handle, @R, False);                                 {!!.02}
    Update;                                                            {!!.02}
    Exit;
    {$ENDIF}
  end else
  ...
  for I := 0 to Pred(ShellItems.Count) do begin
    ...
    Item.Selected := False;                                            {!!.02}
    { Restore the selected items. }                                    {!!.02}
    if SelectedItems.Count > 0 then begin                              {!!.02}
      if FocusItemPidl <> nil then begin                               {!!.02}
        if FocusItemStr <> '' then begin                               {!!.02}
          if FocusItemStr = SI.Path then                               {!!.02}
            Item.Focused := True;                                      {!!.02}
        end else if ILIsEqual(SI.Pidl, FocusItemPidl) then             {!!.02}
          Item.Focused := True;                                        {!!.02}
      end;                                                             {!!.02}
      for J := 0 to Pred(SelectedItems.Count) do                       {!!.02}
        if SelectedItems[J].IsFileSystem then begin                    {!!.02}
          if SelectedItems[J].Path = SI.Path then begin                {!!.02}
            Item.Selected := True;                                     {!!.02}
            Break;                                                     {!!.02}
          end;                                                         {!!.02}
        end else if ILIsEqual(SI.Pidl, SelectedItems[J].Pidl) then begin {!!.02}
          Item.Selected := True;                                       {!!.02}
          Break;                                                       {!!.02}
        end;                                                           {!!.02}
    end;                                                               {!!.02}
...
begin
  inherited;
  if Selected <> nil then begin
    { !!.03 Moved the item selection code up from bottom of method. }
    SelectedItems.Clear;                                               {!!.02}
    for I := 0 to Pred(Items.Count) do                                 {!!.02}
      if Items[I].Selected then begin                                  {!!.02}
        SI := TStShellItem.Create(Controller);                         {!!.02}
        SI.Assign(ShellItems[Integer(Items[I].Data)]);                 {!!.02}
        SelectedItems.FList.Add(SI);                                   {!!.02}
        R := Items[I].DisplayRect(drBounds);                           {!!.02}
        InvalidateRect(Handle, @R, False);                             {!!.02}
        Update;                                                        {!!.02}
      end;                                                             {!!.02}
...
begin
  if Assigned(FTreeView) then
    if not FTreeView.StartFolderSet then
      Exit;
  if SelectedItems.Count > 0 then                                      {!!.02}
    SelectedItems.Clear;                                               {!!.02}
  OwnItem := False;
  HaveDetails := False;
  HaveColumns := False;
  ...
  {$ENDIF}
  if Assigned(Items[0]) then                                           {!!.02}
    Items[0].MakeVisible(False);                                       {!!.02}
  Perform(WM_SETREDRAW, 1, 0);
  ...
  if Assigned(FComboBox) then begin
    ...
    if not (csDesigning in ComponentState) then begin                  {!!.03}
      if FComboBox.ItemIndex = -1 then                                 {!!.02}
        FComboBox.ListViewFolderChange(Folder, False);                 {!!.02}
      FComboBox.SelectItem(Folder);                                    {!!.03}
      { !!Removed in 3.03. }
      { FComboBox.ItemIndex :=
...
begin
  ...
  if not (csLoading in ComponentState) and (FRootFolder <> '') then begin
    ...
    if (FViewStyle = vsReport)                                         {!!.01}
      ...
      Exit;                                                            {!!.01}
    FillList(nil);
    if Assigned(FTreeView) and not (csLoading in ComponentState) then  {!!.02}
      FTreeView.ListViewFolderChange(Folder, False);                   {!!.02}
    if Assigned(FComboBox) and not (csDesigning in ComponentState) then{!!.03}
      FComboBox.ListViewFolderChange(Folder, False);                   {!!.02}
  end;
end;
...
begin
  if SelectedItem = nil then
    ...
    FO := TStFileOperation.Create(Self);
    if SelCount = 1 then                                               {!!.02}
      FO.SourceFiles.Add(SelectedItem.FPath)
    else begin                                                         {!!.02}
      Item := Selected;                                                {!!.02}
      FO.SourceFiles.Add(ShellItems[Integer(Item.Data)].FPath);        {!!.02}
      for I := 1 to Pred(SelCount) do begin                            {!!.02}
        Item := GetNextItem(Item, sdAll, [isSelected]);                {!!.02}
        FO.SourceFiles.Add(ShellItems[Integer(Item.Data)].FPath);      {!!.02}
      end;                                                             {!!.02}
    end;                                                               {!!.02}
    FO.Operation := fopDelete;
...
procedure TStCustomShellListView.SortColumn(                           {!!.03}
  Index : Integer; SortDir : TStSortDirection);                        {!!.03}
begin                                                                  {!!.03}
  if ViewStyle = vsReport then begin                                   {!!.03}
    if SortDir <> sdToggle then                                        {!!.03}
      Ascending := (SortDir = sdDescending);                           {!!.03}
    ColClick(Columns[Index]);                                          {!!.03}
  end;                                                                 {!!.03}
end;                                                                   {!!.03}
...
begin
  ...
  for I := 0 to Pred(FullList.Count) do begin
    ...
    if not FullList[I].IsFile then begin                               {!!.03}
...
begin
  ...
  if {not (csLoading in ComponentState) and} (FWatchFolder <> '') then begin {!!.03}
    { Can't have a SpecialRootFolder if RootFolder is assigned. }
...
begin
  ...
  if FSpecialWatchFolder <> Value then begin
    FSpecialWatchFolder   := Value;
    if (FSpecialWatchFolder <> sfNone) {and not (csLoading in ComponentState)} then begin {!!.03}
      { Can't have a RootFolder if SpecialRootFolder is assigned. }
      WatchFolder := '';
...
begin
  ...
  {!!.02 Added code to center the icon vertically. }
  if SI.IsShared then
...
begin
  ...
  ItemHeight := Controller.SmallFolderImages.Height;                   {!!.02}
  C := TCanvas.Create;                                                 {!!.02}
  C.Handle := GetDC(0);                                                {!!.02}
  C.Font.Assign(Font);                                                 {!!.02}
  I := C.TextHeight('Aj');                                             {!!.02}
  if ItemHeight < I then                                               {!!.02}
    ItemHeight := I;                                                   {!!.02}
  C.Free;                                                              {!!.02}
  { Clear the old items. }
  ShellItems.Clear;
...
begin
  ...
  for I := Pred(ShellItems.FList.Count) downto 0 do begin
    ...
    { SI.DisplayName changed to SI.Path in 3.03. }
    if (SI.Path = SF.Path) and                                         {!!.03}
...
procedure TStCustomShellComboBox.SelectItem(const SI : TStShellItem);  {!!.03}
var
  I     : Integer;
  Index : Integer;
begin
