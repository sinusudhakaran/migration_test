...
begin
  if Length(S) >= LongInt(Len) then                                   
    ...
    SetLength(Result, Len);
    Move(S[1], Result[Succ(Word(Len))-Length(S)], Length(S)*2);
    R := longint(Len) - Length(S);
    J := 1;
    while (R > 0) do begin
      Result[J] := C;
...
begin
  ...
  Move(S[I], Result[1], Length(S)-Pred(I)*2);
end;
...
begin
  ...
  repeat
    Dec(I);
  until (pos(PathName[I], DosDelimSetW) > 0) or (PathName[I] = #0) or (I = 0);
