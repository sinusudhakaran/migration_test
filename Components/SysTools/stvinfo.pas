...
{$Z-}
    ...
    property FileFlags : LongInt
      read GetFileFlags;

    property FileFlagsMask : LongInt
    ...
    property FileType : LongInt
      read GetFileType;

    property FileSubtype : LongInt
      read GetFileSubtype;
...
var
  ...
  begin
    ...
    for I := 1 to Pred(Length(S)) do begin                             {!!.01}
      if S[I] = '.' then begin
...
begin
  ...
  if Size = 0 then begin
    ...
    {$ENDIF}
    {$IFDEF WIN16}
    { Win16 - Probably no version info in the file. }
    RaiseStError(EStVersionInfoError, stscVerInfoFail);
    {$ENDIF}
  end;
...
function TStCustomVersionInfo.GetFileFlags: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
  Result := FFileFlags;
...
function TStCustomVersionInfo.GetFileFlagsMask: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
...
function TStCustomVersionInfo.GetFileOS: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
  Result := FFileOS;
...
function TStCustomVersionInfo.GetFileSubtype: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
  Result := FFileSubtype;
...
function TStCustomVersionInfo.GetFileType: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
  Result := FFileType;
...
function TStCustomVersionInfo.GetFileMajorVersion: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
...
function TStCustomVersionInfo.GetFileMinorVersion: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
...
function TStCustomVersionInfo.GetProductMajorVersion: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
...
function TStCustomVersionInfo.GetProductMinorVersion: LongInt;
begin
  if not VInfoLoaded then
    LoadVersionInfo('');
