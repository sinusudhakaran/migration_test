...
function Str2RealS(const S : ShortString; var R : Real) : Boolean;     {!!.02}
{$ENDIF}                                                               {!!.02}
...
function IsChNumericS(C : Char; const Numbers : ShortString) : Boolean;
 {-Returns true if Ch in numeric set}
begin
  Result := CharExistsS(Numbers, C);                           {!!.03}
end;
...
begin
{$IFDEF MSWINDOWS}
  Result := Windows.IsCharAlpha(C) or CharExistsS(Numbers, C); {!!.03}
{$ENDIF}
{$IFDEF WIN16}
  Result := WinProcs.IsCharAlpha(C) or CharExistsS(Numbers, C);{!!.03}
{$ENDIF}
end;
...
begin
  ...
  if (length(S) > 0) then begin
    for I := 1 to Length(S) do
      ...
      {$ENDIF}
      {$IFDEF WIN16}
      if not WinProcs.IsCharAlpha(S[I]) then
      {$ENDIF}
        Exit;
    Result := true;
  end;
...
begin
  ...
  if (length(S) > 0) then begin
    for I := 1 to Length(S) do
      ...
      {$ENDIF}
      {$IFDEF WIN16}
      if (not WinProcs.IsCharAlpha(S[i])) and
      {$ENDIF}
