...
type
  ...
  public
    ...
    {Dragging : (dNothing, dField, dGroup, dColumn);}                  {!!.02}
...
begin
  ...
  moColumns :
    begin
      ...
      VF.SortDirection := DragField.DefaultSortDirection;              {!!.02}
      PostMessage(Handle, WM_Rebuild, 0, 0);
    end;
  end;
  {Dragging := dNothing;}                                              {!!.02}
  DragField := nil;
  DragColumn := nil;
...
begin
  ...
  moColumns :
    begin
      ...
      VF.SortDirection := VF.Field.DefaultSortDirection;               {!!.02}
      PostMessage(Handle, WM_Rebuild, 0, 0);
    end;
  end;
  {Dragging := dNothing;}                                              {!!.02}
...
begin
  if EditReportView.CurrentView.ViewFields.Count = 0 then                  {!!.02}
    raise Exception.Create('A view layout must have at least one column'); {!!.02}
