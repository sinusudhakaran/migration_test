...
type
  ...
  protected
    {FOwnerReport : TOvcAbstractReportView;}                           {!!.02}
  public
  ...
  protected
    ...
    function GetSelected: Boolean;                                     {!!.05}
    function GetTotal(Field: Integer): Double;
    procedure ResetLines;
    procedure SetExpanded(Value: Boolean);
    procedure SetSelected(const Value: Boolean);                       {!!.05}
    procedure Sort;
  public
    ...
    property IsSelected: Boolean read GetSelected write SetSelected;            {!!.05}
  ...
  protected
    ...
    FOwner: TOvcAbstractReportView;
    FRoot: TOvcRvIndexSuperGroup; {!!.02}
    FSortColumn: Integer;
  ...
  public
    ...
    property Root: TOvcRvIndexSuperGroup read FRoot; {!!.02}
    property SortColumn: Integer read FSortColumn write SetSortColumn;
    {property SortColumn0 : Integer read FSortColumn0;}                {!!.02}
  ...
  protected
    ...
    {property Lines: Integer read GetLines;} {moved to public}         {!!.02}
  ...
  {.Z+}
    ...
    property Lines: Integer read GetLines; {moved from protected}      {!!.02}
...
begin
  ...
  if Result then begin
    FieldName := S;
    ComputeTotals := Storage.ReadBoolean(Prefix, 'CompTotals', False); {!!.04}
    GroupBy := Storage.ReadBoolean(Prefix, 'GroupBy', False);          {!!.04}
  end;
end;
...
var
  ...
  var
    I: Integer;
  begin
    Sel := Sel or Group.IsSelected;                                    {!!.05}
    I := Group.Count - 1;
    while (I >= 0) and not Stop do begin
      if Group.ContainsGroups then
...
begin
  ...
  if FOwner.View.ViewFields.Count <= AGroupColumn then begin
    ...
    FAbsGroupColumn := -1;
    if FOwner.View.ViewFields.Count > 0 then                           {!!.05}
      FAbsSortColumn := FView.ViewField[0].Field.Index                 {!!.04}
    else                                                               {!!.05}
      FAbsSortColumn := 0;                                             {!!.05}
  end else begin
    ...
    if AGroupColumn > 0 then begin
      ...
      FAbsSortColumn := FView.ViewField[AGroupColumn].Field.Index;     {!!.04}
    end else begin
      FAbsGroupColumn := -1;
      FAbsSortColumn := FView.ViewField[0].Field.Index;                {!!.04}
    end;
...
var
  C{, AbsIndex}: Integer;                                              {!!.04}
begin
  {AbsIndex := FOwner.AbsIndex;}                                       {!!.04}
  ...
  Result := FOwner.Owner.DoCompareFields(Item1.Data, Item2.Data,
    {AbsIndex} FAbsSortColumn);                                        {!!.04}
  { Sub-sort the view on the other sortable Fields, from left to right. }
  if Result = 0 then begin
    C := 0;
    while (Result = 0) and (C < OwnerFieldCount) do begin
      if C = FAbsSortColumn{AbsIndex} then                             {!!.04}
        Result := 0
      else
        Result := FOwner.Owner.DoCompareFields(Item1.Data, Item2.Data, C);
      Inc(C)
