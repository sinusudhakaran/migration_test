...
var
  i : Integer;
begin
  if ColumnName[1] in ['A'..'Z','a'..'z'] then
    Result := View.Name + '_' + ColumnName[1]
  else
    Result := View.Name + '_';
  for i := 2 to length(ColumnName) do
    if ColumnName[i] in ['A'..'Z','a'..'z','0'..'9'] then
...
function TOvcReportViewEditor.GetVerb(Index : Integer) : AnsiString;
begin
  case Index of
...
function TOvcRvActiveViewProperty.GetValue : AnsiString;
begin
  Result := GetStrValue;
end;

procedure TOvcRvActiveViewProperty.SetValue(const AValue : AnsiString);
begin
  SetStrValue(AValue);
end;
...
function TOvcRvFieldNameProperty.GetValue : AnsiString;
begin
  Result := GetStrValue;
end;

procedure TOvcRvFieldNameProperty.SetValue(const AValue : AnsiString);
begin
  SetStrValue(AValue);
end;
{$ENDIF}
