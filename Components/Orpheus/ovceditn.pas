...
type
  ...
  public
    ...
    S         : PAnsiChar;  {text of paragraph; nil if empty}
    SLen      : Word;       {current length}

    constructor Init(P       : PAnsiChar;
                     WrapCol : Integer;
                     TabSize : Integer);
      {-create paragraph from text of P^}
    constructor InitLen(P       : PAnsiChar;
                        Len     : Word;
                        WrapCol : Integer;
                        TabSize : Integer);
    ...
    function  NthLine(N : Integer; var Len : Word) : PAnsiChar;
    ...
    function  InsertTextPrim(St : PAnsiChar; StLen : Word;
                             Pos, WrapCol, TabSize : Integer;
                             var FL, LL : Integer) : Word;
      {-insert StLen characters from St^ at Pos}
    function  InsertText(St : PAnsiChar; Pos, WrapCol, TabSize : Integer;
                         var FL, LL : Integer) : Word;
      {-insert St^ at Pos}
...
procedure TUndoNode.Init(PUR : PUndoRec; UT : UndoType; Link : Byte;
                         ...
                         Pos : Integer; D : PAnsiChar; DLen : Word);
begin
  with PUR^ do begin
    PrevSize := PSize;
    Flags := Ord(UT);
...
procedure TUndoNode.InitReplace(PUR : PUndoRec; Link : Byte; MF : Boolean;
                                ...
                                D : PAnsiChar; DLen : Word; R : PAnsiChar; RLen : Word);
var
  DP : PAnsiChar;
begin
  with PUR^ do begin
...
begin
  ...
  if NBS > BufSize then begin
    ...
    if (BufSize <> 0) then begin
      if SLen+1 >= 1024 then
        edMoveBlock(S^, P^, SLen+1)
      else
        edMoveFast(S^, P^, SLen+1);
...
function TParaNode.GetS : PAnsiChar;
  {-Return pointer to text of paragraph}
begin
  if S = nil then
    Result := ''
...
constructor TParaNode.Init(P : PAnsiChar; WrapCol, TabSize : Integer);
  {-Create paragraph from text of P^}
begin
...
constructor TParaNode.InitLen(P : PAnsiChar; Len : Word;
...
function TParaNode.InsertTextPrim(St : PAnsiChar; StLen : Word;
                                  Pos, WrapCol, TabSize : Integer;
                                  var FL, LL : Integer) : Word;
  {-Insert StLen characters from St^ at Pos}
var
...
function TParaNode.InsertText(St : PAnsiChar; Pos, WrapCol, TabSize : Integer;
                              var FL, LL : Integer) : Word;
  {-Insert St^ at Pos}
begin
...
function TParaNode.NthLine(N : Integer; var Len : Word) : PAnsiChar;
  {-Return a pointer to the Nth line of this paragraph; Len is its length}
var
  S1 : PAnsiChar;
begin
