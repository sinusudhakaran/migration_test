...
type
  ...
  {!!.05 new - for optimized streaming}
  ...
  protected
    ...
    IsSimulated   : Boolean;
    HintRect      : TRect;                                             {!!.02}
    FSelectColor : TOvcRvListSelectColors;                              {!!.05}
    function CalcMaxX: Integer;                                        {!!.04}
    ...
    procedure InternalSetItemIndex(Index: Integer; DeselectOld: Boolean); {!!.02}
    procedure Paint; override;
    procedure SetGridLines(Value : TOvcRVGridLines);
    procedure SetItemIndex(Index : LongInt); override;                 {!!.02}
    ...
    {procedure WMMouseMove(var Msg : TWMMouseMove); message WM_MOUSEMOVE;} {!!.02}
  ...
  public
    ...
    property SelectColor : TOvcRvListSelectColors
      read FSelectColor write FSelectColor;                            {!!.05}
  end;
  {.Z-}

  TOvcRvFieldSort = (rfsFirstAscending, rfsFirstDescending,
    rfsAlwaysAscending, rfsAlwaysDescending);                          {!!.02}
  ...
  protected
    ...
    FDefaultSortDirection : TOvcRvFieldSort;                           {!!.02}
    FExpression        : string;
    FImageIndex        : Integer;
    FNoDesign          : Boolean;
    FVPage             : Integer;
    ...
    procedure ReadState(Reader: TReader); override;                    {!!.04}
  public
    constructor Create(AOwner : TComponent); override;
    destructor Destroy; override;
  ...
  published
    ...
    property DefaultSortDirection: TOvcRvFieldSort                     {!!.02}
                   read FDefaultSortDirection write FDefaultSortDirection {!!.02}
                   default rfsFirstAscending;                          {!!.02}
  ...
  protected
    ...
    FSortDirection: TOvcRvFieldSort;                                   {!!.02}
  ...
  published
    ...
    property SortDirection: TOvcRvFieldSort                            {!!.02}
                   read FSortDirection write FSortDirection            {!!.02}
                   default rfsFirstAscending;                          {!!.02}
  end;
  ...
  protected
    ...
    FDirty: Boolean;
    FHidden: Boolean;
    FDFMBased: Boolean;                                                {!!.02}
    FFilterExp: TOvcRvExpression;
    procedure SetDirty(const Value: Boolean);                          {!!.04}
    ...
    function UniqueViewTitle(const Title : string) : string;           {!!.02}
  public
    procedure Assign(Source: TPersistent); override;
    constructor Create(AOwner : TComponent); override;
    destructor Destroy; override;
  ...
  protected
    ...
    procedure SetPrintFont(Value : TRvPrintFont);                      {!!.05}
  public
    constructor Create;
    destructor Destroy; override;
  published
  ...
  protected
    ...
    function GetListSelectColor: TOvcRvListSelectColors;               {!!.05}
  ...
  published
    ...
    property ListRowHeight : Integer
      ...
      stored NotListAutoRowHeight                                      {!!.05}
      default 18;
    ...
    property ListColor : TColor
      ...
      default vlDefColor;
    property ListSelectColor : TOvcRvListSelectColors                  {!!.05}
  ...
  protected
    ...
    LoadingViews             : Boolean;                                {!!.04}
    Searching                : Boolean;                                {!!.04}
    ...
    procedure CountSelection(Sender : TObject; Data : Pointer; var Stop : Boolean; UserData : Pointer);
    procedure CountSelection2(Sender: TObject; Data: Pointer;  var Stop: Boolean; UserData: Pointer);
    ...
    function DoGetFieldAsFloat(Data: Pointer; Field: Integer) : Double; override; {!!.04}
    ...
    {procedure HideHint;}                                              {!!.02}
    {procedure HoverTimerEvent(Sender : TObject; Handle : Integer;
                       Interval : Cardinal; ElapsedTime : LongInt);}   {!!.02}
    ...
    {procedure SetController(Value : TOvcController); override;}       {!!.02}   
...
{$ENDIF}
  ...
  published
    ...
    property ScrollBars;
    property ShowHint default True;                                    {!!.02}
    ...
    with TOvcCustomReportView(Owner) do if (CurrentView <> nil) then
      ...
      {Indexer sortcolumn is bumped by one}
        if Field.CanSort then begin
          if abs(InternalSortColumn) - 1 = Index then                  {!!.02}
            {column didn't change}
            case SortDirection of                                      {!!.02}
            rfsFirstAscending,
            rfsFirstDescending :
...
begin
  ...
  lVMargin := 3;
  ShowHint := True;                                                    {!!.02}
  FSelectColor  := TOvcRvListSelectColors.Create(vlDefSelectText, vlDefSelectBack);{!!.05}
  FSelectColor.OnColorChange := vlbColorChanged;                       {!!.05}
end;
...
begin
  ...
  if not ReportView.IsGroup[N] then begin
    ...
    FocusLeft := 0;
    if (ReportView.CurrentView <> nil) and ReportView.CurrentView.ShowGroupTotals then begin {!!.04}
      HaveTotals := True;                                              {!!.04}
      MaxX := CalcMaxX;                                                {!!.04}
    end else begin                                                     {!!.04}
      HaveTotals := False;                                             {!!.04}
    ...
    DrawGroupItem(LineCanvas.Canvas, Data, Gr,
      ReportView.Expanded[GrRef], CR2, FGColor, BGColor, GrRef, IsSelected, MaxX); {!!.04}
    if HaveTotals then                                                 {!!.04}     
...
var
  ...
  procedure FindGroupItem(Data : Pointer; Group : Integer;
    const Rect : TRect; MaxX: Integer);                                      {!!.04}
  {Finds a group line}
  var
    R : TRect;
    ViewFieldIndex,W : Integer;
    ...
    F : TOvcRvField;                                                   {!!.04}
  {const
    GlyphIdx : array[boolean] of Integer = (PlusSign,MinusSign);}      {!!.04}
  begin
    ViewFieldIndex := 0;
    R := Rect;
    R.Left := 1;
      ...
      if ViewFieldIndex = Group then
        begin
          ...
          if PtInRect(R,XY) then begin
            ...
            F := ReportView.CurrentView.ViewField[ViewFieldIndex].Field; {!!.04}
...
begin
  if FExp = nil then begin
    ...
    try
      ...
      M.Write(FExpression[1], length(FExpression));
...
begin
  ...
  DefaultSortDirection := TOvcRvFieldSort(
    Storage.ReadInteger(Prefix, 'DefaultSortDirection', 0));           {!!.02}
...
begin
  ...
  if DefaultSortDirection <> rfsFirstAscending then
    Storage.WriteInteger(Prefix, 'DefaultSortDirection', ord(DefaultSortDirection)); {!!.02}
  if NoDesign then
...
begin
  if Source is TOvcRvField then begin
    ...
    DefaultSortDirection := Field.DefaultSortDirection;                {!!.02}
  end;
...
begin
  ...
  if SortDirection <> rfsFirstAscending then                           {!!.02}
    Storage.WriteInteger(Prefix, 'SortDirection', ord(SortDirection)); {!!.02}
...
begin
  if inherited LoadFromStorage(Storage, Prefix) then begin
    ...
    SortDirection := TOvcRvFieldSort(Storage.ReadInteger(Prefix, 'SortDirection', 0)); {!!.02}
...
begin
  if FAggExp = nil then begin
    ...
    try
      M := TMemoryStream.Create;
      M.Write(FAggregate[1], length(FAggregate));
      RvExpParser.SourceStream.Free;
      RvExpParser.SourceStream := nil;
      RvExpParser.SourceStream := M;
      RvExpParser.Execute;
...
begin
  if Source is TOvcRvViewField then begin
    ...
    SortDirection := TOvcRvViewField(Source).SortDirection;            {!!.02}
  end;
  inherited Assign(Source);
end;
...
begin
  ...
  {FOwnerReport := TOvcCustomReportView(AOwner.Owner);}                {!!.02}
end;
...
begin
  if Owner <> nil then                                                   {!!.02}
    if ([csDesigning, csDestroying] * Owner.ComponentState = []) then  {!!.04}
    {if not (csDestroying in Owner.ComponentState) then}        {!!.02}{!!.04}
  ...
  and not FDFMBased                                                      {!!.02}
  and not OwnerReport.LoadingViews then                                  {!!.04}
  ...
  if SaveOwner <> nil then
    if not (csDestroying in SaveOwner.ComponentState) then             {!!.04}
...
begin
  if FFilterExp = nil then begin
    ...
    try
      M := TMemoryStream.Create;
      M.Write(FFilter[1], length(FFilter));
      RvExpParser.SourceStream.Free;
      RvExpParser.SourceStream := nil;
      RvExpParser.SourceStream := M;
      RvExpParser.Execute;
...
begin
  if Value <> FTitle then begin
    FTitle := UniqueViewTitle(Value);                                  {!!.02}
...
begin
  FPrintFont := TRvPrintFont.Create;                                   {!!.05}
...
procedure TOvcRvPrintProps.SetPrintFont(Value : TRvPrintFont);         {!!.05}
begin
    ...
    for i := 0 to pred(Views.Count) do
      if Value = View[i].Name then begin
        ...
        try
          ...
          if CurrentItem = nil then                                    {!!.03}
            Navigate(rvpScrollToTop);                                  {!!.03}
        end;
...
begin
  ...
  HintWindow.Color := Application.HintColor;}                          {!!.02}
...
begin
  ...
  {if Controller <> nil then
    FHoverTimer := Controller.TimerPool.Add(HoverTimerEvent, 250);}    {!!.02}
  {$ENDIF}
...
begin
  if rvHaveHS then
    begin
      ...
      SetScrollInfo(Handle, SB_HORZ, SI, True);                        {!!.02}
    end;
end;
...
var
  ...
  OldDFMBased: Boolean;                                                {!!.02}
begin
  if csDesigning in ComponentState then
    exit;
  SaveCurrent := ActiveView;
  LoadingViews := True;                                                {!!.04}
  try
      ...
      try
        ...
        for i := 0 to pred(Count) do begin
          ...
          if FieldKey <> '' then begin
            ...
            OldField := TOvcRvField(Fields.ItemByName(FieldName));     {!!.04}
            if OldField <> nil then begin                              {!!.04}
              OldDFMBased := OldField.FDFMBased;                       {!!.04}
              OldField.FDFMBased := False; {allow free}                {!!.04}
              OldField.Free;                                           {!!.04}
            end else                                                   {!!.04}
              OldDFMBased := False;                                    {!!.04}
            NewField := Fields.Add;
            NewField.Name := FieldName;
            NewField.LoadFromStorage(CustomViewStore, FieldKey);
            NewField.FDFMBased := OldDFMBased;                         {!!.04}
          end;
        end;
        ...
        for i := 0 to pred(Count) do begin
          ...
          if ViewKey <> '' then begin
            ...
            if OldView <> nil then begin
              OldDFMBased := OldView.FDFMBased;                        {!!.02}
              OldView.FDFMBased := False; {allow free}                 {!!.02}
            end else
              OldDFMBased := False;
            OldView.Free;
            NewView := Views.Add;
            ...
            NewView.FDFMBased := OldDFMBased;                          {!!.02}
          end;
        end;
      finally
    ...
    LoadingViews := False;                                             {!!.04}
    DoChangeNotification(rvViewDestroy);                               {!!.04}
    DoChangeNotification(rvViewCreate);                                {!!.04}
...
var
  NewView : TOvcRvView;
  OldView : TOvcRvView;                                                {!!.02}
begin
  OldView := TOvcRvView(Views.ItemByName(Name));                       {!!.02}
  OldView.FDFMBased := False; {to allow free}                          {!!.02}
  OldView.Free;                                                        {!!.02}
  {Views.ItemByName(Name).Free;}                                       {!!.02}
...
begin
  ...
  FRVListBox.lAnchor := FRVListBox.ItemIndex;                          {!!.05}
end;
  ...
  ' ','+','-',#13 :
    ...
    if KeySearch then begin
      ...
      if InternalSortColumn < 0 then begin
        ...
        fillchar(ExtkeyString[1],255,#255);
        move(KeyString[1],ExtKeyString[1],length(KeyString));
      end else
  ...
  if SaveCurItem <> nil then begin
    ...
    if not CollapseEvent then begin
      ...
      if (Offset <> -1) and not IsGroup[Offset] then begin
        {FRVListBox.ItemIndex := OffsetOfData[SaveCurItem];}               {!!.02}
        FRVListBox.InternalSetItemIndex(OffsetOfData[SaveCurItem], False); {!!.02}
        if AutoCenter then
          FRVListBox.CenterCurrentLine;                                   
      end else begin
        FRVListBox.ItemIndex := -1;
...
begin
  ...
  if FRVListBox.ItemIndex <> -1 then begin
    if IsGroup[FRVListBox.ItemIndex] then begin
      SaveCurItem := nil;
      {FRVListBox.ItemIndex := -1;}                                    {!!.02}
      FRVListBox.InternalSetItemIndex(-1, False);                      {!!.02}
    end else
...
begin
  ...
  for i := 0 to pred(CurrentView.ViewFields.Count) do
    with CurrentView.ViewField[i], Field do begin
      if Visible then begin                                            {!!.03}
        PW := TwipsToPixels(PrintWidth);                               {!!.04}
        Right := CurX + PW;                                            {!!.04}
        if Right >= PrintStopRight then begin
          inc(VPage);
          CurX := PrintStartLeft + PW;                                 {!!.04}
        end else
          CurX := Right;
        FVPage := VPage;
      end;                                                             {!!.03}
...
begin
  ...
  and assigned(FCurrentView)
  and StoreChangedWidths {WidthChanged}                                {!!.05}
  and not (csDesigning in ComponentState) then begin
    ...
    StoreChangedWidths {WidthChanged} := False;                        {!!.05}
  end;
end;
...
var
  i : Integer;
begin
  if Title[1] in ['A'..'Z','a'..'z'] then
    Result := Title[1]
  else
    Result := '_';
  for i := 2 to length(Title) do
    if Title[i] in ['A'..'Z','a'..'z','0'..'9'] then
...
function TOvcRVOptions.NotListAutoRowHeight: Boolean;                  {!!.05}
begin
...
function TOvcRVOptions.GetListSelectColor: TOvcRvListSelectColors;     {!!.05}
begin
  Result := TOvcRvListSelectColors(FOwner.FRVListBox.SelectColor);     {!!.05}
end;
...
procedure TOvcRVOptions.SetListSelectColor(const Value: TOvcRvListSelectColors); {!!.05}
begin
