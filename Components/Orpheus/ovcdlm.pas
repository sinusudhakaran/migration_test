...
begin
  Result := @PChar(Buffer)[Index * (sizeof(Pointer) + FValueSize + sizeof(LongInt))];
end;

function TOvcLiteCache.GetTimeStamp(Index: Integer): LongInt;
begin
  Result := LongInt(Pointer(@
    PChar(Buffer)[Index * (sizeof(Pointer) + FValueSize + sizeof(LongInt))
...
begin
  Result := @PChar(Buffer)[Index * (sizeof(Pointer)
