...
type
  ...
  TPictureDataType   = (
    ...
    pftDouble, pftSingle, pftComp, pftDate, pftTime, pftDateTime);    {!!.05}
...
begin
  ...
  if not efSaveData and FInitDateTime then begin
    ...
    if DataType = pftDateTime then                                    {!!.05}
      AsDateTime := now;                                              {!!.05}
  end;
            ...
            if Found and (Ch = IntlSupport.DecimalChar) then begin
              ...
              if not CharIsOK then begin
                if ((DataType = pftDate) and (Ch = DateSeparator)) or
                   ((DataType = pfttime) and (Ch = TimeSeparator)) or {!!.05}
                   ((DataType = pftDateTime) and ((Ch = TimeSeparator){!!.05}
                                               or (Ch = DateSeparator))){!!.05}
                then                                                    {!!.05}
...
begin
  ...
  case FPictureDataType of
    ...
    pftDateTime  : IncDecValueDouble;                                 {!!.05}
...
begin
  case Value of
    ...
    pftDateTime  : Result := fidPictureDate;                          {!!.05}
...
begin
  case FT of
    ...
    pftDateTime  : begin                                              {!!.05}
                     FC := Pos('yyyy', ShortDateFormat) > 0;          {!!.05}
                     M := IntlSupport.InternationalDate(FC) + ' '     {!!.05}
                       + IntlSupport.InternationalTime(False);        {!!.05}
                     Update(M);                                       {!!.05}
                   end;                                               {!!.05}
  else
    raise EOvcException.Create(GetOrphStr(SCInvalidParamValue));
