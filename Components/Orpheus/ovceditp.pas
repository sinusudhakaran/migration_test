...
(*Changes)
{!!.03}
  01/21/02- Added code to strip carriage returns out of a block of text.
*)
...
function TOvcParaList.ActualCol(S : PAnsiChar; Len, Col : Word) : Word;
  {-given an effective column #, return an actual column #}
var
  ELen : Word;
  HT   : Boolean;
...
function TOvcParaList.AppendParaEof(S : PAnsiChar; SLen : Word; Trim : Boolean) : Word;
  {-create a new paragraph from S and append to end of file}
var
  PPN : TParaNode;
begin
...
function TOvcParaList.EffCol(S : PAnsiChar; Len, Col : Word) : Word;
  {-compute effective column}
begin
  if (Col <= 1) or (Len = 0) then
    if Col = 0 then
...
function TOvcParaList.EffStrLen(S : PAnsiChar; Len : Word) : Word;
  {-compute effective length of S}
begin
...
function TOvcParaList.NthLine(N : LongInt; var S : PAnsiChar;
...
procedure TOvcUndoBuffer.Append(D : PAnsiChar; DLen : Word);
var
  S : PAnsiChar;
begin
...
procedure TOvcUndoBuffer.AppendReplace(D : PAnsiChar; DLen : Word;
                                    R : PAnsiChar; RLen : Word);
var
  S : PAnsiChar;
  Len : Word;
...
procedure TOvcUndoBuffer.Prepend(D : PAnsiChar; DLen : Word);
var
  S : PAnsiChar;
begin
  {make sure there's room}
