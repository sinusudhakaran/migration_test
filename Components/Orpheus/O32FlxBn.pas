...
{!!.02 - HWnd changed to TOvcHWnd for BCB Compatibility}
  ...
  published
    property About;
    property ActiveItem default -1;                                   {!!.05}
    property ItemCollection;
    {$IFDEF VERSION4}
    property WheelSelection default True;                             {!!.05}
    {$ENDIF}
    property PopPosition default ppBottomRight;                       {!!.05}
    property PopRowCount default 5;                                   {!!.05}
    property PopWidth default 125;                                    {!!.05}
    property SmartPop default True;                                   {!!.05}
    property OnClick;
    property OnMenuPop;
...
begin
  if (not FMenuActive) and (FItems.Count > 0) then begin
    ...
    FPopupMenu.Parent := GetImmediateParentForm(self);                {!!.05}
//    FPopupMenu.Parent := Self.Parent;                               {!!.05}
    ...
    case FPopPosition of
      ppBottomLeft: begin
        FPopupMenu.Left := Left;
        FPopupMenu.Top := Top + Height;
        { if the menu extends below the bottom of the form then display it at }
        { the top of the button }
        if SmartPop then
          if FPopupMenu.Top + FPopupMenu.Height > Parent.Height then
            FPopupMenu.Top := Top - FPopupMenu.Height;
      end;

      ppBottomRight: begin
        FPopupMenu.Left := (Left + Width) - FPopupMenu.DefaultColWidth - 4;
        FPopupMenu.Top := Top + Height;
        { if the menu extends below the bottom of the form then display it at }
        { the top of the button }
        if SmartPop then
...
begin
  ...
  case FPopPosition of
    ppBottomLeft  : begin
      ...
      FPopRect.Top := R.Bottom - PopAreaSize;
      FPopRect.Bottom := R.Bottom;
    end;

    ppBottomRight : begin
      FPopRect.Left := R.Right - PopAreaSize;
      FPopRect.Right := R.Right;
      FPopRect.Top := R.Bottom - PopAreaSize;
      FPopRect.Bottom := R.Bottom;
    end;
