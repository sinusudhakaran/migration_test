...
begin
  FReportView.DatasetScrolled;                                         {!!.05}
end;
...
var
  ...
  B1,B2 : Boolean;
  F1,F2 : double;
  ActiveRecord: Integer;                                               {!!.05}
begin                                                                  {!!.05}
  result := 0;                                                         {!!.05}
  ActiveRecord := FDataLink.ActiveRecord;                              {!!.05}
  try
    ...
    FDataLink.ActiveRecord := ActiveRecord;                            {!!.05}
  end;                                                                 {!!.05}
end;
...
//function TOvcDbReportView.DoGetGroupString(FieldIndex : Integer; GroupRef : TGroupRef) : string;
var
  D : double;
  FmtStr: string;
...
begin
  ...
  if F is TIntegerField then
    ...
    Result := '';
  if assigned(F.OnGetText) then                                        {!!.05}
    F.OnGetText(F, Result, True);                                      {!!.05}
end;
...
begin
  if UseRecordCount then
    ...
    Result := 0;
    P := DataSource.DataSet.GetBookmark;                               {!!.05}
    ...
    try
      ...
      DataSource.DataSet.GotoBookmark(P);                              {!!.05}
