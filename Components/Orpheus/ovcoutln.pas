...
type
  ...
  protected
    ...
    FTextSort       : Boolean;
    IsSimulated     : Boolean;
    {HintShownHere   : Boolean;}                                       {!!.02}
    {HintWindow      : THintWindow;}                                   {!!.02}
    {HintX           : Integer;}                                       {!!.02}
    {HintY           : Integer;}                                       {!!.02}
    LineCanvas      : TBitmap;
    {FHoverTimer     : Integer;}                                       {!!.02}
    ...
    Clearing        : Boolean;
    UpdateScrollWidthPending : Boolean;                                {!!.02}
    FCacheSize: Integer;                                               {!!.02}
    procedure SetCacheSize(const Value: Integer);                      {!!.02}
    function FirstChild: TOvcOutlineNode;
    function NextChild: TOvcOutlineNode;
    function LastChild: TOvcOutlineNode;                               {!!.02}
    procedure PopChildIndex;
    procedure PushChildIndex;
    ...
    function CalcMaxWidth: Integer;                                    {!!.02}
    ...
    procedure SetScrollBars(const Value : TScrollStyle); override;      {!!.02}
    ...
    procedure UpdateScrollWidth;                                       {!!.02}
  ...
  {.Z-}
    ...
    property CacheSize: Integer read FCacheSize write SetCacheSize default 4096; {!!.02}
    procedure Clear;
  ...
  published
    ...
    property Enabled;
    property Font;
    property HideSelection default False;                              {!!.05}
    property Images;
    property IntegralHeight;
    property Nodes;
    property Keys;
    property ParentColor default False;                                {!!.05}
    property ParentCtl3D;
    property ParentFont;
    property ParentShowHint default False;                             {!!.05}
    property PopupMenu;
    property RowHeight stored NotAutoRowHeight;                        {!!.05}
    property ScrollBars;
    ...
    property TabOrder;
    property TabStop default True;                                     {!!.05}
    property TextSort;
    property Visible;
...
begin
  ...
  Assert(Owner <> nil);
  if Parent = nil then
    Result := Outline.FNodes.LastChild = Self                          {!!.02}
  else
...
begin
  ...
  {HintWindow := HintWindowClass.Create(Self);}                        {!!.02}
...
begin
  Clear;
  FOnActiveChange := nil;
  {HintShownHere := True;}                                            {!!.02}
  {if FHoverTimer <> -1 then begin}                                   {!!.02}
    {if FController <> nil then }                              {!!.01}{!!.02}
      {FController.TimerPool.Remove(FHoverTimer);}                    {!!.02}
    {FHoverTimer := -1;}                                              {!!.02}
  {end;}                                                              {!!.02}
  {HintWindow.Free;}                                                  {!!.02}
  FNodes.Free;
...
begin
  ...
  if lUpdating = 0 then begin
    UpdateLines;
    if UpdateScrollWidthPending then                                   {!!.02}
      UpdateScrollWidth;                                               {!!.02}
  end;
end;
...
var
  ...
  begin
    ...
    if Bool(IntersectRect(IR, Clip, CR)) then begin
      ...
      if (ScrollBars in [ssHorizontal, ssBoth]) then                   {!!.02}
        CRWidth := MaxI(Columns + ClientWidth, CR.Right - CR.Left + 1) {!!.02}
      else                                                             {!!.02}
      ...
      R.Top := 0;
      {R.Bottom := R.Top + (CR.Bottom - CR.Top);}                      {!!.02}
      R.Bottom := FRowHeight; {same thing}                             {!!.02}
      ...
      if ShowButtons and (CurNode.HasChildren or ((CurNode.Mode <> omPreload)
        ...
        if lhDelta <> 0 then begin                                     {!!.02}
          R2 := CurNode.ButtonRect;                                    {!!.02}
          OffsetRect(R2, -lhDelta, 0);                                 {!!.02}
          CurNode.ButtonRect := R2;                                    {!!.02}
        end;                                                           {!!.02}
      end;
...
procedure TOvcCustomOutline.SaveAsText(const FileName: string);
  ...
  begin
    for i := 0 to Level - 1 do
      S.Write(Tab, 1);
    S.Write(ParentNode.Text[1], length(ParentNode.Text));
    S.Write(CrLf, 2);
...
procedure TOvcCustomOutline.SaveToStream(Stream: TStream);
  ...
  begin
    ...
    Stream.Write(ParentNode.Text[1], i);
