...
type
  ...
  published
    ...
    property ParentFont;
    property ParentShowHint default False;                             {!!.05}
    property PopupMenu;
    property PrinterProperties;
    property ScrollBars;
    property ShowHint default True;                                    {!!.05}
...
implementation
const
  StreamSig : array[0..3] of char = 'ODRD';

type
  PBoolean  = ^Boolean;
  PDateTime = ^TDateTime;                                             {!!.04}
...
var                                                                    {!!.04}
  F: TOvcRvField;                                                      {!!.04}
begin
  F := Self.Field[Field];                                              {!!.04}
  if F.Expression <> '' then                                           {!!.04}
    Result := F.GetValue(Data)                                         {!!.04}
  else                                                                 {!!.04}
    {Result := TOvcDataRvItem(Data).AsFloat[Field];}                   {!!.04}
    {!!.04 begin}
    case F.DataType of
    dtInteger :
...
var
  FieldIndex: Integer;
begin
  if ViewField.Aggregate <> '' then                                      {!!.04}
    Result := ViewField.AggExp.GetValue(GroupRef)                        {!!.04}
  else begin                                                             {!!.04}
...
begin
  ...
  Temp := Temporary;
  PreAllocate; {Pre-allocate record buffer}                            {!!.04}
  ...
  dtString :
    begin
      if FieldSize[Index] = 0 then
        ...
        SetLength(Result,FieldSize[Index]-1); {account for trailing #0}
  ...
  dtString :
    begin
      if FieldSize[Index] = 0 then
        ...
        SetLength(F,FieldSize[Index]-1); {account for trailing #0}
        if FieldSize[Index] > 1 then
...
begin
  if Value <> AsBoolean[Index] then begin
    {CheckType(Index, dtBoolean);} {redundant Get just checked it}     {!!.04}
    Assert(FieldSize[Index] = sizeof(Boolean));                        {!!.04}
    {FieldSize[Index] := sizeof(Boolean);}                             {!!.04}
    {move(Value,DataBuffer^[FieldIndexTable^[Index]],sizeof(Boolean));}{!!.04}
    PBoolean(@DataBuffer^[FieldIndexTable^[Index]])^ := Value;         {!!.04}
    Changed;
  end;
end;
...
begin
  if Value <> AsDateTime[Index] then begin
    {CheckType(Index, dtDateTime);} {redundant Get just checked it}     {!!.04}
    Assert(FieldSize[Index] = sizeof(TDateTime));                      {!!.04}
    {FieldSize[Index] := sizeof(TDateTime);}                           {!!.04}
    {move(Value,DataBuffer^[FieldIndexTable^[Index]],sizeof(TDateTime));}{!!.04}
    PDateTime(@DataBuffer^[FieldIndexTable^[Index]])^ := Value;         {!!.04}
    Changed;
  end;
end;
...
begin
  if Value <> AsFloat[Index] then begin
    {CheckType(Index, dtFloat);} {redundant Get just checked it}     {!!.04}
    Assert(FieldSize[Index] = sizeof(Extended)); {!!.04}
    {FieldSize[Index] := sizeof(Extended);
    {move(Value,DataBuffer^[FieldIndexTable^[Index]],sizeof(Extended));}
    PExtended(@DataBuffer^[FieldIndexTable^[Index]])^ := Value;         {!!.04}
    Changed;
  end;
end;
...
begin
  if Value <> AsInteger[Index] then begin
    {CheckType(Index, dtInteger);} {redundant Get just checked it}     {!!.04}
    Assert(FieldSize[Index] = sizeof(Integer));                        {!!.04}
    {FieldSize[Index] := sizeof(Integer);}                             {!!.04}
    {move(Value,DataBuffer^[FieldIndexTable^[Index]],sizeof(Integer));}
    PInteger(@DataBuffer^[FieldIndexTable^[Index]])^ := Value;         {!!.04}
    Changed;
  end;
end;
...
begin
  ...
  dtString :
    if Value <> AsString[Index] then begin
      FieldSize[Index] := length(Value) + 1;
      if FieldSize[Index] > 1 then
        move(Value[1],DataBuffer^[FieldIndexTable^[Index]],length(Value)+1);
      Changed;
    end;
  dtFloat  :
    begin
      Val(Value,E,Err);
      if Err = 0 then
        SetAsFloat(Index, E)                                         {!!.04}
      else
        SetAsFloat(Index, 0);                                        {!!.04}
    end;
  dtInteger :
    begin
      Val(Value,E,Err);
      if Err = 0 then
        SetAsInteger(Index, round(E))                                {!!.04}
      else
        SetAsInteger(Index, 0);                                      {!!.04}
    end;
  dtDateTime :
    SetAsDateTime(Index, StrToDateTime(Value));                      {!!.04}
  dtBoolean :
    SetAsBoolean(Index, CompareText(Value, 'TRUE') = 0);             {!!.04}
  {$IFDEF Version4}
  dtDWord :
    begin
      Val(Value,E,Err);
      if Err = 0 then
        SetAsDWord(Index, round(E))                                  {!!.04}
      else
        SetAsDWord(Index, 0);                                        {!!.04}
    end;
  {$ENDIF}
  end;
end;
...
begin
  if Value <> AsDWord[Index] then begin
    {CheckType(Index, dtDWord);} {redundant Get just checked it}     {!!.04}
    Assert(FieldSize[Index] = sizeof(DWord));                          {!!.04}
    {FieldSize[Index] := sizeof(DWord);}                               {!!.04}
    {move(Value, DataBuffer^[FieldIndexTable^[Index]], sizeof(DWord));}{!!.04}
    PDWord(@DataBuffer^[FieldIndexTable^[Index]])^ := Value;         {!!.04}
    Changed;
  end;
end;
