...
(* Changes)
{!!.01}
  - 8/15/01 - Published TO32InspectorItem's ItemsList property. - PB
  - 8/15/01 - Fixed bug which prevented a list item from being updated
              programatically. - PB
  - 8/31/01 - OnGetEditMask and OnGetEditText wouldn't transfer the user
              specified strings to the component.  Changed the Mask and Text
              parameters to var params.
{!!.02}
  - 12/27/01- Prevent InspectorItems from writing their value property to the
              DFM in the event it is an empty string.
  - 12/27/01- Add the ability for InspectorItems to stream their items lists. 
*)
...
uses
  Windows, Graphics, Classes, Forms, Mask, StdCtrls, ExtCtrls, Grids,    {!!.05}
  Messages, Controls, OvcBase, OvcCmbx, OvcClrCb, ovcftcbx;              {!!.05}
...
type
  ...
  protected {private}
    ...
    procedure Changed(Sender: TObject);                                  {!!.05}
  ...
  public
    ...
    {$IFNDEF VERSION4}
      {$IFNDEF CBUILDER}
        {D3}
        procedure Invalidate; override;                                  {!!.03}
        procedure SetFocus; override;                                    {!!.03}
      {$ELSE}
        {BCB3}
  ...
  protected {private}
    ...
    procedure SetIndex(Value: Integer); override;                        {!!.02}
    procedure SetInteger(Value: Integer);
    procedure SetItemsList(Value: TStringList);                          {!!.02}
    ...
    function  GetItemsList: TStringList;                                 {!!.02}
  ...
  public
    ...
    procedure FreeObject;                                                {!!.05}
    procedure AddObject(Ref: TObject);                                   {!!.05}
  ...
  published
    ...
    property ItemsList: TStringList                                      {!!.02}
      read GetItemsList write SetItemsList;                              {!!.02}
    property ItemType: TO32IGridItemType
      read FType write SetType;
    property ReadOnly: Boolean                                           {!!.02}
      read FReadOnly write FReadOnly;                                    {!!.02}
    property Tag: Integer                                                {!!.05}
      read FTag write FTag;                                              {!!.05}
  ...
  protected {private}
    ...
    FAfterEdit : To32IGridItemEvent;                                     {!!.05}
    ...
    FOnItemEditorChange: To32IGridItemEvent;                             {!!.05}
    ...
    function  ParentTypeCount: Integer;                                  {!!.05}
  ...
  {To be published}
    ...
    property Sorted: Boolean read FSorted write SetSorted default false; {!!.05}

    property AfterEdit: To32IGridItemEvent                               {!!.05}
      read FAfterEdit write FAfteredit;                                  {!!.05}
    ...
    property OnItemEditorChange: To32IGridItemEvent                      {!!.05}
      read FOnItemEditorChange write FOnItemEditorChange;                {!!.05}
  ...
  published
    ...
    property EditColor default clWindow;                                 {!!.05}
    property Enabled;
    property ExpandGlyph;
    property Font;
    property GridLineColor default clBtnShadow;                          {!!.05}
    property Hint;                                                       {!!.04}
    property ItemCollection;
    property ItemTextColor default clNavy;                               {!!.05}
    property Images;
    property LabelInfo;
    ...
    property OnItemEditorChange;                                         {!!.05}
...
uses
  Math, Consts, SysUtils, OvcData, OvcConst, OvcVer, Dialogs, OvcUtils;
...
procedure TO32GridEdit.Changed(Sender: TObject);                         {!!.05}
begin                                                                    {!!.05}
  if not Updating then                                                   {!!.05}
    Grid.EditorChanged;                                                  {!!.05}
end;                                                                     {!!.05}
{=====}                                                                  {!!.05}
...
procedure TO32GridCombo.Changed(Sender: TObject);                        {!!.05}
begin                                                                    {!!.05}
  if not Updating then                                                   {!!.05}
    Grid.EditorChanged;                                                  {!!.05}
end;                                                                     {!!.05}
...
procedure TO32GridColorCombo.Changed(Sender: TObject);                   {!!.05}
begin                                                                    {!!.05}
  if not Updating then                                                   {!!.05}
    Grid.EditorChanged;                                                  {!!.05}
end;                                                                     {!!.05}
...
procedure TO32GridFontCombo.Changed(Sender: TObject);                    {!!.05}
begin                                                                    {!!.05}
  if not Updating then                                                   {!!.05}
    Grid.EditorChanged;                                                  {!!.05}
end;                                                                     {!!.05}
...
procedure TO32InspectorItem.FreeObject;                                  {!!.05}
begin                                                                    {!!.05}
  if (ObjectRef <> nil) then                                             {!!.05}
  begin                                                                  {!!.05}
    ObjectRef.Free;                                                      {!!.05}
    ObjectRef := nil;                                                    {!!.05}
  end;                                                                   {!!.05}
...
procedure TO32InspectorItem.AddObject(Ref: TObject);                     {!!.05}
begin                                                                    {!!.05}
  if (Ref <> nil) then begin                                             {!!.05}
    if (ObjectRef <> nil) then                                           {!!.05}
      FreeObject;                                                        {!!.05}
    ObjectRef := Ref;                                                    {!!.05}
  end;                                                                   {!!.05}
...
begin
  if Value <> FCaption then begin
    FCaption := Value;
    if FIGrid.Sorted then                                                {!!.05}
      (Collection as TO32InspectorItems).Sort;                           {!!.05}
    FIGrid.ItemChanged(Self);
  end;
end;
...
procedure TO32CustomInspectorGrid.EditorChanged;                         {!!.05}
begin                                                                    {!!.05}
  if Assigned(FOnItemEditorChange) then                                  {!!.05}
    FOnItemEditorChange(self, ActiveItem);                               {!!.05}
end;                                                                     {!!.05}
{=====}                                                                  {!!.05}
...
begin
  ...
  if Default then begin                                               {!!.03}
    ...
    { Paint the caption cell (left) }
      ...
      { the expand glyph }
      if ParentTypeCount > 0 then begin                                  {!!.05}
        { Indent child items }
...
begin
  ...
  if FInPlaceEdit <> nil then begin
    ...
    if Changed and Assigned(FAfterEdit) then                             {!!.05}
      FAfterEdit(TO32InspectorItem(FItems.VisibleItems[ActiveRow]),      {!!.05}
        TO32InspectorItem(FItems.VisibleItems[ActiveRow]).Index);        {!!.05}
  end;
end;
{=====}
...
begin
  ...
  if ssCtrl in Shift then
    ...
    case Key of
      VK_UP: begin                                                       {!!.05}
        Dec(NewCurrent.Y);                                               {!!.05}
        if (ActiveItem = 0) then UpdateCellContents;                     {!!.05}
      end;                                                               {!!.05}
      VK_DOWN: begin                                                     {!!.05}
        Inc(NewCurrent.Y);                                               {!!.05}
        if (ActiveItem = pred(ItemCount)) then UpdateCellContents;       {!!.05}
      end;                                                               {!!.05}
...
(* - Moved to the UpdateCellContents method
    if Assigned(FAfterEdit) then                                         {!!.05}
      FAfterEdit(TO32InspectorItem(FItems.VisibleItems[ActiveRow]),      {!!.05}
        TO32InspectorItem(FItems.VisibleItems[ActiveRow]).Index);        {!!.05}
*)
  end;
...
function TO32CustomInspectorGrid.ParentTypeCount: Integer;               {!!.05}
var                                                                      {!!.05}
  I: Integer;                                                            {!!.05}
  Item: TO32InspectorItem;                                               {!!.05}
begin                                                                    {!!.05}
  Result := 0;                                                           {!!.05}
  for I := 0 to Pred(FItems.Count) do begin                              {!!.05}
    Item := TO32InspectorItem(FItems.Items[I]);                          {!!.05}
    if (Item.ItemType in [itParent, itSet]) then                         {!!.05}
      Inc(Result);                                                       {!!.05}
  end;                                                                   {!!.05}
end;                                                                     {!!.05}
{=====}                                                                  {!!.05}
