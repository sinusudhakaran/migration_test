...
(*Changes)
{!!.03}
  01/24/02 - Fixed AutoSelect.
  01/24/02 - The control was not respecting efColors.Disabled colors.
             (partial fix as the ancestor is still overriding the
              font color when the control is disabled.)
  02/11/02 - Added SoftValidation functionality to ValidateSelf.
{!!.04}
  03/15/02 - Added an OnFontChange message Handler.
  05/17/02 - Removed the non-implemented DataType property and support methods.
             Originally we planned on implementing a DataType property but the
             Design changed mid-stream and we accidentally forgot to remove
             these stubbed in methods. There are no plans to implement a
             datatype property in this control.
*)
...
//    FDataType         : TO32FlexEditDataType;                       {!!.04}
...
//    procedure SetDataType      (Value : TO32FlexEditDataType);      {!!.04}
...
{!!.03 - was commented out in 4.02 and re-written in 4.05}
...
//    property DataType: TO32FlexEditDataType                         {!!.04}
//      read FDataType write SetDataType default feString;            {!!.04}
...
//      read FPasswordChar write SetPwdChar;
  ...
  published
    ...
    {$ENDIF}
    property AutoSize default False;                                  {!!.05}
    property About;
    property AutoSelect;
    property Borders;
    ...
    property PopupAnchor default paLeft;                              {!!.05}
    property PopupMenu;
    property ReadOnly;
    property ShowButton default False;                                {!!.05}
    property ShowHint;
    property TabOrder;
    property TabStop;
    property Text;
...
begin
  TO32FlexEdit(Parent).SetFocus;                                      {!!.02}
...
begin
  inherited Create;
  FMaxLines       := 3{1};                                            {!!.05}
  FFocusedLines   := 3{1};                                            {!!.05}
  FMouseOverLines := 3{1};                                            {!!.05}
  FDefaultLines   := 1;
end;
{=====}
...
begin
  ...
  {!!.05 now set in TO32EditLines.Create:
