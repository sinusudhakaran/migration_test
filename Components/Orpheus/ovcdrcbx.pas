...
type
  ...
  published
    ...
    property Drive : Char
      read FDrive write SetDrive default #0;                             {!!.05}
      ...
      repeat
        if (SR.Attr and faDirectory) = faDirectory then
          if (SR.Name <> '.') and (SR.Name <> '..') then begin
            Path := FDirectory;
            if (Path <> '') and (Path[Length(Path)] <> '\') then         {!!.04}
              Path := Path + '\';
            Path := Path + SR.Name;
...
begin
  if Value <> FDrive then begin
    FDrive := Value;
    { if FDirectory is an empty string }                                 {!!.05}
    if (FDirectory = '')                                                 {!!.05}
    { or FDirectory is not an empty string, and it points to a drive }   {!!.05}
    { other than FDrive, then modify FDirectory to point at the root }   {!!.05}
    { of the newly selected drive.                                   }   {!!.05}
    or ((FDirectory <> '') and (FDrive <> FDirectory[1])) then           {!!.05}
      FDirectory := FDrive + ':\';                                       {!!.05}
    Populate;
  end;
end;
