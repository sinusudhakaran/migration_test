...
type
  TOvcCustomTable = class(TOvcTableAncestor)
    ...
    protected {private}
      ...
      FOnResizeColumn     : TColResizeEvent;                          {!!.05}
      FOnResizeRow        : TRowResizeEvent;                          {!!.05}
...
{!!.02 - HWnd changed to TOvcHWnd for BCB Compatibility }
      ...
      property OldRowColBehavior : Boolean                               {!!.05}
         read FOldRowColBehavior write FOldRowColBehavior;               {!!.05}
      ...
      property OnResizeColumn : TColResizeEvent                       {!!.05}
         read FOnResizeColumn write FOnResizeColumn;                  {!!.05}

      property OnResizeRow : TRowResizeEvent                          {!!.05}
         read FOnResizeRow write FOnResizeRow;                        {!!.05}
  ...
  TOvcTable = class(TOvcCustomTable)
    ...
    published
      {Properties}
      property LockedRows default 1;                                  {!!.05}
      property TopRow default 1;                                      {!!.05}
      property ActiveRow default 1;                                   {!!.05}
      property RowLimit default 10;                                   {!!.05}

      property LockedCols default 1;                                  {!!.05}
      property LeftCol default 1;                                     {!!.05}
      property ActiveCol default 1;                                   {!!.05}

      property OldRowColBehavior default false;                       {!!.05}
      ...
      property DragKind;
      {$ENDIF}
      property Access default otxNormal;                              {!!.05}
      property Adjust default otaCenterLeft;                          {!!.05}
      property Align;
      property BorderStyle default bsSingle;                          {!!.05}
      property ColCount stored False;
      property Color default tbDefTableColor;
      property ColorUnused default clWindow;                          {!!.05}
      property Colors;
      property Columns;
      property Controller;
      property Ctl3D;
      ...
      property OnResizeColumn;                                        {!!.05}
      property OnResizeRow;                                           {!!.05}
...
procedure TOvcCustomTable.tbSetActiveCellPrim(RowNum : TRowNum; ColNum : TColNum);
  ...
  begin
    ...
    try
      ...
      if not OldRowColBehavior then                                      {!!.05}
        ...
        try
          if (otsDoingRow in tbState) then
            begin
              ...
              if Assigned(OnResizeRow) then                           {!!.05}
                OnResizeRow(Self, tbRowNums^.Ay[tbSizeIndex].Number,  {!!.05}
                  FRows.Height[tbRowNums^.Ay[tbSizeIndex].Number]);   {!!.05}
            end
          else
            begin
              ...
              if Assigned(OnResizeColumn) then                        {!!.05}
                OnResizeColumn(Self, tbColNums^.Ay[tbSizeIndex].Number,{!!.05}
                  FCols[tbColNums^.Ay[tbSizeIndex].Number].Width);    {!!.05}
            end;
