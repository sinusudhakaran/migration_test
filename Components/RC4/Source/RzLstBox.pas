...
begin
  ...
  begin
    if not UseRightToLeftAlignment then
      PaintGradient( Canvas, Rect, gdVerticalEnd, BackColor, Color )
    else
      PaintGradient( Canvas, Rect, gdVerticalEnd, Color, BackColor );
  end
  else
  begin
...
begin
  ...
  if P.X + FPreviewPanel.Width > FarRightEdge then
    P.X := X - FPreviewWidth;
