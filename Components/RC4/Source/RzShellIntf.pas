...
{$IFDEF BCB4_OR_HIGHER}
  { HPPEMIT '#define __exdisp_h__'}
  { HPPEMIT '#include <shlobj.h>'}

  { HPPEMIT '#include <DsgnIntf.hpp>'}
  { HPPEMIT '#include <ShlObj.hpp>'}
  { HPPEMIT '#include <Utilcls.h>'}
...
type
  TStrRet = packed record
    ...
    case Integer of
      ...
      2: ( cStr: array[ 0..MAX_PATH ] of Char ); // Buffer to fill in
  end;
  PStrRet = ^TStrRet;
...
type
  TCMInvokeCommandInfo = packed record
    ...
    lpVerb: PChar;                          // either a string of MAKEINTRESOURCE(idOffset)
    lpParameters: PChar;                    // might be NULL (indicating no parameter)
    lpDirectory: PChar;                     // might be NULL (indicating no specific directory)
  ...
  public
    ...
    function GetCommandString( idCmd, uType: UINT; pwReserved: PUINT;
      pszName: PChar; cchMax: UINT ): HResult; virtual; stdcall; abstract;
  end;
