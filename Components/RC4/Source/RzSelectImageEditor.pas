...
type
  ...
  TRzCustomImageIndexProperty = class( TIntegerProperty )
  protected
    {** Descendant class must override GetImageList **}
    function GetImageList: TCustomImageList; virtual; abstract;
...
begin
  ...
  try
    ...
    {$ELSE}
    inherited ListDrawValue( Value, ACanvas, Rect( R.Right, ARect.Top, ARect.Right, ARect.Bottom ), ASelected );
    {$ENDIF}
  end;
end;
...
function TRzToolButtonEditor.MenuBitmapResourceName( Index: Integer ): string;
begin
  case Index of
    0: Result := 'RZDESIGNEDITORS_SELECT_IMAGE';
    1: Result := 'RZDESIGNEDITORS_SELECT_DISABLED_IMAGE';
    2: Result := 'RZDESIGNEDITORS_SELECT_HOT_IMAGE';
    5: Result := 'RZDESIGNEDITORS_IMAGELIST';
    8: Result := 'RZDESIGNEDITORS_TOOLSTYLE_BUTTON';
    9: Result := 'RZDESIGNEDITORS_TOOLSTYLE_DROPDOWN';
  end;
end;
...
procedure TRzToolButtonEditor.PrepareMenuItem( Index: Integer; const Item: TMenuItem );
  ...
  begin
    ...
    case Layout of
      blGlyphLeft:   NewItem.Bitmap.LoadFromResourceName( HInstance, 'RZDESIGNEDITORS_LAYOUT_IMAGE_LEFT' );
      blGlyphRight:  NewItem.Bitmap.LoadFromResourceName( HInstance, 'RZDESIGNEDITORS_LAYOUT_IMAGE_RIGHT' );
      blGlyphTop:    NewItem.Bitmap.LoadFromResourceName( HInstance, 'RZDESIGNEDITORS_LAYOUT_IMAGE_TOP' );
      blGlyphBottom: NewItem.Bitmap.LoadFromResourceName( HInstance, 'RZDESIGNEDITORS_LAYOUT_IMAGE_BOTTOM' );
    end;
...
function TRzBitBtnEditor.MenuBitmapResourceName( Index: Integer ): string;
begin
  case Index of
    0: Result := 'RZDESIGNEDITORS_SELECT_IMAGE';
...
function TRzRapidFireButtonEditor.MenuBitmapResourceName( Index: Integer ): string;
begin
  case Index of
    0: Result := 'RZDESIGNEDITORS_SELECT_IMAGE';
  end;
...
function TRzSelectImageEditor.MenuBitmapResourceName( Index: Integer ): string;
begin
  case Index of
    0: Result := 'RZDESIGNEDITORS_SELECT_IMAGE';
    1: Result := 'RZDESIGNEDITORS_IMAGELIST';
