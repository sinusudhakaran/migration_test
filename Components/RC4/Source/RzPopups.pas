...
function StrToDateEx( const S: string; var DateTime: TDateTime ): Boolean;
...
procedure DrawString( Canvas: TCanvas; const S: string; var Bounds: TRect; Flags: Cardinal );
begin
...
begin
  ...
  DrawString( Canvas, Caption, Bounds, dt_Center );
...
begin {= TRzCalendar.Paint =}
  ...
  begin
    ...
    begin
      ...
      DrawString( Canvas, '3', Areas[ caLeftArrow ], dt_Center );   // Draw Left Arrow
      DrawString( Canvas, '4', Areas[ caRightArrow ], dt_Center );  // Draw Right Arrow
    end;
    // Draw month and year
    ...
    if ceYear in FElements then
      F := F + ' yyyy';
    DrawString( Canvas, FormatDateTime( F, ViewDate ), Areas[ caYear ], dt_Center );
  end;
      ...
      begin
        ...
        begin
          ...
          if Canvas.TextWidth( S2 ) > ( R.Right - R.Left ) then
            DrawString( Canvas, S2[ 1 ], R, dt_Right or dt_NoClip )
          else
            DrawString( Canvas, S2, R, dt_Right or dt_NoClip )
        end
        else
          DrawString( Canvas, S, R, dt_Right or dt_NoClip );
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          DrawString( Canvas, IntToStr( Day ), R, dt_Right or dt_NoClip );
      ...
      begin
        ...
        begin
          ...
          DrawString( Canvas, IntToStr( WN ), WR, dt_Center );
        end
        else
          DrawString( Canvas, IntToStr( WeekOf( DrawDate - 6 ) ), WR, dt_Center );
...
begin
  ...
  for I := 1 to 7 do
  begin
    W := Canvas.TextWidth( ShortDayNames[ I ] );
    if W > Result then
      Result := W;
  end;
end;
...
begin {= TRzTimePicker.Paint =}
  ...
  begin
    ...
    Canvas.Font.Color := FButtonFontColor;
    DrawString( Canvas, FormatDateTime( FormatStr, FTime ), FTimeRect, dt_Center );
    Canvas.Brush.Style := bsSolid;
  end; { if FShowTime }
  ...
  begin
    if FCaptionSet <> '' then
      ...
      S := sRzCaptionSet;
    DrawButton( Canvas, FSetRect, S, FButtonColor, FButtonFontColor, DrawFlat,
...
begin
  ...
  begin
    ElementDetails := ThemeServices.GetElementDetails( teEditTextDisabled );
...
begin
  ...
  begin
    ...
    try
      ...
      Result := TRzPopup( FPopup ).Popup( FPopupControl, Alignment, X, Y );
