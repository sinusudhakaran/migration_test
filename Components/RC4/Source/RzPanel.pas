...
type
  ...
  {$ELSE}
  TRzCustomPanel = class( TCustomPanel, IUnknown, IRzCustomFramingNotification )
  {$ENDIF}
  private
...
begin
  ...
  begin
    ...
    TextRct := Rect;
    H := DrawText( Canvas.Handle, PChar( Caption ), -1, TextRct,
    ...
    DrawText( Canvas.Handle, PChar( Caption ), -1, Rect,
              dt_ExpandTabs or dt_VCenter or
              DrawTextWordWrap[ WordWrap ] or DrawTextAlignments[ Alignment ] );
  end;
...
var
  R: TRect;
  StartColor, StopColor, FC: TColor;
  FCA: Integer;
begin
  R := GetControlRect;
...
var
  ...
  begin
    Result := Rect( 0, 0, 0, 0 );
    GetThemeTextExtent( ThemeServices.Theme[ teButton ], DC, Details.Part, Details.State,
...
begin
  ...
  begin
    ...
    begin
      ...
      Canvas.Brush.Style := bsClear;
      if Enabled and ( FVisualStyle = vsWinXP ) and ThemeServices.ThemesEnabled then
      begin
      ...
      SelectClipRgn( Canvas.Handle, 0 );
      DrawText( Canvas.Handle, PChar( Caption ), -1, CaptionRect,
                DrawTextAlignments[ Alignment ] or dt_SingleLine );
    end;
  end;
...
begin {= TRzToolbarPopupButton.DrawBackground =}
  ...
  case FToolbar.VisualStyle of
    vsClassic, vsWinXP:
    begin
      if ( FToolbar.VisualStyle = vsWinXP ) and ThemeServices.ThemesEnabled then
      begin
        ElementDetails := ThemeServices.GetElementDetails( trRebarRoot );
        ThemeServices.DrawElement( Canvas.Handle, ElementDetails, R );

        if Down then
          ElementDetails := ThemeServices.GetElementDetails( trChevronPressed )
        else
          ElementDetails := ThemeServices.GetElementDetails( trChevronNormal );
        ThemeServices.DrawElement( Canvas.Handle, ElementDetails, R );
      end
      else
      begin
...
begin
  ...
  if not ( ( FToolbar.VisualStyle = vsWinXP ) and ThemeServices.ThemesEnabled ) then
    DrawChevron( R );
...
begin
  if ( FOrientation = orHorizontal ) and ( not FWrapControls ) and
     FShowToolbarPopupButton then
  begin
