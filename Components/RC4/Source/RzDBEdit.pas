...
begin
  ...
  begin
    Result := ( Key in [ '0'..'9', 'E', 'e', '+', '-', DecimalSeparator, '(', ')' ] )
              or
              ( ( Key < #32 ) and ( Key <> Chr( vk_Return ) ) );
  end;
...
var
  ...
  I: Byte;
  IsNeg: Boolean;
  PosExp, PosMinus, PosOpenParen: Integer;
begin
  ...
  begin
    ...
    PosExp := Pos( 'E', Text );
    PosMinus := Pos( '-', Text );
    PosOpenParen := Pos( '(', Text );

    IsNeg := ( ( PosExp = 0 ) and ( PosMinus > 0 ) )
             or
             ( ( PosExp > 0 ) and ( PosMinus > 0 ) and ( PosMinus < PosExp ) )
             or
             ( PosOpenParen > 0 );
    ...
    begin
      C := Text[ I ];
      if C in [ '0'..'9', 'E', 'e', '-', DecimalSeparator ] then
      begin
        if C = '-' then
        begin
          if ( I > 1 ) and ( Text[ I - 1 ] in [ 'E', 'e' ] ) then
            S := S + C;
        end
        else
          S := S + C;
...
begin
  if ReadOnly and ( Key in [ '+', '=', '-', '_' ] ) then
    Key := #0
  else if ( FEditType = etTime ) and not ( Key in [ #8, #13, #27 ] ) and IsCharAlpha( Key ) and
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Result := False; // Do not let CMExit to call inherited
        SetFocus;
...
procedure TRzDBDateTimeEdit.HideButton;
begin
  DropButtonVisible := False;
end;


procedure TRzDBDateTimeEdit.ShowButton;
begin
  DropButtonVisible := True;
end;
