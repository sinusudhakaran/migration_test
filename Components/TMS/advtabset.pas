...
begin
  ...
  begin
    ...
    begin
      ...
      {$ENDIF}
      {$IFDEF TMSDOTNET}
      TTabCollection(Collection).AdvTabSet.DuplicateTabs.Objects[i] := TObject(integer(TTabCollection(Collection).AdvTabSet.DuplicateTabs.Objects[i])-1);
      {$ENDIF}
...
//------------------------------------------------------------------------------
procedure TTabCollectionItem.SetCaption(const Value: string);
begin
  FCaption := Value;
...
begin
  ...
  begin
    ...
    {$ENDIF}
    {$IFDEF TMSDOTNET}
    AdvTabSet.DuplicateTabs.Objects[Items[i].FIndexInAdvTabSet] := TObject(Items[i].Index);
    {$ENDIF}
  end;
end;
  ...
  while (Start < Stop) and (Index < Tabs.Count) do
    ...
    begin
      ...
      begin
        ...
        {$ENDIF}
        {$IFDEF TMSDOTNET}
        TabPositions.Add(TObject(TabPos));    { add to list }
        {$ENDIF}
        Inc(Index);
      end;
    end;
...
begin
  ...
  begin
    ...
    {$ENDIF}
    {$IFDEF TMSDOTNET}
    TabPos := TTabPos(TabPositions[Item]);
    {$ENDIF}
...
begin
  ...
  begin
    ...
    {$ENDIF}
    {$IFDEF TMSDOTNET}
    TabPos := TTabPos(TabPositions[Tab]);
    {$ENDIF}
...
//    RealItemIndex := integer(FDuplicateTabs.Objects[Tab + FirstIndex]);
    ...
    if TabPosition in [pTop, pBottom] then
      ...
      {$ENDIF}
      {$IFDEF TMSDOTNET}
      DrawText(MemBitmap.Canvas.Handle, FAdvTabs.Items[RealItemIndex].Caption, Length(FAdvTabs.Items[RealItemIndex].Caption), TextR, DT_LEFT)
      {$ENDIF}
      ...
      {$ENDIF}
      {$IFDEF TMSDOTNET}
      MemBitmap.Canvas.TextOut(TextR.Left, TextR.Bottom,FAdvTabs.Items[RealItemIndex].Caption)
      {$ENDIF}
      ...
      {$ENDIF}
      {$IFDEF TMSDOTNET}
      MemBitmap.Canvas.TextOut(TextR.Left+ MemBitmap.Canvas.TextHeight('T'), TextR.Top,FAdvTabs.Items[RealItemIndex].Caption);
      {$ENDIF}
...
var
  ...
  var
    ...
    TempX: Integer;
  begin
    if FTabPosition in [pLeft, pRight] then
    begin
  ...
  begin
    ...
    begin
      { Draw the top line }
      Pen.Color := clBtnShadow;
      DrawPoly(Canvas, False, [Point(0, Y), Point(EdgeWidth + 1, Y)]);
      { Fill in the middle }
      Pen.Color := UnselectedColor;
      Brush.Color := UnselectedColor;

      DrawPoly(Canvas, True, [Point(3, Y + YMod),
        Point(EdgeWidth - 1, YEnd), Point(EdgeWidth, YEnd),
        Point(EdgeWidth, Y + YMod), Point(3, Y + YMod)]);

      if SoftTop then
      begin
        Pen.Color := BackgroundColor;
        DrawPoly(Canvas, False, [Point(4, Y + YMod),
          Point(EdgeWidth + 1, Y + YMod)]);

        DrawPoly(Canvas, False, [Point(3, Y + YMod),
          Point(EdgeWidth - 1, YEnd),
          Point(EdgeWidth, YEnd)]);
      end
      else
      begin
        Pen.Color := clWindowFrame;
  ...
  begin
    ...
    begin
      Pen.Color := clBtnShadow;
      DrawPoly(Canvas, False, [Point(-1, Y), Point(EdgeWidth + 1, Y)]);

      Pen.Color := UnselectedColor;
      Brush.Color := UnselectedColor;

      DrawPoly(Canvas, True, [Point(EdgeWidth - 3, Y + YMod),
        Point(1, YEnd), Point(0, YEnd),
        Point(0, Y + YMod), Point(EdgeWidth - 3, Y + YMod)]);
      ...
      begin
        ...
        DrawPoly(Canvas, False, [Point(EdgeWidth - 2, Y + YMod),
          Point(2, YEnd),
          Point(-1, YEnd)]);
      end
      else
  ...
  begin
    ...
    begin
      ...
      Pen.Color := clBtnShadow;
      DrawPoly(Canvas, False, [Point(EdgeWidth + 1, Y),
          Point(EdgeWidth - 3, Y),
          Point(EdgeWidth - 3, Y + YMod),
          Point(1, YEnd),
          Point(-1, YEnd)]);
        Pen.Color := clWindowFrame;
      ...
      if SoftTop then
        ...
        DrawPoly(Canvas, False, [Point(EdgeWidth, Y + YMod),
          Point(EdgeWidth - 2, Y + YMod),
          Point(2, YEnd),
          Point(-1, YEnd)]);
    end;
...
begin
  ...
  try
    ...
    begin
      FTabHeight := MemBitmap.Canvas.TextHeight('T') + 4;
    ...
    if FTabPosition in [pBottom, pTop] then
    begin
      ActualWidth := EdgeWidth;
  ...
  try
    ...
    if FTabPosition in [pBottom, pRight] then
    begin
      Y := 0;
      YMod := 1;
      YEnd := H;
    ...
    begin
      ...
      begin
        Brush.Color := MaskColor;
        Brush.Style := bsSolid;
        FillRect(Types.Rect(0, 0, ActualWidth, ActualHeight));
      end;
      case EdgePart of
        epSelectedLeft: DrawSL(Working.Canvas);
        epUnselectedLeft: DrawUL(Working.Canvas);
...
begin
  ...
  begin
    Font.Assign(self.Font);
    if TabPosition in [pTop, pBottom] then
      W1 := TextWidth(Tabs[Index]) + TabMargin.RightMargin + TabMargin.LeftMargin - 4 + ImgW
    else
      W1 := TextWidth(Tabs[Index]) + TabMargin.RightMargin + TabMargin.TopMargin - 4 + ImgW;

    Font.Assign(self.ActiveFont);
    if TabPosition in [pTop, pBottom] then
      W2 := TextWidth(Tabs[Index]) + TabMargin.RightMargin + TabMargin.LeftMargin - 4 + ImgW
    else
      W2 := TextWidth(Tabs[Index]) + TabMargin.RightMargin + TabMargin.TopMargin - 4 + ImgW;
