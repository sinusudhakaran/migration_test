...
type
  ...
  {$ENDIF}
  {$IFDEF TMSDOTNET}
  TDBAdvStringGrid = class(TAdvStringGrid, IDataControl)
  {$ENDIF}
  private
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if ((dt in [ftMemo,ftFmtMemo{$IFDEF DELPHI5_LVL},ftOraCLOB{$ENDIF}]) or ((dt = ftBlob) and ((Fields[FieldIdx] as TBlobField).BlobType = ftMemo))) and FShowMemoFields then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (dt in [ftMemo,ftFmtMemo{$IFDEF DELPHI5_LVL},ftOraCLOB{$ENDIF}]) and FShowMemoFields then
              begin
...
begin
  ...
  begin
    ...
    if Assigned(Fld) then
    begin
      if ((Fld.DataType in [ftMemo,ftFmtMemo{$IFDEF DELPHI5_LVL},ftOraCLOB{$ENDIF}]) or
...
{$IFDEF TMSDOTNET}
function TDBAdvStringGrid.GetDataLink: TDataLink;
begin
  Result := FDataLink;
end;
{$ENDIF}
