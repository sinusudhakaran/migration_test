...
begin
  ...
  begin
    if FDisplayCaptionTabsCount <= 0 then
      ...
      FSeparatorLine := CaptionTabRect(FAdvNavBarPanel.Count - 1).top - (SplitterHeight div 2); //Height - (FDisplayCaptionTabsCount*(FCaptionTabHeight))- (SplitterHeight div 2);
  end
  else
  begin
    if FDisplayCaptionTabsCount <= 0 then
      ...
      FSeparatorLine := CaptionTabRect(FAdvNavBarPanel.Count - 1).Bottom + (SplitterHeight div 2); //Height - (FDisplayCaptionTabsCount*(FCaptionTabHeight))- (SplitterHeight div 2);
  end;
...
begin
  ...
  begin
    ...
    else if (i < 0) and (FHoverTabIndex >= 0) then
    begin
      j := FHoverTabIndex;
      FHoverTabIndex := -1;
      RefreshCaptionTabOrButton(j);
...
begin
  ...
  begin
    if ((FSeparatorLine < CaptionHeight + CaptionTabHeight + (SplitterHeight div 2) + 2 + MinClientHeight) and (FDefaultTabPosition = tpBottom))
      or ((FSeparatorLine > Height - CaptionHeight - CaptionTabHeight - (SplitterHeight div 2) - 2 - MinClientHeight) and (FDefaultTabPosition = tpTop)) then
      Exit;
