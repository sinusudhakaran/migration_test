...
begin
  ...
  {$ENDIF}
  {$IFDEF TMSDOTNET}
  Result := AnsiCompareStr(HTMLStrip(List.Strings[Index1]),HTMLStrip(List.Strings[Index2]));
  {$ENDIF}
...
{$IFDEF TMSDOTNET}
procedure THTMLComboBox.CMHintShow(var Message: TCMHintShow);
var
  CanShow: Boolean;
  hi: THintInfo;
  Anchor: string;
...
{$IFDEF TMSDOTNET}
procedure TBaseCustomComboBox.WndProc(var Message: TMessage);
begin
  inherited;
