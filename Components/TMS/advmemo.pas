...
{$ENDIF}
  ...
  TextFromPos({cx}CurX,{cy}CurY,ss);  // khn:
...
//  if not (Key in [VK_UP,VK_DOWN,VK_LEFT,VK_RIGHT,VK_MENU,VK_CONTROL,VK_NEXT,VK_PRIOR,VK_HOME,VK_END]) then
  ...
  case Key of
    ...
    //ord('Z'): if ssctrl in Shift then Undo;
...
begin
  ...
  if Message.XPos < FGutter.GutterWidth then
    Exit;
...
{$IFNDEF TMSDOTNET}
    if CanUndo and not ReadOnly then
      InsertMenu(popmenu, $FFFFFFFF, MF_BYPOSITION, 5, pchar('Undo'))
    else
      InsertMenu(popmenu, $FFFFFFFF, MF_BYPOSITION or MF_GRAYED, 5, pchar('Undo'));

    if CanRedo and not ReadOnly then
      InsertMenu(popmenu, $FFFFFFFF, MF_BYPOSITION, 6, pchar('Redo'))
    else
      InsertMenu(popmenu, $FFFFFFFF, MF_BYPOSITION or MF_GRAYED, 5, pchar('Redo'));
...
{$IFDEF TMSDOTNET}
    if CanUndo and not ReadOnly then
      InsertMenu(popmenu, $FFFFFFFF, MF_BYPOSITION, 5, 'Undo')
    else
      InsertMenu(popmenu, $FFFFFFFF, MF_BYPOSITION or MF_GRAYED, 5, 'Undo');

    if CanRedo and not ReadOnly then
      InsertMenu(popmenu, $FFFFFFFF, MF_BYPOSITION, 6, 'Redo')
    else
      InsertMenu(popmenu, $FFFFFFFF, MF_BYPOSITION or MF_GRAYED, 5, 'Redo');
...
{$ENDIF}
  ...
  begin
    ...
    if (FActiveLine > TopLine + VisibleLineCount) or (FActiveLine < TopLine) then
      TopLine := Value; // make sure setting active line makes it visible
