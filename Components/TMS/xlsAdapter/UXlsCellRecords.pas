...
type
  ...
  public
    ...
    procedure SaveFirstMul(const Workbook: TStream; const JoinedRecordSize: word);override;
    procedure SaveMidMul(const Workbook: TStream);override;
    procedure SaveLastMul(const Workbook: TStream);override;
...
procedure TRKRecord.SaveFirstMul(const Workbook: TStream;
  const JoinedRecordSize: word);
var
  x: word;
  r: PArrayOfByte;
begin
  x:=xlr_MULRK; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
  x:=JoinedRecordSize-SizeOf(TRecordHeader); if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
  x:=Row; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
  x:=Column; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
  r:= @Data[4]; if Workbook.Write(r^, 6) <> 6 then raise Exception.Create(ErrCantWrite);
end;

procedure TRKRecord.SaveLastMul(const Workbook: TStream);
var
  x: word;
  r: PArrayOfByte;
begin
...
procedure TRKRecord.SaveMidMul(const Workbook: TStream);
var
  r: PArrayOfByte;
begin
  inherited;
...
procedure TBlankRecord.SaveFirstMul(const Workbook: TStream;
  const JoinedRecordSize: word);
var
  x: word;
begin
  x:=xlr_MULBLANK; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
  x:=JoinedRecordSize-SizeOf(TRecordHeader); if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
  x:=Row; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
  x:=Column; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
  x:=XF; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
end;

procedure TBlankRecord.SaveLastMul(const Workbook: TStream);
var
  x: word;
begin
  x:=XF; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
...
procedure TBlankRecord.SaveMidMul(const Workbook: TStream);
var
  x: word;
begin
  x:=XF; if Workbook.Write(x, Sizeof(x)) <> Sizeof(x) then raise Exception.Create(ErrCantWrite);
