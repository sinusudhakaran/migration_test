...
interface
uses UXlsBaseRecords, UXlsBaseList, XlsMessages, Classes, SysUtils, UFlxMessages;
...
procedure THPageBreakList.CalcIncludedRangeRecords(const CellRange: TXlsCellRange; var FirstRecord, RecordCount: integer);
var
...
procedure THPageBreakList.SaveToStreamExt(const DataStream: TStream; const FirstRecord, RecordCount: integer);
var
...
begin
  ...
  begin
    ...
    if DataStream.Write(MyRecordCount, Sizeof(MyRecordCount)) <> Sizeof(MyRecordCount) then raise Exception.Create(ErrCantWrite);
...
procedure THPageBreakList.SaveRangeToStream(const DataStream: TStream; const CellRange: TXlsCellRange);
var
...
procedure THPageBreakList.SaveToStream(const DataStream: TStream);
begin
...
procedure TVPageBreakList.CalcIncludedRangeRecords(const CellRange: TXlsCellRange; var FirstRecord, RecordCount: integer);
var
  LastRecord, i: integer;
begin
  Sort; //just in case
...
procedure TVPageBreakList.SaveToStreamExt(const DataStream: TStream; const FirstRecord, RecordCount: integer);
var
  RecordHeader: TRecordHeader;
  MyRecordCount: word;
  i: integer;
...
procedure TVPageBreakList.SaveRangeToStream(const DataStream: TStream; const CellRange: TXlsCellRange);
var
  FirstRecord, RecordCount: integer;
begin
  CalcIncludedRangeRecords(CellRange, FirstRecord, RecordCount);
...
procedure TVPageBreakList.SaveToStream(const DataStream: TStream);
begin
  SaveToStreamExt(DataStream, 0, Count);
end;
