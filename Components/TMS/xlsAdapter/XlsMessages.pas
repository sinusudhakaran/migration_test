...
// Escher records
  ...
  procedure GetSimpleString(const Length16Bit: boolean ;const Pdata: PArrayOfByte; const tPos: integer;const UseExtStrLen: boolean; const ExtStrLen: LongWord; var St: widestring; var StSize: integer);
...
begin
  ...
  for i:=1 to length(w) do Result[i]:=Chr(Ord(w[i]) and $FF);
end;
...
begin
  ...
  for i:=1 to length(s) do Result[i]:=WideChar(Ord(s[i]));
end;
