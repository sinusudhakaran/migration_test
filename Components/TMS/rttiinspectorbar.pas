...
begin
  ...
  begin
    ...
    begin
      ...
      InspectorItem.TextValue := SetToString(RTTIItem.PropInfo, GetOrdProp(AObject,RTTIItem.PropInfo.Name),True);
    end;
  end;
      ...
      begin
        ...
        if Assigned(TRTTIInspectorBar(InspectorBar).OnItemInsert) then
          TRTTIInspectorBar(InspectorBar).OnItemInsert(InspectorBar, APropName,
             PropList^[i - 1].PropType^.Kind, Allow);
        ...
        begin
          ...
          begin
            ...
            RTTIItem.BoolValue := GetOrdProp(AObject,PropList^[I - 1].Name) <> 0;
          ...
          if PropList^[I - 1].PropType^.Kind in [tkFloat] then
          begin
            RTTIItem.PropertyType := ptFloat;
            RTTIItem.TextValue := Format('%g',[GetFloatProp(AObject,PropList^[I - 1].Name)]);
          end;
