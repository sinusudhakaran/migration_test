...
begin
  ...
  for ColIndex := FromCol to ToCol do
    ...
    begin
      ...
      RectLineExExEx(ACanvas, DrawRect, GridLineColor, 1);
            ...
            begin
              ...
              begin
                ...
                HtmlText := HtmlText
                  ...
                  + '<div style="overflow:hidden;width:' + IntToStr(Display.DisplayScale - 2)
                  + '">&nbsp;'+'</div></td>'#13#10
              end
              else
              begin
                ...
                begin
                  ...
                  begin
                    if (NumDown > 1) then
                      ...
                      HtmlText := HtmlText + '<td height="' + IntToStr(HeightCell) + 'px"'  + Colspan + '>&nbsp;</td>';
                  end;
                end;
              end;
            end;
...
begin
  ...
  begin
    ...
    begin
      Footer.Captions[j] := IntToStr(GetPositionCompletion(i -1,Footer.CompletionType = ctActiveTime));
    end;
  end;
...
begin
  ...
  begin
    ...
    if (ARow - RowOffset < FColorList.Count) and (ARow - RowOffset >= 0) and
      FPlanner.ShowSelection and (AColumn < NumColors) then
    begin
 ...
 { Handles horizontal planner mode }
  begin
    ...
    begin
      ...
      begin
        FMouseDownMoveFirst := False;
        FPlanner.CreateItem.Assign(FPlanner.Items.Selected);
      end;
...
begin
  ...
  if (FPlanner.Sidebar.Position in [spLeft,spLeftRight,spTop]) and (FPlanner.FPositionGap > 0) and FPlanner.GapAtColumn(X) then
  begin
...
begin
  ...
  begin
    m := 0;
    if InVisibleLayer(Items[i - 1].Layer) and Items[i - 1].Visible and not (Items[i - 1].Background and Items[i - 1].AllowOverlap) then
      for j := 1 to Count do
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                {$IFNDEF TMSDOTNET}
                DrawTextExW(Canvas.Handle, PWideChar(GroupWideCaption), Length(GroupWideCaption), r, DrawGroupFlags, nil);
                {$ENDIF}
              end
              else
              begin
                ...
                {$ENDIF}
                {$IFDEF TMSDOTNET}
                DrawText(Canvas.Handle, s, Length(s), r, DrawGroupFlags);
                {$ENDIF}
...
//          if ((SectionIndex - 1 + FLeftPos) mod Planner.PositionGroup = 0) or AllPainted then
          ...
          begin
            ...
            if Planner.Header.RotateOnLeft and (Planner.SideBar.Position = spTop) then
            begin
              //if Planner.Header.GroupCaptions.Count > GroupIdx then
              //begin
