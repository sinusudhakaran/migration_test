        ...
        begin
          ...
          begin
            ...
            begin
              ...
              {$ENDIF}
              {$IFDEF TMSDOTNET}
              DrawTextEx(Canvas.Handle,su,WordLen,cr,DrawStyle,nil);
              {$ENDIF}
              Offsetrect(cr,-ShadowOffset,-ShadowOffset);
          ...
          begin
            ...
            {$ENDIF}
            {$IFDEF TMSDOTNET}
            DrawText(Canvas.Handle,su,WordLenEx,cr,DrawStyle or DT_CALCRECT);
            {$ENDIF}
            ...
            begin
              ...
              {$IFDEF TMSDOTNET}
              dtp.cbSize := System.Runtime.InteropServices.Marshal.SizeOf(TypeOf(dtp));
              {$ENDIF}
              {$IFNDEF TMSDOTNET}
              ...
              {$ENDIF}
              {$IFDEF TMSDOTNET}
              GetTextExtentExPoint(Canvas.handle,su,length(su),xpos-w,nnfit,nil,nnSize);
              {$ENDIF}
              {$ELSE}
               ...
               begin
                ...
                if ipos('res://',prop)>0 then
                  begin
                    ...
                    {$ENDIF}
                    {$IFDEF TMSDOTNET}
                    if findresource(hinstance,prop,RT_BITMAP)<>0 then
                    {$ENDIF}
                    begin
                    bmp:=tbitmap.Create;
                    try
