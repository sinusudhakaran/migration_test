...
begin
  ...
  {$ENDIF}
  {$IFDEF TMSDOTNET}
  if (Integer((Sender as TSpeedButton).Tag) in [10..14,16,17,18,19..22]) then
  {$ENDIF}
    FDecim := 0;

  if (Integer((Sender as TSpeedButton).Tag) in [0..9,14,15]) then
  begin
  ...
  begin
   ...
   {$ENDIF}
   {$IFDEF TMSDOTNET}
   case Integer((sender as TSpeedButton).tag) of
   {$ENDIF}
