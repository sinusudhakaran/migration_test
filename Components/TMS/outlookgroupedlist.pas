...
type
  ...
  published
    ...
    property OnItemDblClick: TOGLItemEvent
...
begin
  for Result := Low(Result) to High(Result) do
    if aCharCode[Result] = CharCode then Exit;
end;
{$WARNINGS ON}
...
begin
  ...
  ClearSelectionBeforeMove :=
    (not (ssShift in Shift)) and (not (ssCtrl in Shift));
...
begin
  ...
  begin
    ...
    if HitInfo.HitTest in [htOnItem, htOnItemLabel] then
      FOnItemDblClick(Self, HitInfo.HitItem);
  end;
end;
...
var
  ...
  begin
    ...
    begin
      // Child item
      Index := FFocusedItem.Index - FItemsPerRow;
      if Index >= 0 then
...
begin
  ...
  if (FFocusedItem <> nil) and (not IsGroupItem(FFocusedItem)) then
    FOGLItemColumn := FFocusedItem.Index mod FItemsPerRow;
end;
...
begin
  ...
  try
    ...
    if (DragEffect = DROPEFFECT_MOVE) and
