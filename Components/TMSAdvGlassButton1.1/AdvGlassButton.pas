...
type
  ...
  TBackGroundSymbol = (bsArrowLeft, bsArrowRight, bsArrowUp, bsArrowDown, bsPlay, bsPause, bsStop, bsFastForward, bsForward,
                      bsBackward, bsFastBackward, bsSpeaker, bsNoSpeaker, bsNone);
...
{$IFDEF TMSDOTNET}
    procedure ButtonPressed(Group: Integer; Button: TAdvCustomGlassButton);
{$ENDIF}
  ...
  begin
   ...
   // stringFormat.SetAlignment(StringAlignmentCenter);
    ...
    begin
      ...
      szRect.Bottom := DrawText(Canvas.Handle,PChar(DCaption),Length(DCaption), szrect, DT_CALCRECT or DT_LEFT or DT_SINGLELINE{ or DT_VCENTER});
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            {R3 := Rect(R3.Left - Left, R3.Top - Top, R3.Right - Left, R3.Bottom - Top);
            bmp := TBitmap.Create;
            bmp.Height := Height;
            bmp.Width := Width;
            rgn1 :=  CreateRectRgn(R3.Left, R3.Top, R3.Right, R3.Bottom);
            SelectClipRgn(bmp.Canvas.Handle, rgn1);

            i := SaveDC(bmp.Canvas.Handle);
            p := ClientOrigin;
            Windows.ScreenToClient(TWinControl(Parent.Controls[j]).Handle, p);
            p.x := -p.x;
            p.y := -p.y;
            MoveWindowOrg(bmp.Canvas.Handle, p.x, p.y);

            SendMessage(TWinControl(Parent.Controls[j]).Handle, WM_ERASEBKGND, bmp.Canvas.Handle, 0);
            // transparency ?
            SendMessage(TWinControl(Parent.Controls[j]).Handle, WM_PAINT, bmp.Canvas.Handle, 0);

            //if (Parent.Controls[j] is TWinCtrl) then
             //TProWinCtrl(Parent.Controls[j]).PaintControls(Canvas.Handle, nil);

            RestoreDC(bmp.Canvas.Handle, i);

            Canvas.CopyMode := cmSrcCopy;
            Canvas.CopyRect(R3, bmp.Canvas, R3);//BrushCopy(R3, bmp, R3, ColorToRGB(clBlack));
...
begin
  ...
  if not Enabled and not PictureDisabled.Empty then
    Pic := PictureDisabled
  else if (FMouseDownInControl and FMouseInControl) and not PictureDown.Empty then
    Pic := PictureDown
...
procedure TAdvCustomGlassButton.ButtonPressed(Group: Integer; Button: TAdvCustomGlassButton);
begin
