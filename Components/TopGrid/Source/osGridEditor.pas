...
{$ENDIF}
  ...
  TfmOsGridEditor = class(TForm)
    ...
    miShowInvisibleColumns: TMenuItem;
    ...
    procedure gdComboDataKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure miShowInvisibleColumnsClick(Sender: TObject);
...
begin
  ...
  if (iPos > 0) then
  begin
    System.Delete(sName, Pos('->', sName), 2);
    System.Insert('.', sName, iPos);
  end;
  lsDataSources.AddObject(sName, aDs)
end;
...
{$ENDIF}
        ...
        if (SourceGrid is TtsDbGrid) then
        begin
          bVisibleProp := TtsDbGrid(Component).Visible;
          TtsDbGrid(Component).Visible := True;
        end
        else
        begin
          bVisibleProp := TtsGrid(Component).Visible;
          TtsGrid(Component).Visible := True;
        end;
        ...
        if (SourceGrid is TtsDbGrid) then TtsDbGrid(Component).Visible := bVisibleProp
        else TtsGrid(Component).Visible := bVisibleProp;        
    finally
        srcFieldNames.Free;
        srcFieldNames := Nil;
        lsDateTimeDefs.Free;
...
begin
  SelectedtsCol.Tag := 0;
  pnIndicator.Color := clGray;
  SelectedtsCol.Visible := True;
end;

procedure TfmOsGridEditor.HideColumn;
begin
  SelectedtsCol.Tag := 1;
  pnIndicator.Color := clGreen;
  SelectedtsCol.Visible := False;
end;
...
begin
  ...
  begin
    ...
    if GetGridCol(GetGridDataColnr(DataCol)).Tag = 1 then
       pnIndicator.Color := clGray
    else
       pnIndicator.Color := clGreen;
  end;
...
begin
  ...
  begin
    ...
    if (theCol.Visible) or
       (Self.miShowInvisibleColumns.Tag = 1) then
...
procedure TfmOsGridEditor.miInsertColumnClick(Sender: TObject);
var iColPos : Integer;
begin
  iColPos := -1;
  if SelectedtsCol <> Nil then
     iColPos := SelectedtsCol.DisplayCol;

  demoGrid.BeginUpdate;
  try
    AddBlankColumn;
    if iColPos > 0 then
       Self.SelectedtsCol.DisplayCol := iColPos;
    MoveTo(SelectedtsCol.DisplayCol, GetGridCurrentDataRow);
  finally
    demoGrid.EndUpdate;
  end;
...
procedure TfmOsGridEditor.miShowInvisibleColumnsClick(Sender: TObject);
var i : Integer;
begin
  demoGrid.BeginUpdate;
  try
    if miShowInvisibleColumns.Tag = 0 then
    begin
       demoGrid.BeginUpdate;
       try
         for i := 1 to Self.GetGridCols do
           Self.GetGridCol(i).Visible := True;
       finally
         demoGrid.EndUpdate;
       end;
       miShowInvisibleColumns.Tag := 1;
       miShowInvisibleColumns.Caption := 'Hide &Invisible Columns';
    end
    else
    begin
       demoGrid.BeginUpdate;
       try
         for i := 1 to Self.GetGridCols do
           if GetGridCol(i).Tag = 1 then
              Self.GetGridCol(i).Visible := False;
       finally
         demoGrid.EndUpdate;
       end;
       miShowInvisibleColumns.Tag := 0;
       miShowInvisibleColumns.Caption := 'Show &Invisible Columns';
    end;
  finally
    demoGrid.EndUpdate;
    if (not SelectedtsCol.Visible) and
       (miShowInvisibleColumns.Tag = 0) then
    begin
...
procedure TfmOsGridEditor.pnTopMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var iCol, iRow : Integer;
begin
  demoGrid.CellFromXY(X, Y, iCol, iRow);
  if demoGrid is TtsGrid then
     SelectedCol := TtsGrid(demoGrid).DataColnr[iCol]
  else
     SelectedCol := TtsDbGrid(demoGrid).DataColnr[iCol];
end;
