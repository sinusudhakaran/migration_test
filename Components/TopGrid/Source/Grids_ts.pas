...
type
  ...
  TCustomGrid_ts = class;

  TInplaceEdit = class(TCustomMaskEdit)
  private
    FGrid: TCustomGrid_ts;
    FClickTime: Longint;
    procedure InternalMove(const Loc: TRect; Redraw: Boolean);
    procedure SetGrid(Value: TCustomGrid_ts);
  ...
  protected
    ...
    property  Grid: TCustomGrid_ts read FGrid;
  public
  ...
  { TCustomGrid_ts is an abstract base class that can be used to implement
    ...
    Protected members, for implementors of TCustomGrid_ts descendents
      DesignOptionBoost
  ...
  TCustomGrid_ts = class(TCustomControl)
  private
    FAnchor: TGridCoord;
    ...
    //TopGrid adjustment: move procedure to protected area
  ...
  protected
    ...
    //TopGrid adjustment: procedures copied from private area and set to virtual
    procedure MoveTopLeft(ALeft, ATop: Longint); virtual;
    procedure ScrollDataInfo(DX, DY: Integer; var DrawInfo: TGridDrawInfo); virtual;
    procedure TopLeftMoved(const OldTopLeft: TGridCoord); virtual;
  ...
  TCustomDrawGrid = class(TCustomGrid_ts)
  private
  ...
  { TStringGrid_ts adds to TDrawGrid the ability to save a string and associated
  ...
  TStringGrid_ts = class;

  TStringGrid_tsStrings = class(TStrings)
  private
    FGrid: TStringGrid_ts;
    FIndex: Integer;
  ...
  public
    constructor Create(AGrid: TStringGrid_ts; AIndex: Longint);
  ...
  TStringGrid_ts = class(TDrawGrid)
  private
    FData: Pointer;
    FRows: Pointer;
    FCols: Pointer;
    ...
    function EnsureColRow(Index: Integer; IsCol: Boolean): TStringGrid_tsStrings;
...
procedure TInplaceEdit.SetGrid(Value: TCustomGrid_ts);
begin
  FGrid := Value;
end;
...
constructor TCustomGrid_ts.Create(AOwner: TComponent);
const
...
destructor TCustomGrid_ts.Destroy;
begin
  FInplaceEdit.Free;
  inherited Destroy;
  FreeMem(FColWidths);
...
procedure TCustomGrid_ts.AdjustSize(Index, Amount: Longint; Rows: Boolean);
var
  NewCur: TGridCoord;
...
function TCustomGrid_ts.BoxRect(ALeft, ATop, ARight, ABottom: Longint): TRect;
var
  GridRect: TGridRect;
begin
  GridRect.Left := ALeft;
...
procedure TCustomGrid_ts.DoExit;
begin
  inherited DoExit;
  if not (goAlwaysShowEditor in Options) then HideEditor;
end;

function TCustomGrid_ts.CellRect(ACol, ARow: Longint): TRect;
begin
  Result := BoxRect(ACol, ARow, ACol, ARow);
end;

function TCustomGrid_ts.CanEditAcceptKey(Key: Char): Boolean;
begin
  Result := True;
end;

function TCustomGrid_ts.CanGridAcceptKey(Key: Word; Shift: TShiftState): Boolean;
begin
  Result := True;
end;

function TCustomGrid_ts.CanEditModify: Boolean;
begin
  Result := FCanEditModify;
end;

function TCustomGrid_ts.CanEditShow: Boolean;
begin
...
function TCustomGrid_ts.IsActiveControl: Boolean;
var
  H: Hwnd;
...
function TCustomGrid_ts.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
end;

function TCustomGrid_ts.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
end;

procedure TCustomGrid_ts.SetEditText(ACol, ARow: Longint; const Value: string);
begin
end;

function TCustomGrid_ts.GetEditLimit: Integer;
begin
  Result := 0;
end;

function TCustomGrid_ts.GetEditStyle(ACol, ARow: Longint): TEditStyle;
begin
  Result := esSimple;
end;

procedure TCustomGrid_ts.HideEditor;
begin
  FEditorMode := False;
  HideEdit;
end;

procedure TCustomGrid_ts.ShowEditor;
begin
  FEditorMode := True;
  UpdateEdit;
end;

procedure TCustomGrid_ts.ShowEditorChar(Ch: Char);
begin
  ShowEditor;
...
procedure TCustomGrid_ts.InvalidateEditor;
begin
  FInplaceCol := -1;
  FInplaceRow := -1;
  UpdateEdit;
...
procedure TCustomGrid_ts.ReadColWidths(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TCustomGrid_ts.ReadRowHeights(Reader: TReader);
var
  I: Integer;
begin
  with Reader do
...
procedure TCustomGrid_ts.WriteColWidths(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TCustomGrid_ts.WriteRowHeights(Writer: TWriter);
var
  I: Integer;
begin
  with Writer do
...
procedure TCustomGrid_ts.DefineProperties(Filer: TFiler);

  function DoColWidths: Boolean;
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TCustomGrid_ts(Filer.Ancestor).FColWidths, FColWidths)
    else
  ...
  begin
    if Filer.Ancestor <> nil then
      Result := not CompareExtents(TCustomGrid_ts(Filer.Ancestor).FRowHeights, FRowHeights)
    else
...
procedure TCustomGrid_ts.MoveColumn(FromIndex, ToIndex: Longint);
var
  Rect: TGridRect;
begin
...
procedure TCustomGrid_ts.ColumnMoved(FromIndex, ToIndex: Longint);
begin
end;

procedure TCustomGrid_ts.MoveRow(FromIndex, ToIndex: Longint);
begin
...
procedure TCustomGrid_ts.RowMoved(FromIndex, ToIndex: Longint);
begin
end;

function TCustomGrid_ts.MouseCoord(X, Y: Integer): TGridCoord;
var
...
procedure TCustomGrid_ts.MoveColRow(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
begin
  MoveCurrent(ACol, ARow, MoveAnchor, Show);
end;

function TCustomGrid_ts.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := True;
end;

procedure TCustomGrid_ts.SizeChanged(OldColCount, OldRowCount: Longint);
begin
end;

function TCustomGrid_ts.Sizing(X, Y: Integer): Boolean;
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TCustomGrid_ts.TopLeftChanged;
begin
...
procedure TCustomGrid_ts.Paint;
var
  LineColor: TColor;
  DrawInfo: TGridDrawInfo;
  Sel: TGridRect;
...
function TCustomGrid_ts.CalcCoordFromPoint(X, Y: Integer;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TCustomGrid_ts.CalcDrawInfo(var DrawInfo: TGridDrawInfo);
begin
  CalcDrawInfoXY(DrawInfo, ClientWidth, ClientHeight);
end;

procedure TCustomGrid_ts.CalcDrawInfoXY(var DrawInfo: TGridDrawInfo;
  UseWidth, UseHeight: Integer);
...
procedure TCustomGrid_ts.CalcFixedInfo(var DrawInfo: TGridDrawInfo);
...
function TCustomGrid_ts.CalcMaxTopLeft(const Coord: TGridCoord;
  const DrawInfo: TGridDrawInfo): TGridCoord;
...
procedure TCustomGrid_ts.CalcSizingState(X, Y: Integer; var State: TGridState;
  var Index: Longint; var SizingPos, SizingOfs: Integer;
...
procedure TCustomGrid_ts.ChangeGridOrientation(RightToLeftOrientation: Boolean);
var
  Org: TPoint;
  Ext: TPoint;
begin
...
procedure TCustomGrid_ts.ChangeSize(NewColCount, NewRowCount: Longint);
var
...
procedure TCustomGrid_ts.ClampInView(const Coord: TGridCoord);
var
  DrawInfo: TGridDrawInfo;
...
procedure TCustomGrid_ts.DrawSizingLine(const DrawInfo: TGridDrawInfo);
var
  OldPen: TPen;
begin
  OldPen := TPen.Create;
...
procedure TCustomGrid_ts.DrawMove;
var
  OldPen: TPen;
  Pos: Integer;
  R: TRect;
...
procedure TCustomGrid_ts.FocusCell(ACol, ARow: Longint; MoveAnchor: Boolean);
begin
...
procedure TCustomGrid_ts.GridRectToScreenRect(GridRect: TGridRect;
  var ScreenRect: TRect; IncludeLine: Boolean);
...
procedure TCustomGrid_ts.Initialize;
begin
...
procedure TCustomGrid_ts.InvalidateCell(ACol, ARow: Longint);
var
  Rect: TGridRect;
begin
  Rect.Top := ARow;
...
procedure TCustomGrid_ts.InvalidateCol(ACol: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TCustomGrid_ts.InvalidateRow(ARow: Longint);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TCustomGrid_ts.InvalidateGrid;
begin
  Invalidate;
end;

procedure TCustomGrid_ts.InvalidateRect(ARect: TGridRect);
var
  InvalidRect: TRect;
begin
  if not HandleAllocated then Exit;
...
procedure TCustomGrid_ts.ModifyScrollBar(ScrollBar, ScrollCode, Pos: Cardinal;
  UseRightToLeft: Boolean);
var
...
procedure TCustomGrid_ts.MoveAdjust(var CellPos: Longint; FromIndex, ToIndex: Longint);
var
  Min, Max: Longint;
begin
...
procedure TCustomGrid_ts.MoveAnchor(const NewAnchor: TGridCoord);
var
  OldSel: TGridRect;
begin
...
procedure TCustomGrid_ts.MoveCurrent(ACol, ARow: Longint; MoveAnchor,
  Show: Boolean);
var
  OldSel: TGridRect;
...
procedure TCustomGrid_ts.MoveTopLeft(ALeft, ATop: Longint);
var
  OldTopLeft: TGridCoord;
begin
...
procedure TCustomGrid_ts.ResizeCol(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TCustomGrid_ts.ResizeRow(Index: Longint; OldSize, NewSize: Integer);
begin
  InvalidateGrid;
end;

procedure TCustomGrid_ts.SelectionMoved(const OldSel: TGridRect);
var
...
procedure TCustomGrid_ts.ScrollDataInfo(DX, DY: Integer;
...
procedure TCustomGrid_ts.ScrollData(DX, DY: Integer);
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TCustomGrid_ts.TopLeftMoved(const OldTopLeft: TGridCoord);
...
procedure TCustomGrid_ts.UpdateScrollPos;
var
  DrawInfo: TGridDrawInfo;
  MaxTopLeft: TGridCoord;
  GridSpace, ColWidth: Integer;
...
procedure TCustomGrid_ts.UpdateScrollRange;
var
...
function TCustomGrid_ts.CreateEditor: TInplaceEdit;
begin
  Result := TInplaceEdit.Create(Self);
end;

procedure TCustomGrid_ts.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params do
  begin
...
procedure TCustomGrid_ts.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TCustomGrid_ts.KeyPress(var Key: Char);
begin
...
procedure TCustomGrid_ts.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  CellHit: TGridCoord;
  DrawInfo: TGridDrawInfo;
...
procedure TCustomGrid_ts.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  CellHit: TGridCoord;
begin
...
procedure TCustomGrid_ts.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  DrawInfo: TGridDrawInfo;
  NewSize: Integer;
...
procedure TCustomGrid_ts.MoveAndScroll(Mouse, CellHit: Integer;
...
function TCustomGrid_ts.GetColWidths(Index: Longint): Integer;
begin
...
function TCustomGrid_ts.GetRowHeights(Index: Longint): Integer;
begin
...
function TCustomGrid_ts.GetGridWidth: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TCustomGrid_ts.GetGridHeight: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TCustomGrid_ts.GetSelection: TGridRect;
begin
  Result := GridRect(FCurrent, FAnchor);
end;

function TCustomGrid_ts.GetTabStops(Index: Longint): Boolean;
begin
  if FTabStops = nil then Result := True
  else Result := Boolean(PIntArray(FTabStops)^[Index + 1]);
end;

function TCustomGrid_ts.GetVisibleColCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
function TCustomGrid_ts.GetVisibleRowCount: Integer;
var
  DrawInfo: TGridDrawInfo;
begin
  CalcDrawInfo(DrawInfo);
...
procedure TCustomGrid_ts.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TCustomGrid_ts.SetCol(Value: Longint);
begin
  if Col <> Value then FocusCell(Value, Row, True);
end;

procedure TCustomGrid_ts.SetColCount(Value: Longint);
begin
...
procedure TCustomGrid_ts.SetColWidths(Index: Longint; Value: Integer);
begin
  if FColWidths = nil then
    UpdateExtents(FColWidths, ColCount, DefaultColWidth);
...
procedure TCustomGrid_ts.SetDefaultColWidth(Value: Integer);
begin
...
procedure TCustomGrid_ts.SetDefaultRowHeight(Value: Integer);
begin
...
procedure TCustomGrid_ts.SetFixedColor(Value: TColor);
begin
...
procedure TCustomGrid_ts.SetFixedCols(Value: Integer);
begin
...
procedure TCustomGrid_ts.SetFixedRows(Value: Integer);
begin
...
procedure TCustomGrid_ts.SetEditorMode(Value: Boolean);
begin
  if not Value then
    HideEditor
  else
...
procedure TCustomGrid_ts.SetGridLineWidth(Value: Integer);
begin
...
procedure TCustomGrid_ts.SetLeftCol(Value: Longint);
begin
  if FTopLeft.X <> Value then MoveTopLeft(Value, TopRow);
end;

procedure TCustomGrid_ts.SetOptions(Value: TGridOptions);
begin
...
procedure TCustomGrid_ts.SetRow(Value: Longint);
begin
  if Row <> Value then FocusCell(Col, Value, True);
end;

procedure TCustomGrid_ts.SetRowCount(Value: Longint);
begin
...
procedure TCustomGrid_ts.SetRowHeights(Index: Longint; Value: Integer);
begin
  if FRowHeights = nil then
    UpdateExtents(FRowHeights, RowCount, DefaultRowHeight);
...
procedure TCustomGrid_ts.SetScrollBars(Value: TScrollStyle);
begin
...
procedure TCustomGrid_ts.SetSelection(Value: TGridRect);
var
  OldSel: TGridRect;
begin
  OldSel := Selection;
...
procedure TCustomGrid_ts.SetTabStops(Index: Longint; Value: Boolean);
begin
  if FTabStops = nil then
    UpdateExtents(FTabStops, ColCount, Integer(True));
  if Index >= ColCount then InvalidOp(SIndexOutOfRange);
...
procedure TCustomGrid_ts.SetTopRow(Value: Longint);
begin
  if FTopLeft.Y <> Value then MoveTopLeft(LeftCol, Value);
end;

procedure TCustomGrid_ts.HideEdit;
begin
  if FInplaceEdit <> nil then
    try
      UpdateText;
...
procedure TCustomGrid_ts.UpdateEdit;
...
procedure TCustomGrid_ts.WMCommand(var Message: TWMCommand);
begin
  with Message do
  begin
...
procedure TCustomGrid_ts.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
...
procedure TCustomGrid_ts.WMKillFocus(var Msg: TWMKillFocus);
begin
  inherited;
  InvalidateRect(Selection);
...
procedure TCustomGrid_ts.WMLButtonDown(var Message: TMessage);
begin
  inherited;
  if FInplaceEdit <> nil then FInplaceEdit.FClickTime := GetMessageTime;
end;

procedure TCustomGrid_ts.WMNCHitTest(var Msg: TWMNCHitTest);
begin
  DefaultHandler(Msg);
  FHitTest := ScreenToClient(SmallPointToPoint(Msg.Pos));
end;

procedure TCustomGrid_ts.WMSetCursor(var Msg: TWMSetCursor);
var
  DrawInfo: TGridDrawInfo;
  State: TGridState;
  Index: Longint;
...
procedure TCustomGrid_ts.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
...
procedure TCustomGrid_ts.WMSize(var Msg: TWMSize);
begin
  inherited;
  UpdateScrollRange;
...
procedure TCustomGrid_ts.WMVScroll(var Msg: TWMVScroll);
begin
  ModifyScrollBar(SB_VERT, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TCustomGrid_ts.WMHScroll(var Msg: TWMHScroll);
begin
  ModifyScrollBar(SB_HORZ, Msg.ScrollCode, Msg.Pos, True);
end;

procedure TCustomGrid_ts.CancelMode;
var
  DrawInfo: TGridDrawInfo;
begin
  try
...
procedure TCustomGrid_ts.WMCancelMode(var Msg: TWMCancelMode);
begin
  inherited;
  CancelMode;
end;

procedure TCustomGrid_ts.CMCancelMode(var Msg: TMessage);
begin
...
procedure TCustomGrid_ts.CMFontChanged(var Message: TMessage);
begin
  if FInplaceEdit <> nil then FInplaceEdit.Font := Font;
  inherited;
end;

procedure TCustomGrid_ts.CMCtl3DChanged(var Message: TMessage);
begin
  inherited;
  RecreateWnd;
end;

procedure TCustomGrid_ts.CMDesignHitTest(var Msg: TCMDesignHitTest);
begin
  Msg.Result := Longint(BOOL(Sizing(Msg.Pos.X, Msg.Pos.Y)));
end;

procedure TCustomGrid_ts.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
  inherited;
  if (goEditing in Options) and (Char(Msg.CharCode) = #13) then Msg.Result := 1;
end;

procedure TCustomGrid_ts.TimedScroll(Direction: TGridScrollDirection);
var
...
procedure TCustomGrid_ts.WMTimer(var Msg: TWMTimer);
var
  Point: TPoint;
  DrawInfo: TGridDrawInfo;
...
procedure TCustomGrid_ts.ColWidthsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TCustomGrid_ts.RowHeightsChanged;
begin
  UpdateScrollRange;
  UpdateEdit;
end;

procedure TCustomGrid_ts.DeleteColumn(ACol: Longint);
begin
  MoveColumn(ACol, ColCount-1);
  ColCount := ColCount - 1;
end;

procedure TCustomGrid_ts.DeleteRow(ARow: Longint);
begin
  MoveRow(ARow, RowCount - 1);
  RowCount := RowCount - 1;
end;

procedure TCustomGrid_ts.UpdateDesigner;
var
  ParentForm: TCustomForm;
begin
...
function TCustomGrid_ts.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := inherited DoMouseWheelDown(Shift, MousePos);
  //TopGrid adjustment: turn off CustomGrid mouse wheel behaviour
  {if not Result then
  begin
...
function TCustomGrid_ts.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
...
function TCustomGrid_ts.CheckColumnDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TCustomGrid_ts.CheckRowDrag(var Origin,
  Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TCustomGrid_ts.BeginColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TCustomGrid_ts.BeginRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TCustomGrid_ts.EndColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

function TCustomGrid_ts.EndRowDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := True;
end;

procedure TCustomGrid_ts.CMShowingChanged(var Message: TMessage);
begin
  inherited;
...
{ Sparse array classes for TStringGrid_ts }
...
procedure TStringGrid_tsStrings.Assign(Source: TPersistent);
var
  I, Max: Integer;
begin
...
procedure TStringGrid_tsStrings.CalcXY(Index: Integer; var X, Y: Integer);
begin
  if FIndex = 0 then
  begin
    X := -1; Y := -1;
...
function TStringGrid_tsStrings.Add(const S: string): Integer;
var
  I: Integer;
begin
...
procedure TStringGrid_tsStrings.Clear;
var
...
procedure TStringGrid_tsStrings.Delete(Index: Integer);
begin
  InvalidOp(sInvalidStringGridOp);
end;

function TStringGrid_tsStrings.Get(Index: Integer): string;
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
function TStringGrid_tsStrings.GetCount: Integer;
begin
...
function TStringGrid_tsStrings.GetObject(Index: Integer): TObject;
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TStringGrid_tsStrings.Insert(Index: Integer; const S: string);
begin
  InvalidOp(sInvalidStringGridOp);
end;

procedure TStringGrid_tsStrings.Put(Index: Integer; const S: string);
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TStringGrid_tsStrings.PutObject(Index: Integer; AObject: TObject);
var
  X, Y: Integer;
begin
  CalcXY(Index, X, Y);
...
procedure TStringGrid_tsStrings.SetUpdateState(Updating: Boolean);
begin
...
constructor TStringGrid_ts.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Initialize;
end;

destructor TStringGrid_ts.Destroy;
...
procedure TStringGrid_ts.ColumnMoved(FromIndex, ToIndex: Longint);
...
procedure TStringGrid_ts.RowMoved(FromIndex, ToIndex: Longint);
begin
...
function TStringGrid_ts.GetEditText(ACol, ARow: Longint): string;
begin
  Result := Cells[ACol, ARow];
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure TStringGrid_ts.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  DisableEditUpdate;
  try
...
procedure TStringGrid_ts.DrawCell(ACol, ARow: Longint; ARect: TRect;
  AState: TGridDrawState);
begin
...
procedure TStringGrid_ts.DisableEditUpdate;
begin
  Inc(FEditUpdate);
end;

procedure TStringGrid_ts.EnableEditUpdate;
begin
  Dec(FEditUpdate);
end;

procedure TStringGrid_ts.Initialize;
var
  quantum: TSPAQuantum;
begin
  if FCols = nil then
...
procedure TStringGrid_ts.SetUpdateState(Updating: Boolean);
begin
  FUpdating := Updating; 
...
procedure TStringGrid_ts.Update(ACol, ARow: Integer);
begin
...
function  TStringGrid_ts.EnsureColRow(Index: Integer; IsCol: Boolean):
  TStringGrid_tsStrings;
var
  RCIndex: Integer;
  PList: ^TSparseList;
begin
  ...
  Result := TStringGrid_tsStrings(PList^[Index]);
  if Result = nil then
  begin
    if IsCol then RCIndex := -Index - 1 else RCIndex := Index + 1;
    Result := TStringGrid_tsStrings.Create(Self, RCIndex);
    PList^[Index] := Result;
  end;
end;

function  TStringGrid_ts.EnsureDataRow(ARow: Integer): Pointer;
var
  quantum: TSPAQuantum;
begin
...
function TStringGrid_ts.GetCells(ACol, ARow: Integer): string;
var
...
function TStringGrid_ts.GetCols(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, True);
end;

function TStringGrid_ts.GetObjects(ACol, ARow: Integer): TObject;
var
  ssl: TStringSparseList;
begin
  ssl := TStringSparseList(TSparseList(FData)[ARow]);
...
function TStringGrid_ts.GetRows(Index: Integer): TStrings;
begin
  Result := EnsureColRow(Index, False);
end;

procedure TStringGrid_ts.SetCells(ACol, ARow: Integer; const Value: string);
begin
  TStringGrid_tsStrings(EnsureDataRow(ARow))[ACol] := Value;
  EnsureColRow(ACol, True);
  EnsureColRow(ARow, False);
  Update(ACol, ARow);
end;

procedure TStringGrid_ts.SetCols(Index: Integer; Value: TStrings);
begin
  EnsureColRow(Index, True).Assign(Value);
end;

procedure TStringGrid_ts.SetObjects(ACol, ARow: Integer; Value: TObject);
begin
  TStringGrid_tsStrings(EnsureDataRow(ARow)).Objects[ACol] := Value;
  EnsureColRow(ACol, True);
  EnsureColRow(ARow, False);
  Update(ACol, ARow);
end;

procedure TStringGrid_ts.SetRows(Index: Integer; Value: TStrings);
begin
