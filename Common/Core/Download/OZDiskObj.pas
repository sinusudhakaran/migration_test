...
procedure TOZDisk.ExtractFromStreamOF(const FileName: string;
  FileStream: TStream; ToProc: FileExtractProc);

const
  ProcName = 'TOZDisk.ExtractFromStreamOF';
var
...
function TOZDisk.GetZipStream(FromStream: TStream): TMemoryStream;
const
  ChunkSize = 8192;
...
begin
  (* Profile( 'TOZDisk.GetZipStream' ); *)
   ...
   if ID.idFileType <> 'OZLink' then
      raise FHException.Create('the file is not an Australian BankLink DOS disk file');
   if ID.idVersion <> 'V1.0' then
      raise FHException.CreateFmt('the file is for version %s (V1.0 expected)',[ID.idVersion]);
