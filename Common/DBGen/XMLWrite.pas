...
var
   ...
   begin
        if not Assigned(FieldCode) then  exit;
         ...
         cPart.Add(#09#09'/// </summary>');
         cPart.Add(MakeCode(FieldCode.FCClass) );
         cPart.Add('');
   end;
   ...
   Begin
      ...
      Begin
         ...
         Begin
           ...
           ImplementationPart.Add('var PNode, CNode: IXMLNode;');
           ImplementationPart.Add(#9'I: Integer;');
           ImplementationPart.Add('begin');
...
const
   ...
   SArrayRead =
    #13#10#09'PNode := Node.ChildNodes.FindNode(''%s'');'#13#10 +
    #9'if Assigned(PNode) then begin;'#13#10 +
    #9#9'I := Low(%s);'#13#10 +
    #9#9'CNode := PNode.ChildNodes.FindNode(''%s'');'#13#10 +
    #9#9'while assigned(CNode) and (I <= High(%s)) do begin'#13#10 +
   ...
   SArrayWrite = 'PNode := Result.AddChild(''%s'');'#13#10#09'for I := Low(%s) to High(%s) do begin' +
     #13#10#9#9'CNode := PNode.AddChild(''%s'');'#13#10#09#09'CNode.NodeValue := %s[I];'#13#10#09'end;';


   // C# Fields
   baseField = #09#09'[XmlAttribute("%s", DataType = "%s")]'#13#10#09#09'public %s %s { get; set; }';
   NulbaseField = #09#09'[XmlElement("%s", DataType = "%s")]'#13#10#09#09'public %s? %s { get; set; }';
   ArrayField = #09#09'[XmlArray("%s"),XmlArrayItem("%s", DataType = "%s")]'#13#10#09#09'public %s[] %s { get; set; }';
   MArrayField = #09#09'[XmlArray("%s"),XmlArrayItem("%s", DataType = "%s")]'#13#10#09#09'public %s[,] %s { get; set; }';
...
begin
    ...
    F.FCClass  :=  format(baseField,[Clean,'string','String',save]);
    FieldTypes.Add(F);

    F := tCodeBlock.Create;
    F.fCode := 'BS';
    F.fReadCode  := format(SArrayRead,[Plural,Make,Single,Make, Make]);
    F.fWriteCode := format(SArrayWrite,[Plural, make, make, Single, Make]);
    F.FCClass  :=  format(ArrayField,[Plural,Single,'string','String',save]);
    FieldTypes.Add(F);
    ...
    F.FCClass  :=  format(baseField,[save,'int','Int32',save]);
    {}
    FieldTypes.Add(F);
    ...
    F.FCClass  :=  format(ArrayField,[Plural,Single,'int','Int32',save]);
    FieldTypes.Add(F);
    ...
    F.FCClass  :=  format(baseField,[save,'boolean','bool',save]);
    FieldTypes.Add(F);
    ...
    F.fReadCode  := format(ArrayRead,[Save, Make]);
    F.fWriteCode := format(ArrayWrite,[Plural, Make]);
    F.FCClass  :=  format(ArrayField,[Plural,Single,'boolean','bool',save]);
    FieldTypes.Add(F);
    ...
    F.fReadCode  := format(baseRead,[make, 'Int', save]);
    F.fWriteCode := format(baseWrite,['Int',save, Make]);
    F.FCClass  :=  format(baseField,[save,'unsignedByte','byte',save]);
    FieldTypes.Add(F);
    ...
    F.fReadCode  := format(ArrayRead,[Save, Make]);
    F.fWriteCode := format(ArrayWrite,[Plural, Make]);
    F.FCClass  :=  format(ArrayField,[Plural,Single,'unsignedByte','byte',save]);
    FieldTypes.Add(F);
    ...
    F := tCodeBlock.Create;
    F.fCode := '$';
    F.fReadCode  := format(NulbaseRead,[make, 'Money', save]);
    F.fWriteCode := format(NulbaseWrite,['Money',save, Make]);
    //F.FCClass  :=  format(NulbaseField,[save,'decimal','Decimal',save]);
    F.FCClass  :=  format(baseField,[save,'long','Int64',save]);
    FieldTypes.Add(F);
    ...
    F.FCClass  :=  format(arrayField,[Plural,Single,'long','Int64',save]);
    FieldTypes.Add(F);
