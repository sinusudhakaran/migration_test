!! DO NOT USE - INCOMPATIBLE WITH VISTA !!
...
uses Windows,   //This line will not compile under Delphi 1 -- D1 is not supported
     SysUtils, Classes, Forms, Dialogs, ShellApi, Registry, WinUtils, Types;
  ...
  case aHostType of
    ...
    htKeyHHexe:
      begin
        ...
        if not BKFileExists(appPath) then
...
begin
  ...
  if (aFilename = '') or (not BKFileExists(aFilename)) then exit;  //don't continue if file not found
...
begin
  ...
  if (not BKFileExists(DBG_DIR + DBG_FILENAME)) then
    Rewrite(f)  //create
  else
    Append(f);
  if ioresult = 0 then
...
begin
  if BKFileExists(DBG_DIR + DBG_FILENAME) then
...
begin
  ...
  _DebugMode := BKFileExists(_ModuleDir + '\debug.debug');
